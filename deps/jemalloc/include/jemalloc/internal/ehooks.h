 ...
 * those extra bits of computation down below the fast-paths where they get
 * ignored.
 */
...
ehooks_alloc(tsdn_t *tsdn, ehooks_t *ehooks, void *new_addr, size_t size,
  size_t alignment, bool *zero, bool *commit) {
    ...
    if (extent_hooks == &ehooks_default_extent_hooks) {
        ret = ehooks_default_alloc_impl(tsdn, new_addr, size, alignment, zero,
          commit, ehooks_ind_get(ehooks));
    } else {
        ehooks_pre_reentrancy(tsdn);
        ret = extent_hooks->alloc(extent_hooks, new_addr, size, alignment, zero,
          commit, ehooks_ind_get(ehooks));
...
ehooks_dalloc(
  tsdn_t *tsdn, ehooks_t *ehooks, void *addr, size_t size, bool committed) {
    ...
    if (extent_hooks == &ehooks_default_extent_hooks) {
        ...
        ehooks_pre_reentrancy(tsdn);
        bool err = extent_hooks->dalloc(
          extent_hooks, addr, size, committed, ehooks_ind_get(ehooks));
        ehooks_post_reentrancy(tsdn);
        return err;
    }
}
...
ehooks_commit(tsdn_t *tsdn, ehooks_t *ehooks, void *addr, size_t size,
  size_t offset, size_t length) {
    ...
    if (extent_hooks == &ehooks_default_extent_hooks) {
        ...
        ehooks_pre_reentrancy(tsdn);
        err = extent_hooks->commit(
          extent_hooks, addr, size, offset, length, ehooks_ind_get(ehooks));
        ehooks_post_reentrancy(tsdn);
    }
    if (!err) {
...
ehooks_decommit(tsdn_t *tsdn, ehooks_t *ehooks, void *addr, size_t size,
  size_t offset, size_t length) {
    ...
    if (extent_hooks == &ehooks_default_extent_hooks) {
        ...
        ehooks_pre_reentrancy(tsdn);
        bool err = extent_hooks->decommit(
          extent_hooks, addr, size, offset, length, ehooks_ind_get(ehooks));
        ehooks_post_reentrancy(tsdn);
        return err;
    }
...
ehooks_purge_lazy(tsdn_t *tsdn, ehooks_t *ehooks, void *addr, size_t size,
  size_t offset, size_t length) {
    ...
    if (extent_hooks->purge_lazy == NULL) {
        ...
        ehooks_pre_reentrancy(tsdn);
        bool err = extent_hooks->purge_lazy(
          extent_hooks, addr, size, offset, length, ehooks_ind_get(ehooks));
        ehooks_post_reentrancy(tsdn);
        return err;
    }
...
ehooks_purge_forced(tsdn_t *tsdn, ehooks_t *ehooks, void *addr, size_t size,
  size_t offset, size_t length) {
    ...
    if (extent_hooks->purge_forced == NULL) {
        ...
        ehooks_pre_reentrancy(tsdn);
        bool err = extent_hooks->purge_forced(
          extent_hooks, addr, size, offset, length, ehooks_ind_get(ehooks));
        ehooks_post_reentrancy(tsdn);
        return err;
    }
...
ehooks_split(tsdn_t *tsdn, ehooks_t *ehooks, void *addr, size_t size,
  size_t size_a, size_t size_b, bool committed) {
    ...
    if (ehooks_are_default(ehooks)) {
        ...
        ehooks_pre_reentrancy(tsdn);
        bool err = extent_hooks->split(extent_hooks, addr, size, size_a, size_b,
...
ehooks_merge(tsdn_t *tsdn, ehooks_t *ehooks, void *addr_a, size_t size_a,
  void *addr_b, size_t size_b, bool committed) {
    ...
    if (extent_hooks == &ehooks_default_extent_hooks) {
        ...
        ehooks_pre_reentrancy(tsdn);
        bool err = extent_hooks->merge(extent_hooks, addr_a, size_a, addr_b,
