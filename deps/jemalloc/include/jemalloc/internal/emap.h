...
emap_edata_is_acquired(tsdn_t *tsdn, emap_t *emap, edata_t *edata) {
    ...
    rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, &emap->rtree, rtree_ctx,
      (uintptr_t)edata_base_get(edata), /* dependent */ true,
      /* init_missing */ false);
    if (elm == NULL) {
        return true;
    }
    rtree_contents_t contents = rtree_leaf_elm_read(tsdn, &emap->rtree, elm,
      /* dependent */ true);
    if (contents.edata == NULL || contents.metadata.state == extent_state_active
...
emap_alloc_ctx_lookup(
  tsdn_t *tsdn, emap_t *emap, const void *ptr, emap_alloc_ctx_t *alloc_ctx) {
    ...
    rtree_metadata_t metadata =
      rtree_metadata_read(tsdn, &emap->rtree, rtree_ctx, (uintptr_t)ptr);
...
emap_full_alloc_ctx_lookup(tsdn_t *tsdn, emap_t *emap, const void *ptr,
  emap_full_alloc_ctx_t *full_alloc_ctx) {
    ...
    rtree_contents_t contents =
      rtree_read(tsdn, &emap->rtree, rtree_ctx, (uintptr_t)ptr);
