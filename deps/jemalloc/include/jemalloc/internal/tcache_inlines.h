...
tcache_small_bin_disabled(szind_t ind, cache_bin_t *bin) {
    ...
    if (ret && bin != NULL) {
        ...
        assert(ind >= nhbins);
        assert((uintptr_t)(*bin->stack_head) == cache_bin_preceding_junk);
    }
...
tcache_dalloc_small(
  tsd_t *tsd, tcache_t *tcache, void *ptr, szind_t binind, bool slow_path) {
    ...
    if (unlikely(!cache_bin_dalloc_easy(bin, ptr))) {
        ...
        cache_bin_sz_t max =
          cache_bin_info_ncached_max(&tcache_bin_info[binind]);
...
tcache_dalloc_large(
  tsd_t *tsd, tcache_t *tcache, void *ptr, szind_t binind, bool slow_path) {
    ...
    cache_bin_t *bin = &tcache->bins[binind];
    if (unlikely(!cache_bin_dalloc_easy(bin, ptr))) {
        unsigned remain = cache_bin_info_ncached_max(&tcache_bin_info[binind])
