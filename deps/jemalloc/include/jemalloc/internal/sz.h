...
sz_psz2ind(size_t psz) {
     ...
     */
    pszind_t lg_delta =
      (off_to_first_ps_rg == 0) ? LG_PAGE : LG_PAGE + (off_to_first_ps_rg - 1);
...
sz_size2index_compute(size_t size) {
...
#endif
    {
        ...
        szind_t shift =
          (x < SC_LG_NGROUP + LG_QUANTUM) ? 0 : x - (SC_LG_NGROUP + LG_QUANTUM);
...
sz_index2size_compute(szind_t index) {
...
#endif
    {
        ...
        size_t grp_size_mask = ~((!!grp) - 1);
        size_t grp_size =
          ((ZU(1) << (LG_QUANTUM + (SC_LG_NGROUP - 1))) << grp) & grp_size_mask;
