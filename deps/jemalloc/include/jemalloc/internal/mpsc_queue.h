...
#define mpsc_queue_proto(a_attr, a_prefix, a_queue_type, a_type, a_list_type)  \
    /* Initialize a queue. */                                                  \
    a_attr void a_prefix##new (a_queue_type * queue);                          \
    /* Insert all items in src into the queue, clearing src. */                \
    a_attr void a_prefix##push_batch(a_queue_type *queue, a_list_type *src);   \
    /* Insert node into the queue. */                                          \
    a_attr void a_prefix##push(a_queue_type *queue, a_type *node);             \
    /*                                                                         \
     ...
     * in dst).                                                                \
     */                                                                        \
    a_attr void a_prefix##pop_batch(a_queue_type *queue, a_list_type *dst);

#define mpsc_queue_gen(                                                        \
  a_attr, a_prefix, a_queue_type, a_type, a_list_type, a_link)                 \
    a_attr void a_prefix##new (a_queue_type * queue) {                         \
    ...
    a_attr void a_prefix##push_batch(a_queue_type *queue, a_list_type *src) {  \
        ...
        do {                                                                   \
             ...
             */                                                                \
        } while (!atomic_compare_exchange_weak_p(                              \
          &queue->tail, &cur_tail, last, ATOMIC_RELEASE, ATOMIC_RELAXED));     \
        ql_new(src);                                                           \
    }                                                                          \
    a_attr void a_prefix##push(a_queue_type *queue, a_type *node) {            \
