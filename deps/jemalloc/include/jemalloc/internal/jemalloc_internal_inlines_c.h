...
ialloc(tsd_t *tsd, size_t size, szind_t ind, bool zero, bool slow_path) {
    return iallocztm(
      tsd_tsdn(tsd), size, ind, zero, tcache_get(tsd), false, NULL, slow_path);
}
  ...
  bool zero, tcache_t *tcache, arena_t *arena, hook_ralloc_args_t *hook_args) {
    ...
    if (alignment != 0 && ((uintptr_t)ptr & ((uintptr_t)alignment - 1)) != 0) {
        ...
        return iralloct_realign(
          tsdn, ptr, oldsize, size, alignment, zero, tcache, arena, hook_args);
    }
...
JEMALLOC_ALWAYS_INLINE void *
imalloc_fastpath(
  size_t size, void *(fallback_alloc)(size_t, size_t *), size_t *usable_size) {
    ...
    assert(ind < SC_NBINS);
    assert(
      (SC_LOOKUP_MAXCLASS < SC_SMALL_MAXCLASS) && (size <= SC_SMALL_MAXCLASS));
...
iget_defrag_hint(tsdn_t *tsdn, void *ptr) {
    ...
    if (likely(alloc_ctx.slab)) {
        ...
        /* Don't bother moving allocations from the slab currently used for new
         * allocations */
        if (slab != bin->slabcur) {
            ...
            if (free_in_slab) {
                ...
                for (uint32_t i = 0; i < bin_info->n_shards; i++) {
                    ...
                    /* Deduct the regs in full slabs (they're not part of the
                     * game) */
                    unsigned long full_slabs =
                      bb->stats.curslabs - bb->stats.nonfull_slabs;
                    curregs += bb->stats.curregs - full_slabs * bin_info->nregs;
                    if (bb->slabcur) {
                        /* Remove slabcur from the overall utilization (not a
                         * candidate to nove from) */
                        curregs -=
                          bin_info->nregs - edata_nfree_get(bb->slabcur);
                        curslabs -= 1;
                    }
                }
                /* Compare the utilization ratio of the slab in question to the
                 * total average among non-full slabs. To avoid precision loss
                 * in division, we do that by extrapolating the usage of the
                 * slab as if all slabs have the same usage. If this slab is
                 * less used than the average, we'll prefer to move the data to
                 * hopefully more used ones. To avoid stagnation when all slabs
                 * have the same utilization, we give additional 12.5% weight to
                 * the decision to defrag. */
