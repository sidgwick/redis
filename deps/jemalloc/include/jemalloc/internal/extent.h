...
bool extent_decommit_wrapper(
  tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, size_t offset, size_t length);
bool extent_purge_lazy_wrapper(
  tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, size_t offset, size_t length);
bool extent_purge_forced_wrapper(
  tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, size_t offset, size_t length);
...
extent_can_acquire_neighbor(edata_t *edata, rtree_contents_t contents,
  ...
  bool expanding) {
    ...
    if (pai == EXTENT_PAI_PAC) {
        ...
        if (!expanding
          && (edata_committed_get(edata) != edata_committed_get(neighbor))) {
            /*
    ...
    if (opt_retain) {
        assert(edata_arena_ind_get(edata) == edata_arena_ind_get(neighbor));
    } else {
        if (edata_arena_ind_get(edata) != edata_arena_ind_get(neighbor)) {
            return false;
        }
    }
