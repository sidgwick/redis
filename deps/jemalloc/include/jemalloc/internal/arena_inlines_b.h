...
arena_prof_tctx_reset_sampled(tsd_t *tsd, const void *ptr) {
    ...
    edata_t *edata = emap_edata_lookup(tsd_tsdn(tsd), &arena_emap_global, ptr);
...
large_dalloc_safety_checks(edata_t *edata, void *ptr, szind_t szind) {
     ...
     */
    if (unlikely(
          edata == NULL || edata_state_get(edata) != extent_state_active)) {
        safety_check_fail(
...
arena_dalloc(tsdn_t *tsdn, void *ptr, tcache_t *tcache,
  emap_alloc_ctx_t *caller_alloc_ctx, bool slow_path) {
    ...
    if (likely(alloc_ctx.slab)) {
        ...
        arena_dalloc_large(tsdn, ptr, tcache, alloc_ctx.szind, slow_path);
    }
}
...
arena_cache_oblivious_randomize(
  tsdn_t *tsdn, arena_t *arena, edata_t *edata, size_t alignment) {
    ...
    if (alignment < PAGE) {
        ...
        if (!tsdn_null(tsdn)) {
            tsd_t *tsd = tsdn_tsd(tsdn);
            r = (size_t)prng_lg_range_u64(tsd_prng_statep_get(tsd), lg_range);
        } else {
            uint64_t stack_value = (uint64_t)(uintptr_t)&r;
            r = (size_t)prng_lg_range_u64(&stack_value, lg_range);
        }
        uintptr_t random_offset = ((uintptr_t)r) << (LG_PAGE - lg_range);
        edata->e_addr = (void *)((uintptr_t)edata->e_addr + random_offset);
        assert(ALIGNMENT_ADDR2BASE(edata->e_addr, alignment) == edata->e_addr);
    }
}
...
arena_dalloc_bin_locked_step(tsdn_t *tsdn, arena_t *arena, bin_t *bin,
  ...
  void *ptr) {
    ...
    if (nfree == bin_info->nregs) {
        arena_dalloc_bin_locked_handle_newly_empty(tsdn, arena, slab, bin);
        return true;
    } else if (nfree == 1 && slab != bin->slabcur) {
        arena_dalloc_bin_locked_handle_newly_nonempty(tsdn, arena, slab, bin);
    }
    return false;
}
