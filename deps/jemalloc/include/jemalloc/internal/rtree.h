...
rtree_cache_direct_map(uintptr_t key) {
    return (size_t)((key >> rtree_leaf_maskbits()) & (RTREE_CTX_NCACHE - 1));
}
...
rtree_leaf_elm_bits_encode(rtree_contents_t contents) {
    ...
    uintptr_t edata_bits =
      (uintptr_t)contents.edata & (((uintptr_t)1 << LG_VADDR) - 1);
    ...
    uintptr_t metadata_bits =
      szind_bits | state_bits | is_head_bits | slab_bits;
...
rtree_leaf_elm_bits_decode(uintptr_t bits) {
    ...
    uintptr_t state_bits =
      (bits & RTREE_LEAF_STATE_MASK) >> RTREE_LEAF_STATE_SHIFT;
...
#    else
    ...
    contents.edata =
      (edata_t *)((uintptr_t)((intptr_t)(bits << RTREE_NHIB) >> RTREE_NHIB)
        & low_bit_mask);
#    endif
    assert((uintptr_t)contents.edata % (uintptr_t)EDATA_ALIGNMENT == 0);
    return contents;
...
rtree_leaf_elm_read(
  tsdn_t *tsdn, rtree_t *rtree, rtree_leaf_elm_t *elm, bool dependent) {
    ...
    contents.metadata.szind =
      metadata_bits >> (RTREE_LEAF_STATE_SHIFT + RTREE_LEAF_STATE_WIDTH);
...
/*
 * Tries to look up the key in the L1 cache, returning false if there's a hit,
 * or true if there's a miss. Key is allowed to be NULL; returns true in this
 * case.
 */
...
rtree_read_independent(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx,
  uintptr_t key, rtree_contents_t *r_contents) {
    rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, rtree, rtree_ctx, key,
      /* dependent */ false, /* init_missing */ false);
    if (elm == NULL) {
        return true;
    }
