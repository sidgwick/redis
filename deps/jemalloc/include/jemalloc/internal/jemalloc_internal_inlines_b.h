...
arena_choose_impl(tsd_t *tsd, arena_t *arena, bool internal) {
    ...
    if (unlikely(ret == NULL)) {
        ...
        if (tcache_available(tsd)) {
            ...
            if (tcache_slow->arena != NULL) {
                ...
                assert(
                  tcache_slow->arena == arena_get(tsd_tsdn(tsd), 0, false));
                if (tcache_slow->arena != ret) {
                    tcache_arena_reassociate(
                      tsd_tsdn(tsd), tcache_slow, tcache, ret);
                }
            } else {
                tcache_arena_associate(tsd_tsdn(tsd), tcache_slow, tcache, ret);
            }
        }
    }
     ...
     */
    if (have_percpu_arena && PERCPU_ARENA_ENABLED(opt_percpu_arena) && !internal
