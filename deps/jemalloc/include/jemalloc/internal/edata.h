...
edata_binshard_get(const edata_t *edata) {
    unsigned binshard = (unsigned)((edata->e_bits & EDATA_BITS_BINSHARD_MASK)
...
edata_slab_get(const edata_t *edata) {
    return (
      bool)((edata->e_bits & EDATA_BITS_SLAB_MASK) >> EDATA_BITS_SLAB_SHIFT);
}
...
edata_base_get(const edata_t *edata) {
    assert(
      edata->e_addr == PAGE_ADDR2BASE(edata->e_addr) || !edata_slab_get(edata));
...
edata_last_get(const edata_t *edata) {
    return (
      void *)((uintptr_t)edata_base_get(edata) + edata_size_get(edata) - PAGE);
}

static inline void *
edata_past_get(const edata_t *edata) {
    return (void *)((uintptr_t)edata_base_get(edata) + edata_size_get(edata));
}
...
edata_esn_set(edata_t *edata, size_t esn) {
    edata->e_size_esn =
      (edata->e_size_esn & ~EDATA_ESN_MASK) | (esn & EDATA_ESN_MASK);
}
...
edata_nfree_binshard_set(edata_t *edata, unsigned nfree, unsigned binshard) {
    ...
    assert(binshard < bin_infos[edata_szind_get(edata)].n_shards);
    edata->e_bits =
      (edata->e_bits & (~EDATA_BITS_NFREE_MASK & ~EDATA_BITS_BINSHARD_MASK))
      | ((uint64_t)binshard << EDATA_BITS_BINSHARD_SHIFT)
      | ((uint64_t)nfree << EDATA_BITS_NFREE_SHIFT);
}
...
edata_prof_recent_alloc_set_dont_call_directly(
  edata_t *edata, prof_recent_t *recent_alloc) {
    atomic_store_p(
      &edata->e_prof_info.e_prof_recent_alloc, recent_alloc, ATOMIC_RELAXED);
}
...
edata_cmp_summary_get(const edata_t *edata) {
    return (edata_cmp_summary_t){
      edata_sn_get(edata), (uintptr_t)edata_addr_get(edata)};
}
