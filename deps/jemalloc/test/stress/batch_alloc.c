...
batch_alloc_wrapper(size_t batch) {
    ...
    assert_d_eq(mallctlbymib(mib, miblen, &filled, &len, &batch_alloc_packet,
                  sizeof(batch_alloc_packet)),
      0, "");
...
compare_without_free(size_t batch, size_t iter,
  ...
  void (*item_alloc_without_free_func)(void)) {
    ...
    item_ptrs_next = 0;
    compare_funcs(0, iter, "batch allocation", batch_alloc_without_free_func,
...
TEST_BEGIN(test_tiny_batch_with_free) {
    compare_with_free(TINY_BATCH, TINY_BATCH_ITER, batch_alloc_with_free_tiny,
      item_alloc_with_free_tiny);
}
TEST_END
...
TEST_BEGIN(test_huge_batch_with_free) {
    compare_with_free(HUGE_BATCH, HUGE_BATCH_ITER, batch_alloc_with_free_huge,
      item_alloc_with_free_huge);
}
TEST_END
