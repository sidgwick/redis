...
mallctl_long(void) {
    ...
    int err = mallctl("stats.arenas.0.bins.0.nmalloc", &nmalloc, &sz, NULL, 0);
    assert_d_eq(err, 0, "mallctl failure");
}
...
TEST_BEGIN(test_mallctl_vs_mallctlbymib_long) {
    ...
    size_t miblen = sizeof(mib_long) / sizeof(mib_long[0]);
    int err =
      mallctlnametomib("stats.arenas.0.bins.0.nmalloc", mib_long, &miblen);
    assert_d_eq(err, 0, "mallctlnametomib failure");
...
main(void) {
    return test_no_reentrancy(
      test_mallctl_vs_mallctlbymib_short, test_mallctl_vs_mallctlbymib_long);
}
