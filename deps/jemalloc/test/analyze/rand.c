...
print_buckets(const size_t buckets[], const size_t means[],
  const size_t stddevs[], const size_t n_bucket) {
    for (size_t i = 0; i < n_bucket; ++i) {
        malloc_printf("%zu:\tmean = %zu,\tstddev = %zu,\tbucket = %zu", i,
          means[i], stddevs[i], buckets[i]);
...
#define QUOTIENT_CEIL(n, d) (((n) - 1) / (d) + 1)
    ...
    const size_t q =
      1 << QUOTIENT_CEIL(
        LG_CEIL(QUOTIENT_CEIL(MIN_BUCKET_MEAN, N_BUCKET * (N_BUCKET - 1))), 2);
...
#    define N_GEO_BUCKET (N_DIVIDE << LG_N_MULTIPLY)
    ...
    assert_d_eq(
      mallctl("prof.reset", NULL, NULL, &lg_prof_sample_test, sizeof(size_t)),
      0, "");
    malloc_printf("lg_prof_sample = %zu\n", lg_prof_sample_test);

    double proportions[N_GEO_BUCKET + 1];
    const double min_proportion =
      fill_geometric_proportions(proportions, N_GEO_BUCKET + 1, N_DIVIDE);
    const size_t n_iter = round_to_nearest(MIN_BUCKET_MEAN / min_proportion);
    ...
    assert_d_eq(
      mallctl("prof.reset", NULL, NULL, &lg_prof_sample_orig, sizeof(size_t)),
      0, "");
