...
test_extent_body(unsigned arena_ind) {
    ...
    expect_d_eq(mallctl("arenas.lextent.1.size", (void *)&large1, &sz, NULL, 0),
      0, "Unexpected arenas.lextent.1.size failure");
    expect_d_eq(mallctl("arenas.lextent.2.size", (void *)&large2, &sz, NULL, 0),
      0, "Unexpected arenas.lextent.2.size failure");

    /* Test dalloc/decommit/purge cascade. */
    purge_miblen = sizeof(purge_mib) / sizeof(size_t);
    expect_d_eq(mallctlnametomib("arena.0.purge", purge_mib, &purge_miblen), 0,
    ...
    expect_d_eq(mallctlbymib(purge_mib, purge_miblen, NULL, NULL, NULL, 0), 0,
      "Unexpected arena.%u.purge error", arena_ind);
    if (xallocx_success_a) {
        expect_true(called_dalloc, "Expected dalloc call");
        expect_true(called_decommit, "Expected decommit call");
        expect_true(did_purge_lazy || did_purge_forced, "Expected purge");
    ...
    if (did_split) {
        expect_b_eq(did_decommit, did_commit, "Expected decommit/commit match");
    }
...
test_manual_hook_auto_arena(void) {
    ...
    expect_d_eq(
      mallctlnametomib("arena.0.extent_hooks", hooks_mib, &hooks_miblen), 0,
      "Unexpected mallctlnametomib() failure");
    hooks_mib[1] = 1;
...
test_manual_hook_body(void) {
    ...
    expect_d_eq(mallctlbymib(hooks_mib, hooks_miblen, (void *)&old_hooks,
      ...
      0, "Unexpected extent_hooks error");
    expect_ptr_ne(
      old_hooks->alloc, extent_alloc_hook, "Unexpected extent_hooks error");
    expect_ptr_ne(
      old_hooks->dalloc, extent_dalloc_hook, "Unexpected extent_hooks error");
    expect_ptr_ne(
      old_hooks->commit, extent_commit_hook, "Unexpected extent_hooks error");
    ...
    expect_ptr_ne(old_hooks->purge_forced, extent_purge_forced_hook,
      "Unexpected extent_hooks error");
    expect_ptr_ne(
      old_hooks->split, extent_split_hook, "Unexpected extent_hooks error");
    expect_ptr_ne(
      old_hooks->merge, extent_merge_hook, "Unexpected extent_hooks error");
    ...
    expect_ptr_eq(
      old_hooks->alloc, default_hooks->alloc, "Unexpected extent_hooks error");
    ...
    expect_ptr_eq(old_hooks->purge_forced, default_hooks->purge_forced,
      "Unexpected extent_hooks error");
    expect_ptr_eq(
      old_hooks->split, default_hooks->split, "Unexpected extent_hooks error");
    expect_ptr_eq(
      old_hooks->merge, default_hooks->merge, "Unexpected extent_hooks error");
}
...
test_arenas_create_ext_base(
  arena_config_t config, bool expect_hook_data, bool expect_hook_metadata) {
    ...
    called_alloc = false;
    expect_d_eq(mallctl("experimental.arenas_create_ext", (void *)&arena, &sz,
                  &config, sizeof(arena_config_t)),
      0, "Unexpected mallctl() failure");
    ...
    expect_b_eq(
      called_alloc, expect_hook_data, "expected hook data alloc mismatch");
