...
TEST_BEGIN(test_oom) {
    ...
    expect_true(oom,
      "Expected OOM during series of calls to smallocx(size=%zu, 0)", largemax);
    for (i = 0; i < sizeof(ptrs) / sizeof(void *); i++) {
        if (ptrs[i] != NULL) {
            dallocx(ptrs[i], 0);
        }
...
#if LG_SIZEOF_PTR == 3
    expect_ptr_null(
      smallocx(0x8000000000000000ULL, MALLOCX_ALIGN(0x8000000000000000ULL)).ptr,
...
#define MAXSZ (((size_t)1) << 23)
    ...
    for (sz = 1; sz < MAXSZ; sz = nallocx(sz, 0) + 1) {
        ...
        p = ret.ptr;
        expect_ptr_not_null(
          p, "Unexpected smallocx(size=%zx, flags=MALLOCX_ZERO) error", nsz);
        rsz = sallocx(p, 0);
    ...
    for (alignment = 8; alignment <= MAXALIGN; alignment <<= 1) {
        ...
        for (sz = 1; sz < 3 * alignment && sz < (1U << 31);
          sz += (alignment >> (LG_SIZEOF_PTR - 1)) - 1) {
            for (i = 0; i < NITER; i++) {
                nsz = nallocx(sz, MALLOCX_ALIGN(alignment) | MALLOCX_ZERO);
                expect_zu_ne(nsz, 0,
