...
arena_ind(void) {
    ...
    if (ind == 0) {
        ...
        expect_d_eq(mallctl("arenas.create", (void *)&ind, &sz, NULL, 0), 0,
...
TEST_BEGIN(test_size) {
    ...
    expect_zu_eq(xallocx(p, 1, 0, 0), small0, "Unexpected xallocx() behavior");

    /* Test largest supported size. */
    expect_zu_le(
      xallocx(p, largemax, 0, 0), largemax, "Unexpected xallocx() behavior");

    /* Test size overflow. */
    expect_zu_le(xallocx(p, largemax + 1, 0, 0), largemax,
      "Unexpected xallocx() behavior");
    expect_zu_le(
      xallocx(p, SIZE_T_MAX, 0, 0), largemax, "Unexpected xallocx() behavior");
...
TEST_BEGIN(test_size_extra_overflow) {
    ...
    expect_zu_le(xallocx(p, largemax - 1, 2, 0), largemax,
      "Unexpected xallocx() behavior");
    expect_zu_le(
      xallocx(p, largemax, 1, 0), largemax, "Unexpected xallocx() behavior");
...
TEST_BEGIN(test_extra_large) {
    ...
    expect_zu_eq(
      xallocx(p, large3, 0, flags), large3, "Unexpected xallocx() behavior");
    /* Test size decrease with zero extra. */
    expect_zu_ge(
      xallocx(p, large1, 0, flags), large1, "Unexpected xallocx() behavior");
    expect_zu_ge(
      xallocx(p, smallmax, 0, flags), large1, "Unexpected xallocx() behavior");
...
#define FILL_BYTE 0x7aU
    ...
    expect_false(
      validate_fill(p, FILL_BYTE, 0, sz), "Memory not filled: sz=%zu", sz);
...
main(void) {
    return test(test_same_size, test_extra_no_move, test_no_move_fail,
      test_size, test_size_extra_overflow, test_extra_small, test_extra_large,
      test_zero_large);
}
