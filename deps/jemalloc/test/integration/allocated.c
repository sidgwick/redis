...
thd_start(void *arg) {
    ...
    if ((err = mallctl("thread.allocated", (void *)&a0, &sz, NULL, 0))) {
        ...
        test_fail("%s(): Error in mallctl(): %s", __func__, strerror(err));
    }
    sz = sizeof(ap0);
    ...
    sz = sizeof(dp0);
    if ((err = mallctl("thread.deallocatedp", (void *)&dp0, &sz, NULL, 0))) {
        if (err == ENOENT) {
            goto label_ENOENT;
        }
        test_fail("%s(): Error in mallctl(): %s", __func__, strerror(err));
    ...
    expect_ptr_eq(
      ap0, ap1, "Pointer returned by \"thread.allocatedp\" should not change");
...
label_ENOENT:
    expect_false(
      config_stats, "ENOENT should only be returned if stats are disabled");
