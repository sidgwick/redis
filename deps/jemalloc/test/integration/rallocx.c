    ...
    for (i = 0; i < NCYCLES; i++) {
        for (j = 1; j < NSZS && szs[j - 1] < MAXSZ; j++) {
            ...
            expect_ptr_not_null(q,
              "Unexpected rallocx() error for size=%zu-->%zu", szs[j - 1],
              szs[j - 1] + 1);
            szs[j] = sallocx(q, 0);
        ...
        for (j--; j > 0; j--) {
            ...
            expect_ptr_not_null(q,
              "Unexpected rallocx() error for size=%zu-->%zu", szs[j],
              szs[j - 1]);
            tsz = sallocx(q, 0);
            expect_zu_eq(tsz, szs[j - 1], "Expected size=%zu, got size=%zu",
              szs[j - 1], tsz);
            p = q;
        }
    }
...
#define RANGE 2048
    ...
    for (i = 0; i < sizeof(start_sizes) / sizeof(size_t); i++) {
        ...
        expect_false(validate_fill(p, 0, 0, psz), "Expected zeroed memory");
        memset(p, FILL_BYTE, psz);
        expect_false(
          validate_fill(p, FILL_BYTE, 0, psz), "Expected filled memory");
        ...
        for (j = 1; j < RANGE; j++) {
            ...
            qsz = sallocx(q, 0);
            if (q != p || qsz != psz) {
                expect_false(validate_fill(q, FILL_BYTE, 0, psz),
                  "Expected filled memory");
                expect_false(validate_fill(q, 0, psz, qsz - psz),
                  "Expected zeroed memory");
            }
            if (psz != qsz) {
                memset((void *)((uintptr_t)q + psz), FILL_BYTE, qsz - psz);
                psz = qsz;
            }
            p = q;
        }
...
#define LG_MAX 15
    for (size_t lg_align = LG_MIN; lg_align <= LG_MAX; ++lg_align) {
        for (size_t lg_size = LG_MIN; lg_size <= LG_MAX; ++lg_size) {
            ...
            for (size_t lg_align_next = LG_MIN; lg_align_next <= LG_MAX;
              ++lg_align_next) {
                int flags = MALLOCX_LG_ALIGN(lg_align);
                void *p = mallocx(1, flags);
                assert_ptr_not_null(p, "Unexpected mallocx() error");
                assert_zu_eq(nallocx(1, flags), TEST_MALLOC_SIZE(p),
                  "Wrong mallocx() usable size");
                int flags_next = MALLOCX_LG_ALIGN(lg_align_next);
                p = rallocx(p, size, flags_next);
                assert_ptr_not_null(p, "Unexpected rallocx() error");
                expect_zu_eq(nallocx(size, flags_next), TEST_MALLOC_SIZE(p),
...
#define MAX_VALIDATE (ZU(1) << 22)
    ...
    for (lg_align++; lg_align <= MAX_LG_ALIGN; lg_align++) {
        ...
        expect_ptr_null((void *)((uintptr_t)q & ((ZU(1) << lg_align) - 1)),
          "%p inadequately aligned for lg_align=%u", q, lg_align);
        sz = sallocx(q, 0);
        if ((sz << 1) <= MAX_VALIDATE) {
            expect_false(validate_fill(q, 0, 0, sz), "Expected zeroed memory");
        } else {
            expect_false(
              validate_fill(q, 0, 0, MAX_VALIDATE), "Expected zeroed memory");
            expect_false(
              validate_fill(
                (void *)((uintptr_t)q + sz - MAX_VALIDATE), 0, 0, MAX_VALIDATE),
              "Expected zeroed memory");
        }
        p = q;
    }
...
int
main(void) {
    return test(test_grow_and_shrink, test_zero, test_align, test_align_enum,
      test_lg_align_and_zero, test_overflow);
}
