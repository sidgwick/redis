...
remote_alloc(void *arg) {
    ...
    sz = sizeof(size_t);
    expect_d_eq(
      mallctl("arenas.lextent.0.size", (void *)&large_sz, &sz, NULL, 0), 0,
      "Unexpected mallctl failure");

    void *ptr = mallocx(large_sz, MALLOCX_ARENA(arena) | MALLOCX_TCACHE_NONE);
    void **ret = (void **)arg;
    *ret = ptr;
...
TEST_BEGIN(test_oom) {
    ...
    expect_true(oom,
      "Expected OOM during series of calls to mallocx(size=%zu, 0)", largemax);
    for (i = 0; i < sizeof(ptrs) / sizeof(void *); i++) {
        if (ptrs[i] != NULL) {
            dallocx(ptrs[i], 0);
        }
...
#if LG_SIZEOF_PTR == 3
    expect_ptr_null(
      mallocx(0x8000000000000000ULL, MALLOCX_ALIGN(0x8000000000000000ULL)),
      "Expected OOM for mallocx()");
    expect_ptr_null(mallocx(0x8000000000000000ULL, MALLOCX_ALIGN(0x80000000)),
      "Expected OOM for mallocx()");
#else
...
#define MAXSZ (((size_t)1) << 23)
    ...
    for (sz = 1; sz < MAXSZ; sz = nallocx(sz, 0) + 1) {
        ...
        expect_ptr_not_null(
          p, "Unexpected mallocx(size=%zx, flags=MALLOCX_ZERO) error", nsz);
        rsz = sallocx(p, 0);
    ...
    for (alignment = 8; alignment <= MAXALIGN; alignment <<= 1) {
        ...
        for (sz = 1; sz < 3 * alignment && sz < (1U << 31);
          sz += (alignment >> (LG_SIZEOF_PTR - 1)) - 1) {
            for (i = 0; i < NITER; i++) {
                nsz = nallocx(sz,
                  MALLOCX_ALIGN(alignment) | MALLOCX_ZERO | MALLOCX_ARENA(0));
                expect_zu_ne(nsz, 0,
                ...
                expect_zu_eq(nsz, rsz,
                  ...
                  "alignment=%zu, size=%zu",
                  alignment, sz);
                expect_ptr_null((void *)((uintptr_t)ps[i] & (alignment - 1)),
