...
TEST_BEGIN(test_slab_sizes) {
    ...
    len = sizeof(page);
    expect_d_eq(
      mallctl("arenas.page", &page, &len, NULL, 0), 0, "page mallctl failure");
    ...
    len = 4;
    expect_d_eq(mallctlnametomib("arenas.bin.0.slab_size", slabmib, &len), 0,
      "slab size mallctlnametomib failure");
    ...
    for (unsigned i = 0; i < nbins; i++) {
        ...
        sizemib[2] = i;
        slabmib[2] = i;
        expect_d_eq(mallctlbymib(sizemib, 4, (void *)&bin_size, &len, NULL, 0),
          0, "bin size mallctlbymib failure");

        len = sizeof(size_t);
        expect_d_eq(mallctlbymib(slabmib, 4, (void *)&slab_size, &len, NULL, 0),
        ...
        if (bin_size < 100) {
             ...
             */
            expect_zu_ge(
              slab_size, biggest_slab_seen, "Slab sizes should go up");
            biggest_slab_seen = slab_size;
        } else if ((100 <= bin_size && bin_size < 128)
          || (128 < bin_size && bin_size <= 200)) {
            expect_zu_eq(slab_size, page, "Forced-small slabs should be small");
        } else if (bin_size == 128) {
            expect_zu_eq(
              slab_size, 2 * page, "Forced-2-page slab should be 2 pages");
