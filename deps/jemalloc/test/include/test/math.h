...
pt_norm(double p) {
    ...
    if (fabs(q) <= 0.425) {
        ...
        if (r <= 5.0) {
            ...
            r -= 1.6;
            ret =
              ((((((((7.74545014278341407640e-4 * r + 2.27238449892691845833e-2)
                        * r
            ...
            r -= 5.0;
            ret =
              ((((((((2.01033439929228813265e-7 * r + 2.71155556874348757815e-5)
                        * r
...
pt_chi2(double p, double df, double ln_gamma_df_2) {
    ...
    if (df < -1.24 * log(p)) {
        ...
        if (df > 0.32) {
            ...
            if (ch > 2.2 * df + 6.0) {
                ch = -2.0 * (log(1.0 - p) - c * log(0.5 * ch) + ln_gamma_df_2);
            }
        } else {
            ch = 0.4;
            a = log(1.0 - p);
            while (true) {
                ...
                ch -=
                  (1.0 - exp(a + ln_gamma_df_2 + 0.5 * ch + c * aa) * p2 / p1)
                  / t;
    ...
    for (i = 0; i < 20; i++) {
        ...
        a = 0.5 * t - b * c;
        s1 = (210.0
               + a * (140.0 + a * (105.0 + a * (84.0 + a * (70.0 + 60.0 * a)))))
          / 420.0;
