...
extent_commit_hook(extent_hooks_t *extent_hooks, void *addr, size_t size,
  size_t offset, size_t length, unsigned arena_ind) {
    ...
    err = default_hooks->commit(default_hooks, addr, size, offset, length, 0);
    did_commit = !err;
    return err;
}
...
extent_decommit_hook(extent_hooks_t *extent_hooks, void *addr, size_t size,
  size_t offset, size_t length, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks,
      "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(
      extent_hooks->decommit, extent_decommit_hook, "Wrong hook function");
    called_decommit = true;
    if (!try_decommit) {
        return true;
    }
    err = default_hooks->decommit(default_hooks, addr, size, offset, length, 0);
    did_decommit = !err;
    return err;
}
...
extent_purge_lazy_hook(extent_hooks_t *extent_hooks, void *addr, size_t size,
  size_t offset, size_t length, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks,
      "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(
      extent_hooks->purge_lazy, extent_purge_lazy_hook, "Wrong hook function");
...
extent_merge_hook(extent_hooks_t *extent_hooks, void *addr_a, size_t size_a,
  void *addr_b, size_t size_b, bool committed, unsigned arena_ind) {
    ...
    expect_ptr_eq(
      (void *)((uintptr_t)addr_a + size_a), addr_b, "Extents not mergeable");
    called_merge = true;
    if (!try_merge) {
        return true;
    }
...
extent_hooks_prep(void) {
    ...
    expect_d_eq(
      mallctl("arena.0.extent_hooks", (void *)&default_hooks, &sz, NULL, 0), 0,
