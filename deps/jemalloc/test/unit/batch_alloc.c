...
test_wrapper(size_t size, size_t alignment, bool zero, unsigned arena_flag) {
    ...
    arena_t *arena;
    if (arena_flag != 0) {
        arena = arena_get(tsd_tsdn(tsd), MALLOCX_ARENA_GET(arena_flag), false);
    } else {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        for (int j = -1; j <= 1; ++j) {
            ...
            size_t filled =
              batch_alloc_wrapper(global_ptrs, batch, size, flags);
            assert_zu_eq(filled, batch, "");
            verify_batch_basic(tsd, global_ptrs, batch, usize, zero);
            verify_batch_locality(tsd, global_ptrs, batch, usize, arena, nregs);
