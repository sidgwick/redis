...
get_max_size_class(void) {
    ...
    sz = sizeof(unsigned);
    expect_d_eq(mallctl("arenas.nlextents", (void *)&nlextents, &sz, NULL, 0),
...
TEST_BEGIN(test_size_classes) {
    ...
    for (index = 0, size_class = sz_index2size(index);
      ...
      index++, size_class = sz_index2size(index)) {
        ...
        expect_zu_eq(size_class, sz_index2size(sz_size2index(size_class)),
        ...
        expect_zu_eq(size_class,
          (index > 0) ? sz_s2u(sz_index2size(index - 1) + 1) : sz_s2u(1),
    ...
    expect_zu_eq(max_size_class, sz_index2size(sz_size2index(max_size_class)),
...
TEST_BEGIN(test_psize_classes) {
    ...
    for (pind = 0, size_class = sz_pind2sz(pind);
      ...
      pind++, size_class = sz_pind2sz(pind)) {
        ...
        expect_zu_eq(size_class,
          (pind > 0) ? sz_psz2u(sz_pind2sz(pind - 1) + 1) : sz_psz2u(1),
...
TEST_BEGIN(test_overflow) {
    ...
    expect_zu_eq(sz_s2u(ZU(PTRDIFF_MAX) + 1), 0,
      "sz_s2u() should return 0 for unsupported size");
    expect_zu_eq(sz_s2u(SIZE_T_MAX), 0, "sz_s2u() should return 0 on overflow");
...
int
main(void) {
    return test(test_size_classes, test_psize_classes, test_overflow);
}
