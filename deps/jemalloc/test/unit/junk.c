...
#define JUNK_ALLOC(...)                                                        \
    do {                                                                       \
        ...
        if (opt_junk_alloc && !zero) {                                         \
            ...
            expect_zu_eq(last_junked_usize, TEST_MALLOC_SIZE(ptr), "");        \
        }                                                                      \
    } while (0)
    if (!zero && lg_align == 0) {
    ...
    JUNK_ALLOC(mallocx(
      size, zero_flag | MALLOCX_LG_ALIGN(lg_align) | MALLOCX_TCACHE_NONE));
    if (lg_align >= LG_SIZEOF_PTR) {
        void *memalign_result;
        int err = posix_memalign(&memalign_result, (1 << lg_align), size);
        assert_d_eq(err, 0, "");
    ...
    for (size_t i = 0; i < arraylen(zerovals); i++) {
        for (size_t j = 0; j < arraylen(sizevals); j++) {
            for (size_t k = 0; k < arraylen(lg_alignvals); k++) {
                ...
                size_t usize = nallocx(size, MALLOCX_LG_ALIGN(lg_align));

                JUNK_FREE(free(ptr));
                JUNK_FREE(dallocx(ptr, 0));
                JUNK_FREE(dallocx(ptr, MALLOCX_TCACHE_NONE));
                JUNK_FREE(dallocx(ptr, MALLOCX_LG_ALIGN(lg_align)));
                JUNK_FREE(sdallocx(ptr, usize, MALLOCX_LG_ALIGN(lg_align)));
...
TEST_BEGIN(test_realloc_expand) {
    ...
    expect_zu_eq(last_junked_usize, SC_LARGE_MINCLASS - SC_SMALL_MAXCLASS, "");
    free(expanded);
