...
create_test_data(hpa_hooks_t *hooks, hpa_shard_opts_t *opts) {
    ...
    err =
      hpa_shard_init(&test_data->shard, &test_data->central, &test_data->emap,
...
TEST_BEGIN(test_alloc_max) {
    ...
    hpa_shard_t *shard =
      create_test_data(&hpa_hooks_default, &test_hpa_shard_opts_default);
    ...
    edata = pai_alloc(tsdn, &shard->pai, ALLOC_MAX, PAGE, false, false, false,
      &deferred_work_generated);
    expect_ptr_not_null(edata, "Allocation of small max failed");
    edata = pai_alloc(tsdn, &shard->pai, ALLOC_MAX + PAGE, PAGE, false, false,
...
TEST_BEGIN(test_stress) {
    ...
    for (size_t i = 0; i < 100 * 1000; i++) {
        ...
        if (operation == 0) {
            ...
            size_t npages =
              npages_min + prng_range_zu(&prng_state, npages_max - npages_min);
            edata_t *edata = pai_alloc(tsdn, &shard->pai, npages * PAGE, PAGE,
            ...
            pai_dalloc(tsdn, &shard->pai, to_free, &deferred_work_generated);
        }
    }
...
TEST_BEGIN(test_alloc_dalloc_batch) {
    ...
    pai_dalloc_batch(tsdn, &shard->pai, &allocs_list, &deferred_work_generated);
    for (size_t i = NALLOCS / 2; i < NALLOCS; i++) {
        pai_dalloc(tsdn, &shard->pai, allocs[i], &deferred_work_generated);
    }
    ...
    expect_ptr_eq(orig_base, new_base, "Failed to reuse the allocated memory.");
    expect_contiguous(allocs, NALLOCS);
...
TEST_BEGIN(test_defer_time) {
    ...
    for (int i = 0; i < (int)HUGEPAGE_PAGES; i++) {
        edatas[i] = pai_alloc(tsdn, &shard->pai, PAGE, PAGE, false, false,
          false, &deferred_work_generated);
    ...
    for (int i = 0; i < (int)HUGEPAGE_PAGES / 2; i++) {
        pai_dalloc(tsdn, &shard->pai, edatas[i], &deferred_work_generated);
    }
...
main(void) {
    ...
    return test_no_reentrancy(
      test_alloc_max, test_stress, test_alloc_dalloc_batch, test_defer_time);
}
