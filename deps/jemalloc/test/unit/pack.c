...
binind_compute(void) {
    ...
    for (i = 0; i < nbins; i++) {
        ...
        sz = sizeof(size);
        expect_d_eq(mallctlbymib(mib, miblen, (void *)&size, &sz, NULL, 0), 0,
...
TEST_BEGIN(test_pack) {
    ...
    for (i = offset = 0; i < NSLABS; i++) {
        for (j = 0; j < nregs_per_run; j++) {
            void *p =
              mallocx(SZ, MALLOCX_ARENA(arena_ind) | MALLOCX_TCACHE_NONE);
    ...
    for (i = offset = 0; i < NSLABS;
      i++, offset = (offset + 1) % nregs_per_run) {
        for (j = 0; j < nregs_per_run; j++) {
            ...
            dallocx(p, MALLOCX_ARENA(arena_ind) | MALLOCX_TCACHE_NONE);
        }
    }
    ...
    for (i = offset = 0; i < NSLABS;
      i++, offset = (offset + 1) % nregs_per_run) {
        for (j = 0; j < nregs_per_run; j++) {
            ...
            p = mallocx(SZ, MALLOCX_ARENA(arena_ind) | MALLOCX_TCACHE_NONE);
