...
test_empty_list(list_head_t *head) {
    ...
    expect_ptr_null(ql_last(head, link), "Unexpected element for empty list");
...
test_entries_list(list_head_t *head, list_t *entries, unsigned nentries) {
    ...
    expect_c_eq(
      ql_last(head, link)->id, entries[nentries - 1].id, "Element id mismatch");
    ...
    i = 0;
    ql_reverse_foreach(t, head, link) {
        expect_c_eq(t->id, entries[nentries - i - 1].id, "Element id mismatch");
        i++;
    }
...
TEST_BEGIN(test_ql_concat_split) {
    ...
    test_concat_split_entries(entries, NENTRIES / 2, NENTRIES - NENTRIES / 2);
