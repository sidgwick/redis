...
#define TEST_UTIL_EINVAL(node, a, b, c, d, why_inval)                          \
    do {                                                                       \
        assert_d_eq(mallctl("experimental.utilization." node, a, b, c, d),     \
...
#define TEST_UTIL_VALID(node)                                                  \
    do {                                                                       \
        assert_d_eq(                                                           \
          mallctl("experimental.utilization." node, out, &out_sz, in, in_sz),  \
...
#define BIN_NREGS_READ(out) COUNTS(out)[4]
        ...
        expect_zu_le(
          sz, SIZE_READ(out), "Extent size should be at least allocation size");
        ...
        if (!opt_prof && sz <= SC_SMALL_MAXCLASS) {
            ...
            expect_zu_ne(
              NREGS_READ(out), 0, "Extent region count must be positive");
            expect_true(NFREE_READ(out) == 0
                || (SLABCUR_READ(out) != NULL && SLABCUR_READ(out) <= p),
              "Allocation should follow first fit principle");

            if (config_stats) {
                expect_zu_le(BIN_NFREE_READ(out), BIN_NREGS_READ(out),
                  "Bin free count exceeded region count");
                expect_zu_ne(
                  BIN_NREGS_READ(out), 0, "Bin region count must be positive");
                expect_zu_le(NFREE_READ(out), BIN_NFREE_READ(out),
                  "Extent free count exceeded bin free count");
                expect_zu_le(NREGS_READ(out), BIN_NREGS_READ(out),
                  "Extent region count exceeded "
                  "bin region count");
                expect_zu_eq(BIN_NREGS_READ(out) % NREGS_READ(out), 0,
                  "Bin region count isn't a multiple of "
                  "extent region count");
                expect_zu_le(BIN_NFREE_READ(out) - NFREE_READ(out),
            ...
            expect_zu_eq(
              NFREE_READ(out), 0, "Extent free count should be zero");
            expect_zu_eq(
              NREGS_READ(out), 1, "Extent region count should be one");
...
TEST_BEGIN(test_batch) {
    ...
    for (sz = 17; sz <= TEST_MAX_SIZE && sz <= SC_LARGE_MAXCLASS;
      sz += (sz <= SC_SMALL_MAXCLASS ? 1019 : 99991)) {
        ...
        out_sz_ref = out_sz -= 2 * sizeof(size_t);
        TEST_UTIL_BATCH_EINVAL(
          out, &out_sz, in, in_sz, "*oldlenp is not an exact multiple");
        out_sz_ref = out_sz += 2 * sizeof(size_t);
        in_sz -= sizeof(const void *);
        TEST_UTIL_BATCH_EINVAL(
          out, &out_sz, in, in_sz, "*oldlenp and newlen do not match");
        in_sz += sizeof(const void *);
...
#define SIZE_READ(out, i) out[(i) * 3 + 2]
        ...
        if (sz <= SC_SMALL_MAXCLASS && !opt_prof) {
            ...
            expect_zu_le(NREGS_READ(out, 0), SIZE_READ(out, 0),
              "Extent region count exceeded size");
            expect_zu_ne(
              NREGS_READ(out, 0), 0, "Extent region count must be positive");
        } else if (sz > SC_SMALL_MAXCLASS) {
            expect_zu_eq(
              NFREE_READ(out, 0), 0, "Extent free count should be zero");
            expect_zu_eq(
              NREGS_READ(out, 0), 1, "Extent region count should be one");
        }
