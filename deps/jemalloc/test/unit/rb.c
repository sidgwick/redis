...
#define rbtn_black_height(a_type, a_field, a_rbt, r_height)                    \
    do {                                                                       \
        a_type *rbp_bh_t;                                                      \
        for (rbp_bh_t = (a_rbt)->rbt_root, (r_height) = 0; rbp_bh_t != NULL;   \
...
node_summarize(node_t *a, const node_t *lchild, const node_t *rchild) {
    uint64_t new_summary_max_specialness =
      node_subtree_specialness(a, lchild, rchild);
...
tree_iterate_cb(tree_t *tree, node_t *node, void *data) {
    ...
    expect_ptr_eq(search_node, node, "tree_search() returned unexpected node");

    /* Test rb_nsearch(). */
    search_node = tree_nsearch(tree, node);
    expect_ptr_eq(search_node, node, "tree_nsearch() returned unexpected node");

    /* Test rb_psearch(). */
    search_node = tree_psearch(tree, node);
    expect_ptr_eq(search_node, node, "tree_psearch() returned unexpected node");
...
TEST_BEGIN(test_rb_empty) {
    ...
    key.key = 0;
    key.magic = NODE_MAGIC;
    expect_ptr_null(tree_search_filtered(&tree, &key, &specialness_filter_node,
                      &specialness_filter_subtree, NULL),
      "");
    expect_ptr_null(tree_nsearch_filtered(&tree, &key, &specialness_filter_node,
                      &specialness_filter_subtree, NULL),
      "");
    expect_ptr_null(tree_psearch_filtered(&tree, &key, &specialness_filter_node,
                      &specialness_filter_subtree, NULL),
      "");
}
TEST_END
...
tree_recurse(node_t *node, unsigned black_height, unsigned black_depth) {
    ...
    uint64_t expected_subtree_specialness =
      node_subtree_specialness(node, left_node, right_node);
    expect_u64_eq(expected_subtree_specialness, node->summary_max_specialness,
      "Incorrect summary");
    ...
    if (rbtn_red_get(node_t, link, node)) {
        if (left_node != NULL) {
            expect_false(
              rbtn_red_get(node_t, link, left_node), "Node should be black");
        }
        if (right_node != NULL) {
            expect_false(
              rbtn_red_get(node_t, link, right_node), "Node should be black");
        }
    }
...
node_remove(tree_t *tree, node_t *node, unsigned nnodes) {
    ...
    search_node = tree_nsearch(tree, node);
    if (search_node != NULL) {
        expect_u64_ge(search_node->key, node->key, "Key ordering error");
    }
    ...
    for (i = 0; i < NBAGS; i++) {
        ...
        for (j = 1; j <= NNODES; j++) {
            ...
            for (k = 0; k < j; k++) {
                ...
                nodes[k].key = bag[k];
                nodes[k].specialness = gen_rand64_range(sfmt, NNODES);
            ...
            for (k = 0; k < j; k++) {
                ...
                expect_false(tree_empty(&tree), "Tree should not be empty");
                expect_ptr_not_null(
                  tree_first(&tree), "Tree should not be empty");
                expect_ptr_not_null(
                  tree_last(&tree), "Tree should not be empty");
            ...
            case 2: {
                ...
                do {
                    start = tree_iter(
                      &tree, start, remove_iterate_cb, (void *)&nnodes);
                    nnodes--;
                } while (start != NULL);
                expect_u_eq(nnodes, 0, "Removal terminated early");
                break;
            }
            case 3: {
                node_t *start;
                ...
                start = NULL;
                do {
                    start = tree_reverse_iter(
                      &tree, start, remove_reverse_iterate_cb, (void *)&nnodes);
                    nnodes--;
                } while (start != NULL);
                expect_u_eq(nnodes, 0, "Removal terminated early");
                break;
            ...
            case 4: {
                ...
                expect_u_eq(nnodes, 0, "Destruction terminated early");
                break;
            }
            default:
                not_reached();
...
tree_iterate_filtered_cb(tree_t *tree, node_t *node, void *arg) {
    ...
    if (ctx->last_node != NULL) {
        if (ctx->forward) {
            expect_d_lt(
              node_cmp(ctx->last_node, node), 0, "Incorrect iteration order");
        } else {
            expect_d_gt(
              node_cmp(ctx->last_node, node), 0, "Incorrect iteration order");
        }
...
check_consistency(tree_t *tree, node_t nodes[UPDATE_TEST_MAX], int nnodes) {
    ...
    for (int i = 0; i < nnodes; i++) {
        if (nodes[i].specialness >= specialness) {
            real_empty = false;
            if (real_first == NULL || node_cmp(&nodes[i], real_first) < 0) {
                real_first = &nodes[i];
            }
            if (real_last == NULL || node_cmp(&nodes[i], real_last) > 0) {
                real_last = &nodes[i];
            }
        }
    }
    ...
    for (int i = 0; i < nnodes; i++) {
        ...
        next_filtered = tree_next_filtered(tree, &nodes[i],
          &specialness_filter_node, &specialness_filter_subtree, &specialness);
        ...
        real_nsearch_filtered =
          (nodes[i].specialness >= specialness ? &nodes[i]
                                               : real_next_filtered);
        ...
        real_search_filtered =
          (nodes[i].specialness >= specialness ? &nodes[i] : NULL);
    ...
    iter_result =
      tree_iter_filtered(tree, NULL, &tree_iterate_filtered_cb, &ctx,
        &specialness_filter_node, &specialness_filter_subtree, &specialness);
    ...
    for (int i = 0; i < nnodes; i++) {
        for (int j = 0; j < nspecial - nodes[i].filtered_rank; j++) {
            ...
            expect_ptr_eq(sorted_filtered_nodes[nodes[i].filtered_rank + j],
              iter_result, "");
        }
    }
    ...
    iter_result =
      tree_reverse_iter_filtered(tree, NULL, &tree_iterate_filtered_cb, &ctx,
        &specialness_filter_node, &specialness_filter_subtree, &specialness);
    expect_ptr_null(iter_result, "");
    expect_d_eq(nspecial, ctx.ncalls, "");
    ...
    for (int i = 0; i < nspecial; i++) {
        ...
        iter_result = tree_reverse_iter_filtered(tree, NULL,
          &tree_iterate_filtered_cb, &ctx, &specialness_filter_node,
          &specialness_filter_subtree, &specialness);
        expect_ptr_eq(sorted_filtered_nodes[nspecial - i - 1], iter_result, "");
        expect_d_eq(ctx.ncalls, i + 1, "");
    }
    /* Filtered backward iteration from a starting point, with stopping. */
    for (int i = 0; i < nnodes; i++) {
        int surplus_rank = (nodes[i].specialness >= 1 ? 1 : 0);
        for (int j = 0; j < nodes[i].filtered_rank + surplus_rank; j++) {
            ctx.ncalls = 0;
            ctx.last_node = NULL;
            ctx.ncalls_max = j + 1;
            iter_result = tree_reverse_iter_filtered(tree, &nodes[i],
              ...
              &specialness_filter_subtree, &specialness);
            expect_d_eq(j + 1, ctx.ncalls, "");
            expect_ptr_eq(sorted_filtered_nodes[nodes[i].filtered_rank - j - 1
                            + surplus_rank],
              iter_result, "");
        }
    }
...
TEST_BEGIN(test_rb_unsummarized) {
    ...
    unsigned nnodes = 0;
    unsummarized_tree_iter(&tree, NULL, &unsummarized_tree_iterate_cb, &nnodes);
