...
test_entries_ring(ring_t *entries) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        ...
        qr_foreach (t, &entries[i], link) {
            expect_c_eq(
              t->id, entries[(i + j) % NENTRIES].id, "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
        j = 0;
        qr_reverse_foreach(t, &entries[i], link) {
            expect_c_eq(t->id, entries[(NENTRIES + i - j - 1) % NENTRIES].id,
              "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
        t = qr_next(&entries[i], link);
        expect_c_eq(
          t->id, entries[(i + 1) % NENTRIES].id, "Element id mismatch");
    }
    for (i = 0; i < NENTRIES; i++) {
        t = qr_prev(&entries[i], link);
...
TEST_BEGIN(test_qr_remove) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        ...
        qr_reverse_foreach(t, &entries[i], link) {
            expect_c_eq(
              t->id, entries[NENTRIES - 1 - j].id, "Element id mismatch");
            j++;
        }
...
TEST_BEGIN(test_qr_before_insert) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        j = 0;
        qr_foreach (t, &entries[i], link) {
            expect_c_eq(t->id, entries[(NENTRIES + i - j) % NENTRIES].id,
              "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
        ...
        qr_reverse_foreach(t, &entries[i], link) {
            expect_c_eq(
              t->id, entries[(i + j + 1) % NENTRIES].id, "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
...
test_split_entries(ring_t *entries) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        ...
        qr_foreach (t, &entries[i], link) {
            if (i < SPLIT_INDEX) {
                expect_c_eq(t->id, entries[(i + j) % SPLIT_INDEX].id,
                  "Element id mismatch");
            } else {
                expect_c_eq(t->id,
                  entries[(i + j - SPLIT_INDEX) % (NENTRIES - SPLIT_INDEX)
                    + SPLIT_INDEX]
                    .id,
                  "Element id mismatch");
            }
