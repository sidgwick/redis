...
do_fill_test(cache_bin_t *bin, cache_bin_info_t *info, void **ptrs,
  ...
  cache_bin_sz_t nfill_succeed) {
    ...
    for (cache_bin_sz_t i = 0; i < nfill_succeed; i++) {
        ...
        expect_true(success, "");
        expect_ptr_eq(ptr, (void *)&ptrs[i], "Should pop in order filled");
        expect_true(
          cache_bin_low_water_get(bin, info) == nfill_succeed - i - 1, "");
    }
...
do_batch_alloc_test(cache_bin_t *bin, cache_bin_info_t *info, void **ptrs,
  cache_bin_sz_t nfill, size_t batch) {
    ...
    expect_true(
      cache_bin_low_water_get(bin, info) == nfill - (cache_bin_sz_t)n, "");
    while (cache_bin_ncached_get_local(bin, info) > 0) {
        bool success;
        cache_bin_alloc(bin, &success);
    }
...
TEST_BEGIN(test_cache_bin) {
    ...
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        ...
        success = cache_bin_dalloc_easy(&bin, &ptrs[i]);
        expect_true(
          success, "Should be able to dalloc into a non-full cache bin.");
        expect_true(cache_bin_low_water_get(&bin, &info) == 0,
    ...
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        expect_true(
          cache_bin_low_water_get(&bin, &info) == ncached_max - i, "");
        expect_true(
          cache_bin_ncached_get_local(&bin, &info) == ncached_max - i, "");
        /*
        ...
        expect_true(success, "");
        expect_ptr_eq(
          ptr, &ptrs[ncached_max - i - 1], "Alloc should pop in stack order");
        expect_true(
          cache_bin_low_water_get(&bin, &info) == ncached_max - i - 1, "");
        expect_true(
          cache_bin_ncached_get_local(&bin, &info) == ncached_max - i - 1, "");
    }
    ...
    do_fill_test(&bin, &info, ptrs, ncached_max, ncached_max, ncached_max / 2);
    /* Try to fill all, fail completely. */
    do_fill_test(&bin, &info, ptrs, ncached_max, ncached_max, 0);

    /* Try to fill some, succeed fully. */
    do_fill_test(
      &bin, &info, ptrs, ncached_max, ncached_max / 2, ncached_max / 2);
    /* Try to fill some, succeed partially. */
    do_fill_test(
      &bin, &info, ptrs, ncached_max, ncached_max / 2, ncached_max / 4);
    ...
    do_batch_alloc_test(&bin, &info, ptrs, ncached_max / 2, ncached_max / 2);
    do_batch_alloc_test(&bin, &info, ptrs, ncached_max / 2, ncached_max);
    do_batch_alloc_test(&bin, &info, ptrs, ncached_max / 2, ncached_max / 4);
...
do_flush_stashed_test(cache_bin_t *bin, cache_bin_info_t *info, void **ptrs,
  cache_bin_sz_t nfill, cache_bin_sz_t nstash) {
    expect_true(cache_bin_ncached_get_local(bin, info) == 0, "Bin not empty");
    expect_true(cache_bin_nstashed_get_local(bin, info) == 0, "Bin not empty");
    ...
    expect_true(
      cache_bin_ncached_get_local(bin, info) == nfill, "Wrong cached count");
    ...
    expect_true(
      cache_bin_nstashed_get_local(bin, info) == nstash, "Wrong stashed count");
    ...
    expect_true(
      cache_bin_ncached_get_local(bin, info) == 0, "Wrong cached count");
    expect_true(
      cache_bin_nstashed_get_local(bin, info) == nstash, "Wrong stashed count");
    ...
    expect_true(
      cache_bin_ncached_get_local(bin, info) == 0, "Wrong cached count");
    expect_true(
      cache_bin_nstashed_get_local(bin, info) == 0, "Wrong stashed count");
...
TEST_BEGIN(test_cache_bin_stash) {
    ...
    bool ret;
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        expect_true(cache_bin_ncached_get_local(&bin, &info) == (i / 2 + i % 2),
          "Wrong ncached value");
    ...
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        ...
        if (i < ncached_max / 2) {
            ...
            uintptr_t diff =
              ((uintptr_t)ptr - (uintptr_t)&ptrs[0]) / sizeof(void *);
            expect_true(diff % 2 == 0, "Should be able to alloc");
        } else {
            expect_false(ret, "Should not alloc stashed");
            expect_true(
              cache_bin_nstashed_get_local(&bin, &info) == ncached_max / 2,
              "Wrong nstashed value");
        }
    }
