...
tcache_bytes_read(void) {
    uint64_t epoch;
    assert_d_eq(mallctl("epoch", NULL, NULL, (void *)&epoch, sizeof(epoch)), 0,
...
test_tcache_max_impl(void) {
    ...
    assert_d_eq(mallctl("arenas.tcache_max", (void *)&tcache_max, &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
    ...
    size_t large;
    sz = sizeof(large);
    assert_d_eq(mallctl("arenas.lextent.0.size", (void *)&large, &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
