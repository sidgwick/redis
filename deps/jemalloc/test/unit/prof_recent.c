...
#define ASSERT_SHOULD_FAIL(opt, a, b, c, d)                                    \
    do {                                                                       \
        assert_d_eq(mallctl("experimental.prof_recent." opt, a, b, c, d),      \
...
TEST_BEGIN(test_prof_recent_on) {
    ...
    assert_d_eq(
      mallctl("experimental.prof_recent.alloc_max", NULL, NULL, NULL, 0), 0,
      "no-op mallctl should be allowed");
    confirm_prof_setup();

    assert_d_eq(
      mallctl("experimental.prof_recent.alloc_max", &past, &len, NULL, 0), 0,
      "Read error");
    expect_zd_eq(past, OPT_ALLOC_MAX, "Wrong read result");
    future = OPT_ALLOC_MAX + 1;
    assert_d_eq(
      mallctl("experimental.prof_recent.alloc_max", NULL, NULL, &future, len),
      0, "Write error");
    future = -1;
    assert_d_eq(
      mallctl("experimental.prof_recent.alloc_max", &past, &len, &future, len),
      0, "Read/write error");
    ...
    assert_d_eq(mallctl("experimental.prof_recent.alloc_max", &past, &len,
                  &future, len * 2),
      EINVAL, "Invalid write should return EINVAL");
    expect_zd_eq(past, -1, "Output should not be touched given invalid write");
...
TEST_BEGIN(test_prof_recent_alloc) {
    ...
    for (i = 0; i < 2 * OPT_ALLOC_MAX; ++i) {
        ...
        if (i < OPT_ALLOC_MAX - 1) {
            assert_false(
              ql_empty(&prof_recent_alloc_list), "Empty recent allocation");
            free(p);
            /*
    ...
    assert_u_eq(c, OPT_ALLOC_MAX - 1, "Incorrect total number of allocations");
...
call_dump() {
    ...
    dump_out_len = 0;
    assert_d_eq(mallctl("experimental.prof_recent.alloc_dump", NULL, NULL, in,
                  sizeof(in)),
    ...
    do {                                                                       \
        ...
        while (true) {
            ...
            if (opt_prof_sys_thread_name) {
                ASSERT_FORMATTED_STR("\"%s_thread_name\"", *type);
                ASSERT_CHAR(':');
                ASSERT_CHAR('"');
                while (*start != '"') {
                    ++start;
...
TEST_BEGIN(test_prof_recent_alloc_dump) {
    ...
    assert_zu_eq(lg_prof_sample, (size_t)0, "lg_prof_sample not set correctly");
    ...
    for (int i = 0; i < N_ITERS; i++) {
        ...
        if (rand < data_p->count) {
            ...
            if (rand != data_p->count - 1) {
                ...
                data_p->ptrs[rand] = data_p->ptrs[data_p->count - 1];
            ...
            last_max = prof_recent_alloc_max_ctl_write(tsd, test_max * 2);
        } else if (rand % 5 == 3) {
...
int
main(void) {
    return test(test_confirm_setup, test_prof_recent_off, test_prof_recent_on,
      test_prof_recent_alloc, test_prof_recent_alloc_dump,
