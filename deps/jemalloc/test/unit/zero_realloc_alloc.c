...
allocated() {
    ...
    expect_d_eq(mallctl("thread.allocated", (void *)&allocated, &sz, NULL, 0),
...
deallocated() {
    ...
    expect_d_eq(
      mallctl("thread.deallocated", (void *)&deallocated, &sz, NULL, 0), 0,
...
TEST_BEGIN(test_realloc_alloc) {
    ...
    if (config_stats) {
        expect_u64_lt(
          allocated_before, allocated_after, "Unexpected stats change");
        expect_u64_lt(
          deallocated_before, deallocated_after, "Unexpected stats change");
    }
    dallocx(ptr, 0);
}
TEST_END
