...
#define DO_TESTS(t, ta, val1, val2, val3)                                      \
    do {                                                                       \
        t val;                                                                 \
        t expected;                                                            \
        bool success;                                                          \
        ...
        for (int retry = 0; retry < 10 && !success; retry++) {                 \
            expected = val2;                                                   \
            success = atomic_compare_exchange_weak_##ta(                       \
              &atom, &expected, val3, ATOMIC_RELAXED, ATOMIC_RELAXED);         \
            expect_##ta##_eq(val1, expected, "CAS should update expected");    \
        }                                                                      \
        expect_b_eq(                                                           \
          val1 == val2, success, "Weak CAS did the wrong state update");       \
        val = atomic_load_##ta(&atom, ATOMIC_RELAXED);                         \
        if (success) {                                                         \
            expect_##ta##_eq(                                                  \
              val3, val, "Successful CAS should update atomic");               \
        } else {                                                               \
            expect_##ta##_eq(                                                  \
              val1, val, "Unsuccessful CAS should not update atomic");         \
        }                                                                      \
                                                                               \
        /* Strong CAS. */                                                      \
        atomic_store_##ta(&atom, val1, ATOMIC_RELAXED);                        \
        expected = val2;                                                       \
        success = atomic_compare_exchange_strong_##ta(                         \
          &atom, &expected, val3, ATOMIC_RELAXED, ATOMIC_RELAXED);             \
        expect_b_eq(                                                           \
          val1 == val2, success, "Strong CAS did the wrong state update");     \
        val = atomic_load_##ta(&atom, ATOMIC_RELAXED);                         \
        if (success) {                                                         \
            expect_##ta##_eq(                                                  \
              val3, val, "Successful CAS should update atomic");               \
...
#define DO_INTEGER_TESTS(t, ta, val1, val2)                                    \
    do {                                                                       \
        ...
        expect_##ta##_eq(val1, val, "Fetch-add should return previous value"); \
        val = atomic_load_##ta(&atom, ATOMIC_RELAXED);                         \
        expect_##ta##_eq(val1 + val2, val, "Fetch-add should update atomic");  \
                                                                               \
        /* Fetch-sub. */                                                       \
        atomic_store_##ta(&atom, val1, ATOMIC_RELAXED);                        \
        val = atomic_fetch_sub_##ta(&atom, val2, ATOMIC_RELAXED);              \
        expect_##ta##_eq(val1, val, "Fetch-sub should return previous value"); \
        val = atomic_load_##ta(&atom, ATOMIC_RELAXED);                         \
        expect_##ta##_eq(val1 - val2, val, "Fetch-sub should update atomic");  \
                                                                               \
        /* Fetch-and. */                                                       \
        atomic_store_##ta(&atom, val1, ATOMIC_RELAXED);                        \
        val = atomic_fetch_and_##ta(&atom, val2, ATOMIC_RELAXED);              \
        expect_##ta##_eq(val1, val, "Fetch-and should return previous value"); \
        val = atomic_load_##ta(&atom, ATOMIC_RELAXED);                         \
        expect_##ta##_eq(val1 &val2, val, "Fetch-and should update atomic");   \
                                                                               \
        /* Fetch-or. */                                                        \
        atomic_store_##ta(&atom, val1, ATOMIC_RELAXED);                        \
        val = atomic_fetch_or_##ta(&atom, val2, ATOMIC_RELAXED);               \
        expect_##ta##_eq(val1, val, "Fetch-or should return previous value");  \
        val = atomic_load_##ta(&atom, ATOMIC_RELAXED);                         \
        expect_##ta##_eq(val1 | val2, val, "Fetch-or should update atomic");   \
                                                                               \
        /* Fetch-xor. */                                                       \
        atomic_store_##ta(&atom, val1, ATOMIC_RELAXED);                        \
        val = atomic_fetch_xor_##ta(&atom, val2, ATOMIC_RELAXED);              \
        expect_##ta##_eq(val1, val, "Fetch-xor should return previous value"); \
        val = atomic_load_##ta(&atom, ATOMIC_RELAXED);                         \
        expect_##ta##_eq(val1 ^ val2, val, "Fetch-xor should update atomic");  \
    } while (0)
...
#define TEST_CASES(t)                                                          \
    {{(t) - 1, (t) - 1, (t) - 2}, {(t) - 1, (t)0, (t) - 2},                    \
      {(t) - 1, (t)1, (t) - 2},                                                \
                                                                               \
      {(t)0, (t) - 1, (t) - 2}, {(t)0, (t)0, (t) - 2}, {(t)0, (t)1, (t) - 2},  \
                                                                               \
      {(t)1, (t) - 1, (t) - 2}, {(t)1, (t)0, (t) - 2}, {(t)1, (t)1, (t) - 2},  \
                                                                               \
      {(t)0, (t) - (1 << 22), (t) - 2}, {(t)0, (t)(1 << 22), (t) - 2},         \
      {(t)(1 << 22), (t) - (1 << 22), (t) - 2},                                \
      {(t)(1 << 22), (t)(1 << 22), (t) - 2}}

#define TEST_BODY(t, ta)                                                       \
    do {                                                                       \
...
int
main(void) {
    return test(test_atomic_u64, test_atomic_u32, test_atomic_p, test_atomic_zu,
      test_atomic_zd, test_atomic_u);
}
