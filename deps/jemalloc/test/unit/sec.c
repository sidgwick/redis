  ...
  size_t nallocs, edata_list_active_t *results, bool *deferred_work_generated) {
    ...
    for (size_t i = 0; i < nallocs; i++) {
        ...
        edata_init(edata, /* arena_ind */ 0, (void *)ta->next_ptr, size,
          /* slab */ false, /* szind */ 0, /* sn */ 1, extent_state_active,
          /* zero */ false, /* comitted */ true,
...
TEST_BEGIN(test_reuse) {
    ...
    expect_zu_le(2 * NALLOCS, max_allocs, "Incorrect number of allocations");
    ...
    for (int i = NALLOCS - 1; i >= 0; i--) {
        pai_dalloc(tsdn, &sec.pai, one_page[i], &deferred_work_generated);
    }
    for (int i = NALLOCS - 1; i >= 0; i--) {
        pai_dalloc(tsdn, &sec.pai, two_page[i], &deferred_work_generated);
    }
...
TEST_BEGIN(test_auto_flush) {
    ...
    size_t max_allocs = ta.alloc_count + ta.alloc_batch_count;
    expect_zu_le(NALLOCS + 1, max_allocs, "Incorrect number of allocations");
    expect_zu_eq(0, ta.dalloc_count, "Incorrect number of allocations");
    ...
    expect_zu_eq(max_allocs, ta.alloc_count + ta.alloc_batch_count,
      "Incorrect number of allocations");
    expect_zu_eq(
      0, ta.dalloc_count, "Incorrect number of (non-batch) deallocations");
...
do_disable_flush_test(bool is_disable) {
     ...
     */
    pai_dalloc(tsdn, &sec.pai, allocs[NALLOCS - 1], &deferred_work_generated);
...
TEST_BEGIN(test_max_alloc_respected) {
    ...
    for (size_t i = 0; i < 100; i++) {
        expect_zu_eq(i, ta.alloc_count, "Incorrect number of allocations");
        expect_zu_eq(i, ta.dalloc_count, "Incorrect number of deallocations");
        edata_t *edata = pai_alloc(tsdn, &sec.pai, attempted_alloc, PAGE,
          /* zero */ false, /* guarded */ false,
          /* frequent_reuse */ false, &deferred_work_generated);
        expect_ptr_not_null(edata, "Unexpected alloc failure");
        expect_zu_eq(i + 1, ta.alloc_count, "Incorrect number of allocations");
        expect_zu_eq(i, ta.dalloc_count, "Incorrect number of deallocations");
...
TEST_BEGIN(test_expand_shrink_delegate) {
    ...
    err = pai_shrink(
      tsdn, &sec.pai, edata, 4 * PAGE, 2 * PAGE, &deferred_work_generated);
...
TEST_BEGIN(test_stats_simple) {
    ...
    for (size_t i = 0; i < NITERS; i++) {
        for (size_t j = 0; j < FLUSH_PAGES / 2; j++) {
            pai_dalloc(tsdn, &sec.pai, allocs[j], &deferred_work_generated);
...
TEST_BEGIN(test_stats_auto_flush) {
    ...
    for (size_t i = 0; i < FLUSH_PAGES; i++) {
        pai_dalloc(
          tsdn, &sec.pai, allocs[FLUSH_PAGES + i], &deferred_work_generated);
    }
...
main(void) {
    return test(test_reuse, test_auto_flush, test_disable, test_flush,
      test_max_alloc_respected, test_expand_shrink_delegate, test_nshards_0,
      test_stats_simple, test_stats_auto_flush, test_stats_manual_flush);
}
