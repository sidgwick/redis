...
mock_bt_augmenting_hook(void **vec, unsigned *len, unsigned max_len) {
    ...
    expect_u_lt(
      *len, max_len, "Default backtrace hook returned too large backtrace");
...
mock_dump_hook(const char *filename) {
    ...
    expect_str_eq(
      filename, dump_filename, "Incorrect file name passed to the dump hook");
}
...
TEST_BEGIN(test_prof_backtrace_hook_replace) {
    ...
    expect_d_eq(
      mallctl("experimental.hooks.prof_backtrace", (void *)&default_hook,
        &default_hook_sz, (void *)&hook, sizeof(hook)),
    ...
    expect_d_eq(
      mallctl("experimental.hooks.prof_backtrace", (void *)&current_hook,
        &current_hook_sz, (void *)&default_hook, sizeof(default_hook)),
      0, "Unexpected mallctl failure resetting hook to default");

    expect_ptr_eq(
      current_hook, hook, "Hook returned by mallctl is not equal to mock hook");
...
TEST_BEGIN(test_prof_dump_hook) {
    ...
    expect_d_eq(mallctl("experimental.hooks.prof_dump", (void *)&default_hook,
                  &default_hook_sz, (void *)&hook, sizeof(hook)),
      0, "Unexpected mallctl failure setting hook");
    ...
    expect_d_eq(
      mallctl("experimental.hooks.prof_dump", (void *)&current_hook,
        &current_hook_sz, (void *)&default_hook, sizeof(default_hook)),
      0, "Unexpected mallctl failure resetting hook to default");
