...
TEST_BEGIN(test_mallctl_errors) {
    ...
    expect_d_eq(mallctl("version", NULL, NULL, "0.0.0", strlen("0.0.0")), EPERM,
      "mallctl() should return EPERM on attempt to write "
      "read-only value");

    expect_d_eq(mallctl("epoch", NULL, NULL, (void *)&epoch, sizeof(epoch) - 1),
...
TEST_BEGIN(test_mallctlbymib_errors) {
    ...
    expect_d_eq(mallctlbymib(mib, miblen, NULL, NULL, "0.0.0", strlen("0.0.0")),
      EPERM,
    ...
    expect_d_eq(
      mallctlbymib(mib, miblen, NULL, NULL, (void *)&epoch, sizeof(epoch) - 1),
      EINVAL, "mallctlbymib() should return EINVAL for input size mismatch");
    expect_d_eq(
      mallctlbymib(mib, miblen, NULL, NULL, (void *)&epoch, sizeof(epoch) + 1),
      EINVAL, "mallctlbymib() should return EINVAL for input size mismatch");

    sz = sizeof(epoch) - 1;
    expect_d_eq(mallctlbymib(mib, miblen, (void *)&epoch, &sz, NULL, 0), EINVAL,
...
TEST_BEGIN(test_mallctl_read_write) {
    ...
    /* Blind. */
    expect_d_eq(
      mallctl("epoch", NULL, NULL, NULL, 0), 0, "Unexpected mallctl() failure");
    ...
    /* Write. */
    expect_d_eq(
      mallctl("epoch", NULL, NULL, (void *)&new_epoch, sizeof(new_epoch)), 0,
      "Unexpected mallctl() failure");
    expect_zu_eq(sz, sizeof(old_epoch), "Unexpected output size");

    /* Read+write. */
    expect_d_eq(mallctl("epoch", (void *)&old_epoch, &sz, (void *)&new_epoch,
                  sizeof(new_epoch)),
...
TEST_BEGIN(test_mallctlnametomib_short_mib) {
    ...
    expect_zu_eq(
      mib[3], 42, "mallctlnametomib() wrote past the end of the input mib");
}
TEST_END
...
TEST_BEGIN(test_mallctlbymibname) {
    ...
    assert_d_eq(ctl_bymibname(
                  tsd, mib, 1, "bin.0", &miblen, &result, &len_result, NULL, 0),
      ENOENT, "");
    miblen = 4;
    ...
    assert_d_eq(ctl_bymibname(
                  tsd, mib, 3, "nregs", &miblen, &result, &len_result, NULL, 0),
      0, "");
    assert_zu_eq(miblen, 4, "");
    expect_zu_eq(result, result_ref, "Unexpected result");
}
...
#define TEST_MALLCTL_CONFIG(config, t)                                         \
    do {                                                                       \
        ...
        expect_d_eq(mallctl("config." #config, (void *)&oldval, &sz, NULL, 0), \
          0, "Unexpected mallctl() failure");                                  \
...
#define TEST_MALLCTL_OPT(t, opt, config)                                       \
    do {                                                                       \
        ...
        int result = mallctl("opt." #opt, (void *)&oldval, &sz, NULL, 0);      \
        expect_d_eq(                                                           \
          result, expected, "Unexpected mallctl() result for opt." #opt);      \
        expect_zu_eq(sz, sizeof(oldval), "Unexpected output size");            \
    } while (0)
...
TEST_BEGIN(test_manpage_example) {
    ...
    for (i = 0; i < nbins; i++) {
        ...
        mib[2] = i;
        len = sizeof(bin_size);
        expect_d_eq(mallctlbymib(mib, miblen, (void *)&bin_size, &len, NULL, 0),
...
#define NTCACHES 10
    ...
    for (i = 0; i < NTCACHES; i++) {
        sz = sizeof(unsigned);
        expect_d_eq(mallctl("tcache.create", (void *)&tis[i], &sz, NULL, 0), 0,
          "Unexpected mallctl() failure, i=%u", i);
    }

    /* Exercise tcache ID recycling. */
    for (i = 0; i < NTCACHES; i++) {
        expect_d_eq(mallctl("tcache.destroy", NULL, NULL, (void *)&tis[i],
                      sizeof(unsigned)),
    ...
    for (i = 0; i < NTCACHES; i++) {
        ...
        expect_ptr_not_null(ps[i], "Unexpected mallocx() failure, i=%u", i);
        dallocx(ps[i], MALLOCX_TCACHE(tis[i]));

        qs[i] = mallocx(qsz, MALLOCX_TCACHE(tis[i]));
        expect_ptr_not_null(qs[i], "Unexpected mallocx() failure, i=%u", i);
    ...
    for (i = 0; i < NTCACHES; i++) {
        ...
        expect_ptr_not_null(qs[i], "Unexpected rallocx() failure, i=%u", i);
        if (!san_uaf_detection_enabled()) {
...
TEST_BEGIN(test_thread_arena) {
    ...
    sz = sizeof(unsigned);
    expect_d_eq(mallctl("arenas.narenas", (void *)&narenas, &sz, NULL, 0), 0,
      "Unexpected mallctl() failure");
    ...
    if (strcmp(opa, "disabled") == 0) {
        ...
        expect_d_eq(
          mallctl("thread.arena", (void *)&old_arena_ind, &sz, NULL, 0), 0,
          "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_initialized) {
    ...
    expect_d_eq(mallctlbymib(mib, miblen, &initialized, &sz, NULL, 0), 0,
      "Unexpected mallctl() failure");
    expect_true(
      initialized, "Merged arena statistics should always be initialized");

    /* Equivalent to the above but using mallctl() directly. */
    sz = sizeof(initialized);
    expect_d_eq(mallctl("arena." STRINGIFY(MALLCTL_ARENAS_ALL) ".initialized",
...
TEST_BEGIN(test_arena_i_dirty_decay_ms) {
    ...
    expect_d_eq(mallctl("arena.0.dirty_decay_ms", (void *)&orig_dirty_decay_ms,
                  &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
    ...
    for (prev_dirty_decay_ms = dirty_decay_ms, dirty_decay_ms = -1;
      ...
      prev_dirty_decay_ms = dirty_decay_ms, dirty_decay_ms++) {
        ...
        expect_d_eq(
          mallctl("arena.0.dirty_decay_ms", (void *)&old_dirty_decay_ms, &sz,
            (void *)&dirty_decay_ms, sizeof(ssize_t)),
          0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_muzzy_decay_ms) {
    ...
    expect_d_eq(mallctl("arena.0.muzzy_decay_ms", (void *)&orig_muzzy_decay_ms,
                  &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
    ...
    for (prev_muzzy_decay_ms = muzzy_decay_ms, muzzy_decay_ms = -1;
      ...
      prev_muzzy_decay_ms = muzzy_decay_ms, muzzy_decay_ms++) {
        ...
        expect_d_eq(
          mallctl("arena.0.muzzy_decay_ms", (void *)&old_muzzy_decay_ms, &sz,
            (void *)&muzzy_decay_ms, sizeof(ssize_t)),
          0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_dss) {
    ...
    expect_d_eq(mallctlbymib(mib, miblen, (void *)&dss_prec_old, &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
    expect_str_ne(
...
TEST_BEGIN(test_arena_i_retain_grow_limit) {
    ...
    miblen = sizeof(mib) / sizeof(size_t);
    expect_d_eq(mallctlnametomib("arena.0.retain_grow_limit", mib, &miblen), 0,
      "Unexpected mallctlnametomib() error");
    ...
    new_limit = PAGE - 1;
    expect_d_eq(
      mallctlbymib(mib, miblen, NULL, NULL, &new_limit, sizeof(new_limit)),
      EFAULT, "Unexpected mallctl() success");

    new_limit = PAGE + 1;
    expect_d_eq(
      mallctlbymib(mib, miblen, NULL, NULL, &new_limit, sizeof(new_limit)), 0,
      "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arenas_dirty_decay_ms) {
    ...
    expect_d_eq(mallctl("arenas.dirty_decay_ms", (void *)&orig_dirty_decay_ms,
                  &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
    ...
    for (prev_dirty_decay_ms = dirty_decay_ms, dirty_decay_ms = -1;
      ...
      prev_dirty_decay_ms = dirty_decay_ms, dirty_decay_ms++) {
        ...
        expect_d_eq(
          mallctl("arenas.dirty_decay_ms", (void *)&old_dirty_decay_ms, &sz,
            (void *)&dirty_decay_ms, sizeof(ssize_t)),
          0, "Unexpected mallctl() failure");
        expect_zd_eq(old_dirty_decay_ms, prev_dirty_decay_ms,
...
TEST_BEGIN(test_arenas_muzzy_decay_ms) {
    ...
    expect_d_eq(mallctl("arenas.muzzy_decay_ms", (void *)&orig_muzzy_decay_ms,
                  &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
    ...
    for (prev_muzzy_decay_ms = muzzy_decay_ms, muzzy_decay_ms = -1;
      ...
      prev_muzzy_decay_ms = muzzy_decay_ms, muzzy_decay_ms++) {
        ...
        expect_d_eq(
          mallctl("arenas.muzzy_decay_ms", (void *)&old_muzzy_decay_ms, &sz,
            (void *)&muzzy_decay_ms, sizeof(ssize_t)),
          0, "Unexpected mallctl() failure");
        expect_zd_eq(old_muzzy_decay_ms, prev_muzzy_decay_ms,
...
#define TEST_ARENAS_CONSTANT(t, name, expected)                                \
    do {                                                                       \
        t name;                                                                \
        size_t sz = sizeof(t);                                                 \
        expect_d_eq(mallctl("arenas." #name, (void *)&name, &sz, NULL, 0), 0,  \
          "Unexpected mallctl() failure");                                     \
...
#define TEST_ARENAS_BIN_CONSTANT(t, name, expected)                            \
    do {                                                                       \
        ...
        size_t sz = sizeof(t);                                                 \
        expect_d_eq(                                                           \
          mallctl("arenas.bin.0." #name, (void *)&name, &sz, NULL, 0), 0,      \
          "Unexpected mallctl() failure");                                     \
        expect_zu_eq(name, expected, "Incorrect " #name " size");              \
    } while (0)
...
#define TEST_ARENAS_LEXTENT_CONSTANT(t, name, expected)                        \
    do {                                                                       \
        ...
        size_t sz = sizeof(t);                                                 \
        expect_d_eq(                                                           \
          mallctl("arenas.lextent.0." #name, (void *)&name, &sz, NULL, 0), 0,  \
          "Unexpected mallctl() failure");                                     \
        expect_zu_eq(name, expected, "Incorrect " #name " size");              \
    } while (0)
...
TEST_BEGIN(test_arenas_create) {
    ...
    expect_d_eq(mallctl("arenas.create", (void *)&arena, &sz, NULL, 0), 0,
      "Unexpected mallctl() failure");
    expect_d_eq(mallctl("arenas.narenas", (void *)&narenas_after, &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arenas_lookup) {
    ...
    expect_d_eq(mallctl("arenas.lookup", &arena1, &sz, &ptr, sizeof(ptr)), 0,
      "Unexpected mallctl() failure");
...
#define TEST_STATS_ARENAS(t, name)                                             \
    do {                                                                       \
        ...
        size_t sz = sizeof(t);                                                 \
        expect_d_eq(                                                           \
          mallctl("stats.arenas.0." #name, (void *)&name, &sz, NULL, 0), 0,    \
          "Unexpected mallctl() failure");                                     \
    } while (0)
...
TEST_BEGIN(test_hooks) {
    ...
    err =
      mallctl("experimental.hooks.remove", NULL, NULL, &handle, sizeof(handle));
...
TEST_BEGIN(test_hooks_exhaustion) {
    ...
    for (int i = 0; i < HOOK_MAX; i++) {
        err = mallctl("experimental.hooks.remove", NULL, NULL, &handles[i],
          sizeof(handles[i]));
        expect_d_eq(err, 0, "Hook removal failed");
    }
...
TEST_BEGIN(test_thread_activity_callback) {
    ...
    activity_callback_thunk_t new_thunk = {&activity_test_callback, &test_data};
...
main(void) {
    return test(test_mallctl_errors, test_mallctlnametomib_errors,
      ...
      test_mallctlmibnametomib, test_mallctlbymibname, test_mallctl_config,
      test_mallctl_opt, test_manpage_example, test_tcache_none, test_tcache,
      test_thread_arena, test_arena_i_initialized, test_arena_i_dirty_decay_ms,
      test_arena_i_muzzy_decay_ms, test_arena_i_purge, test_arena_i_decay,
      test_arena_i_dss, test_arena_i_retain_grow_limit,
      test_arenas_dirty_decay_ms, test_arenas_muzzy_decay_ms,
      test_arenas_constants, test_arenas_bin_constants,
      test_arenas_lextent_constants, test_arenas_create, test_arenas_lookup,
      test_prof_active, test_stats_arenas, test_hooks, test_hooks_exhaustion,
      test_thread_idle, test_thread_peak, test_thread_activity_callback);
}
