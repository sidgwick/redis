...
create_arena() {
    ...
    sz = sizeof(unsigned);
    expect_d_eq(mallctl("arenas.create", (void *)&arena_ind, &sz, NULL, 2), 0,
...
get_empty_ndirty(unsigned arena_ind) {
    ...
    err =
      je_mallctl("epoch", (void *)&epoch, &sz, (void *)&epoch, sizeof(epoch));
    expect_d_eq(0, err, "Unexpected mallctl() failure");

    size_t mib[6];
    size_t miblen = sizeof(mib) / sizeof(mib[0]);
    err = mallctlnametomib(
      "stats.arenas.0.hpa_shard.empty_slabs.ndirty_nonhuge", mib, &miblen);
...
set_background_thread_enabled(bool enabled) {
    int err;
    err =
      je_mallctl("background_thread", NULL, NULL, &enabled, sizeof(enabled));
    expect_d_eq(0, err, "Unexpected mallctl failure");
}
...
wait_until_thread_is_enabled(unsigned arena_id) {
    ...
    int iterations = 0;
    do {
        background_thread_info_t *info = background_thread_info_get(arena_id);
...
expect_purging(unsigned arena_ind, bool expect_deferred) {
    ...
    for (int i = 0; i < 10; i++) {
        void *ptr =
          mallocx(PAGE, MALLOCX_TCACHE_NONE | MALLOCX_ARENA(arena_ind));
        empty_ndirty = get_empty_ndirty(arena_ind);
        ...
        if (expect_deferred) {
            ...
            assert_zu_eq(
              0, empty_ndirty, "Saw dirty pages without deferred purging");
        }
        if (empty_ndirty > 0) {
     ...
     */
    unsigned retry = 0;
    while ((empty_ndirty = get_empty_ndirty(arena_ind)) > 0 && expect_deferred
      && (retry++ < 100)) {
        sleep_for_background_thread_interval();
    }
...
main(void) {
    ...
    return test_no_reentrancy(test_hpa_background_thread_purges,
