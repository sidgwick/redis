...
static extent_hooks_t hooks_not_null = {
  extent_alloc_hook, extent_dalloc_hook, extent_destroy_hook, NULL, /* commit */
...
TEST_BEGIN(test_base_hooks_default) {
    ...
    if (config_stats) {
        base_stats_get(tsdn, base, &allocated0, &resident, &mapped, &n_thp);
        expect_zu_ge(
          allocated0, sizeof(base_t), "Base header should count as allocated");
        if (opt_metadata_thp == metadata_thp_always) {
            expect_zu_gt(n_thp, 0, "Base should have 1 THP at least.");
        }
    }
    ...
    if (config_stats) {
        base_stats_get(tsdn, base, &allocated1, &resident, &mapped, &n_thp);
...
TEST_BEGIN(test_base_hooks_not_null) {
     ...
     */
    {
        ...
        for (i = 0; i < sizeof(alignments) / sizeof(size_t); i++) {
            ...
            size_t align_ceil = ALIGNMENT_CEILING(alignment, QUANTUM);
            p = base_alloc(tsdn, base, 1, alignment);
            expect_ptr_not_null(p, "Unexpected base_alloc() failure");
            expect_ptr_eq(p,
              (void *)(ALIGNMENT_CEILING((uintptr_t)p, alignment)),
            ...
            expect_ptr_eq((void *)((uintptr_t)p + align_ceil), q,
              "Minimal allocation should take up %zu bytes", align_ceil);
...
main(void) {
    return test(test_base_hooks_default, test_base_hooks_null,
      test_base_hooks_not_null, test_base_ehooks_get_for_metadata_default_hook,
