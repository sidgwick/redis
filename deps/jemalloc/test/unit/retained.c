...
thd_start(void *arg) {
    for (unsigned next_epoch = 1; next_epoch < NEPOCHS; next_epoch++) {
        ...
        while (
          (cur_epoch = atomic_load_u(&epoch, ATOMIC_ACQUIRE)) != next_epoch) {
...
TEST_BEGIN(test_retained) {
    ...
    for (unsigned e = 1; e < NEPOCHS; e++) {
        ...
        size_t allocated = (esz - guard_sz) * nthreads * PER_THD_NALLOCS;
        ...
        expect_u_eq(do_arena_create(NULL), arena_ind, "Unexpected arena index");
    }
