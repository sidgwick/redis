...
TEST_BEGIN(test_prng_lg_range_u64) {
    ...
    for (lg_range = 63; lg_range > 0; lg_range--) {
        ...
        expect_u64_eq((rb & (UINT64_C(0xffffffffffffffff) << lg_range)), 0,
          "High order bits should be 0, lg_range=%u", lg_range);
...
TEST_BEGIN(test_prng_lg_range_zu) {
    ...
    expect_zu_eq(ra, rb, "Repeated generation should produce repeated results");
    ...
    for (lg_range = (ZU(1) << (3 + LG_SIZEOF_PTR)) - 1; lg_range > 0;
      lg_range--) {
        ...
        expect_zu_eq((rb & (SIZE_T_MAX << lg_range)), 0,
          "High order bits should be 0, lg_range=%u", lg_range);
        expect_zu_eq(rb, (ra >> ((ZU(1) << (3 + LG_SIZEOF_PTR)) - lg_range)),
...
int
main(void) {
    return test_no_reentrancy(test_prng_lg_range_u32, test_prng_lg_range_u64,
      test_prng_lg_range_zu, test_prng_range_u32, test_prng_range_u64,
      test_prng_range_zu);
}
