...
node_validate(const node_t *node, const node_t *parent) {
    ...
    if (parent != NULL) {
        expect_d_ge(
          node_cmp_magic(node, parent), 0, "Child is less than parent");
    }
...
#define SEED 42
    ...
    for (i = 0; i < NBAGS; i++) {
        ...
        for (j = 1; j <= NNODES; j++) {
            ...
            heap_new(&heap);
            expect_u_eq(heap_validate(&heap), 0, "Incorrect node count");
            for (k = 0; k < j; k++) {
            ...
            for (k = 0; k < j; k++) {
                ...
                if (i % 13 == 12) {
                    expect_ptr_not_null(
                      heap_any(&heap), "Heap should not be empty");
                    /* Trigger merging. */
                    expect_ptr_not_null(
                      heap_first(&heap), "Heap should not be empty");
                }
                expect_u_eq(
                  heap_validate(&heap), k + 1, "Incorrect node count");
            }

            expect_false(heap_empty(&heap), "Heap should not be empty");
            ...
            case 0:
                for (k = 0; k < j; k++) {
                    expect_u_eq(
                      heap_validate(&heap), j - k, "Incorrect node count");
                    node_remove(&heap, &nodes[k]);
                    expect_u_eq(
                      heap_validate(&heap), j - k - 1, "Incorrect node count");
                }
                break;
            case 1:
                for (k = j; k > 0; k--) {
            ...
            case 2: {
                ...
                for (k = 0; k < j; k++) {
                    ...
                    if (prev != NULL) {
                        expect_d_ge(
                          node_cmp(node, prev), 0, "Bad removal order");
                    }
                    prev = node;
                }
                break;
            ...
            expect_ptr_null(heap_first(&heap), "Heap should be empty");
            expect_ptr_null(heap_any(&heap), "Heap should be empty");
            expect_true(heap_empty(&heap), "Heap should be empty");
        }
    }
    fini_gen_rand(sfmt);
