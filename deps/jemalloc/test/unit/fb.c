...
#undef NB
  ...
  static void expect_exhaustive_results(fb_group_t *mostly_full,
    fb_group_t *mostly_empty, size_t nbits, size_t special_bit,
    size_t position) {
    if (position < special_bit) {
        ...
        expect_zu_eq(position + 1, fb_ffu(mostly_empty, nbits, position),
          "mismatch at %zu, %zu", position, special_bit);
        expect_zd_eq(position - 1, fb_flu(mostly_empty, nbits, position),
          "mismatch at %zu, %zu", position, special_bit);
...
#undef NB
  ...
  TEST_BEGIN(test_range_simple) {
    ...
    for (size_t i = 0; i < nbits; i++) {
        expect_b_eq(
          i % 2 == 0, fb_get(fb, nbits, i), "mismatch at position %zu", i);
    }
    fb_set_range(fb, nbits, 0, nbits / 2);
    fb_unset_range(fb, nbits, nbits / 2, nbits / 2);
    for (size_t i = 0; i < nbits; i++) {
        expect_b_eq(
          i < nbits / 2, fb_get(fb, nbits, i), "mismatch at position %zu", i);
    }
...
do_test_empty_full_exhaustive(size_t nbits) {
    ...
    for (size_t i = 0; i < nbits; i++) {
        ...
        if (nbits != 1) {
            expect_false(fb_full(empty, nbits), "error at bit %zu", i);
            expect_false(fb_empty(full, nbits), "error at bit %zu", i);
        } else {
            expect_true(fb_full(empty, nbits), "error at bit %zu", i);
            expect_true(fb_empty(full, nbits), "error at bit %zu", i);
        }
...
#undef NB
  ...
  TEST_BEGIN(test_iter_range_simple) {
    ...
    expect_zu_eq(
      set_limit, fb_srange_longest(fb, nbits), "Incorrect longest set range");
    ...
    expect_zu_eq(
      set_limit, fb_urange_longest(fb, nbits), "Incorrect longest unset range");
    for (size_t i = 0; i < set_limit; i++) {
        result = fb_srange_iter(fb, nbits, i, &begin, &len);
        expect_true(result, "Should have found a range at %zu", i);
        expect_zu_eq(set_limit, begin, "Incorrect begin at %zu", i);
...
expect_iter_results_at(
  fb_group_t *fb, size_t nbits, size_t pos, bool val, bool forward) {
    ...
    if (val) {
        if (forward) {
            iter_res = fb_srange_iter(fb, nbits, pos, &iter_begin, &iter_len);
        } else {
            iter_res = fb_srange_riter(fb, nbits, pos, &iter_begin, &iter_len);
        }
    } else {
        if (forward) {
            iter_res = fb_urange_iter(fb, nbits, pos, &iter_begin, &iter_len);
        } else {
            iter_res = fb_urange_riter(fb, nbits, pos, &iter_begin, &iter_len);
        }
    }
    ...
    if (iter_res && simple_iter_res) {
        assert_zu_eq(
          iter_begin, simple_iter_begin, "Begin mismatch at %zu", pos);
...
#undef NB
   ...
   */
  static size_t scount_contiguous(
  ...
  fb_group_t *fb, size_t nbits, size_t set_start, size_t set_end) {
    for (size_t i = 0; i < nbits; i++) {
        for (size_t j = i + 1; j <= nbits; j++) {
            size_t cnt = j - i;
            size_t scount_expected =
              scount_contiguous(set_start, set_end, i, j);
            ...
            size_t ucount_expected =
              ucount_contiguous(set_start, set_end, i, j);
  ...
  fb_group_t *fb_even, fb_group_t *fb_odd, size_t nbits) {
    for (size_t i = 0; i < nbits; i++) {
        for (size_t j = i + 1; j <= nbits; j++) {
            ...
            size_t odd_scount_computed = fb_scount(fb_odd, nbits, i, j - i);
            ...
            size_t odd_ucount_computed = fb_ucount(fb_odd, nbits, i, j - i);
            ...
            size_t even_scount_computed = fb_scount(fb_even, nbits, i, j - i);
            ...
            size_t even_ucount_computed = fb_ucount(fb_even, nbits, i, j - i);
...
#undef NB
  ...
  static void do_test_bit_op(size_t nbits, bool (*op)(bool a, bool b),
    void (*fb_op)(
