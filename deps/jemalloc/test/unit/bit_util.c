...
#define TEST_POW2_CEIL(t, suf, pri)                                            \
    do {                                                                       \
        ...
        for (i = 0; i < sizeof(t) * 8; i++) {                                  \
            expect_##suf##_eq(                                                 \
              pow2_ceil_##suf(((t)1) << i), ((t)1) << i, "Unexpected result"); \
        }                                                                      \
                                                                               \
        for (i = 2; i < sizeof(t) * 8; i++) {                                  \
            expect_##suf##_eq(pow2_ceil_##suf((((t)1) << i) - 1), ((t)1) << i, \
              "Unexpected result");                                            \
        }                                                                      \
                                                                               \
        ...
        for (pow2 = 1; pow2 < 25; pow2++) {                                    \
            for (x = (((t)1) << (pow2 - 1)) + 1; x <= ((t)1) << pow2; x++) {   \
                expect_##suf##_eq(pow2_ceil_##suf(x), ((t)1) << pow2,          \
...
expect_lg_ceil_range(size_t input, unsigned answer) {
    ...
    expect_zu_le(
      input, (ZU(1) << answer), "Got %u as lg_ceil of %zu", answer, input);
...
expect_lg_floor_range(size_t input, unsigned answer) {
    ...
    expect_zu_ge(
      input, (ZU(1) << answer), "Got %u as lg_floor of %zu", answer, input);
    expect_zu_lt(input, (ZU(1) << (answer + 1)), "Got %u as lg_floor of %zu",
      answer, input);
}
...
TEST_BEGIN(test_lg_ceil_floor) {
    ...
    for (int i = 10; i < 8 * (1 << LG_SIZEOF_PTR) - 5; i++) {
        for (size_t j = 0; j < (1 << 4); j++) {
            size_t num1 = ((size_t)1 << i) - j * ((size_t)1 << (i - 4));
            size_t num2 = ((size_t)1 << i) + j * ((size_t)1 << (i - 4));
        ...
        for (unsigned i = 0; i < sizeof(t) * 8; i++) {                         \
            for (unsigned j = 0; j <= i; j++) {                                \
                for (unsigned k = 0; k <= j; k++) {                            \
                    ...
                    x |= (t)1 << j;                                            \
                    x |= (t)1 << k;                                            \
                    expect_##test_suf##_eq(                                    \
                      ffs_##suf(x), k, "Unexpected result, x=%" pri, x);       \
                }                                                              \
            }                                                                  \
        }                                                                      \
    } while (0)
        ...
        for (unsigned i = 0; i < sizeof(t) * 8; i++) {                         \
            for (unsigned j = 0; j <= i; j++) {                                \
                for (unsigned k = 0; k <= j; k++) {                            \
                    ...
                    x |= (t)1 << k;                                            \
                    expect_##test_suf##_eq(                                    \
                      fls_##suf(x), i, "Unexpected result, x=%" pri, x);       \
                }                                                              \
            }                                                                  \
        }                                                                      \
    } while (0)
...
#define TEST_POPCOUNT(t, suf, pri_hex)                                         \
    do {                                                                       \
        ...
        for (unsigned i = 0; i < (1 << sizeof(t)); i++) {                      \
            for (unsigned j = 0; j < 256; j++) {                               \
                ...
                t x = (bmul * j) & mask;                                       \
                expect_u_eq(popcount_byte(i) * popcount_byte(j),               \
                  popcount_##suf(x), "Unexpected result, x=0x%" pri_hex, x);   \
            }                                                                  \
        }                                                                      \
    } while (0)
...
main(void) {
    return test_no_reentrancy(test_pow2_ceil_u64, test_pow2_ceil_u32,
      ...
      test_fls_u_slow, test_fls_lu_slow, test_fls_llu_slow, test_popcount_u,
      test_popcount_u_slow, test_popcount_lu, test_popcount_lu_slow,
      test_popcount_llu, test_popcount_llu_slow);
}
