...
#define NTICKS 3
    ...
    for (i = 0; i < NREPS; i++) {
        ...
        expect_u32_eq(ticker_read(&ticker), 0, "Expected ticker depletion");
        expect_true(ticker_tick(&ticker), "Expected ticker fire (i=%d)", i);
        expect_u32_eq(ticker_read(&ticker), NTICKS, "Expected ticker reset");
    }
#undef NTICKS
}
TEST_END
...
TEST_BEGIN(test_ticker_geom) {
     ...
     * expected ticks.
     */
    expect_u64_ge(total_ticks, niters * ticks * 9 / 10, "Mean off by > 10%%");
    expect_u64_le(total_ticks, niters * ticks * 11 / 10, "Mean off by > 10%%");
}
TEST_END
...
main(void) {
    return test(
      test_ticker_tick, test_ticker_ticks, test_ticker_copy, test_ticker_geom);
}
