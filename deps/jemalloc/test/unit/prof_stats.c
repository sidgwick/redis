...
#define MALLCTL_STR_LEN 64
    ...
    assert_d_eq(
      mallctl(mallctl_accum_str, &accum_stats_orig, &stats_len, NULL, 0), 0,
      "");
    ...
    for (size_t i = 0; i < N_PTRS; ++i) {
        ...
        assert_d_eq(
          mallctl(mallctl_live_str, &live_stats, &stats_len, NULL, 0), 0, "");
        expect_u64_eq(live_stats[0] - live_stats_orig[0], live_req_sum, "");
        expect_u64_eq(live_stats[1] - live_stats_orig[1], live_count, "");
        uint64_t accum_stats[2];
        assert_d_eq(
          mallctl(mallctl_accum_str, &accum_stats, &stats_len, NULL, 0), 0, "");
        expect_u64_eq(accum_stats[0] - accum_stats_orig[0], accum_req_sum, "");
        expect_u64_eq(accum_stats[1] - accum_stats_orig[1], accum_count, "");
    }
...
test_szind_aligned_wrapper(szind_t ind, unsigned lg_align) {
    ...
    test_combinations(sz_size2index(sz_sa2u(sz_index2size(ind), 1 << lg_align)),
