...
prof_tdata_init(tsd_t *tsd) {
    return prof_tdata_init_impl(tsd, prof_thr_uid_alloc(tsd_tsdn(tsd)), 0, NULL,
      prof_thread_active_init_get(tsd_tsdn(tsd)));
}
...
prof_tdata_reinit(tsd_t *tsd, prof_tdata_t *tdata) {
    ...
    char *thread_name = (tdata->thread_name != NULL)
      ? prof_thread_name_alloc(tsd, tdata->thread_name)
      : NULL;
    bool active = tdata->active;

    prof_tdata_detach(tsd, tdata);
    return prof_tdata_init_impl(tsd, thr_uid, thr_discrim, thread_name, active);
}
...
prof_boot0(void) {
    ...
    memcpy(opt_prof_prefix, PROF_PREFIX_DEFAULT, sizeof(PROF_PREFIX_DEFAULT));
}
...
prof_boot2(tsd_t *tsd, base_t *base) {
    ...
    if (malloc_mutex_init(&tdatas_mtx, "prof_tdatas", WITNESS_RANK_PROF_TDATAS,
          malloc_mutex_rank_exclusive)) {
        return true;
    }
    ...
    if (malloc_mutex_init(&prof_dump_mtx, "prof_dump", WITNESS_RANK_PROF_DUMP,
          malloc_mutex_rank_exclusive)) {
        return true;
    }
