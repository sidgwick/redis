...
#endif
    return iallocztm(tsdn, buf_len, sz_size2index(buf_len), false, NULL, true,
...
buf_writer_init(tsdn_t *tsdn, buf_writer_t *buf_writer, write_cb_t *write_cb,
  void *cbopaque, char *buf, size_t buf_len) {
    if (write_cb != NULL) {
        ...
        buf_writer->write_cb =
          je_malloc_message != NULL ? je_malloc_message : wrtmessage;
    }
    ...
    if (buf != NULL) {
        ...
        buf_writer->buf = buf_writer_allocate_internal_buf(tsdn, buf_len);
...
buf_writer_pipe(
  buf_writer_t *buf_writer, read_cb_t *read_cb, void *read_cbopaque) {
    ...
    assert(read_cb != NULL);
    if (buf_writer->buf == NULL) {
        buf_writer_init(TSDN_NULL, &backup_buf_writer, buf_writer->write_cb,
          buf_writer->cbopaque, backup_buf, sizeof(backup_buf));
    ...
    do {
        ...
        nread = read_cb(read_cbopaque, buf_writer->buf + buf_writer->buf_end,
