  ...
  hpa_shard_stats_t *hpastats, sec_stats_t *secstats) {
    ...
    for (szind_t i = 0; i < SC_NSIZES - SC_NBINS; i++) {
        ...
        uint64_t nrequests = locked_read_u64(tsdn,
          LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[i].nrequests);
        ...
        uint64_t nflush = locked_read_u64(tsdn, LOCKEDINT_MTX(arena->stats.mtx),
    ...
    ql_foreach (descriptor, &arena->cache_bin_array_descriptor_ql, link) {
        for (szind_t i = 0; i < nhbins; i++) {
            ...
            cache_bin_nitems_get_remote(
              cache_bin, &tcache_bin_info[i], &ncached, &nstashed);

            astats->tcache_bytes += ncached * sz_index2size(i);
            astats->tcache_stashed_bytes += nstashed * sz_index2size(i);
        }
    }
...
#define READ_ARENA_MUTEX_PROF_DATA(mtx, ind)                                   \
    ...
    malloc_mutex_prof_read(tsdn, &astats->mutex_prof_data[ind], &arena->mtx);  \
...
#undef READ_ARENA_MUTEX_PROF_DATA
    pa_shard_mtx_stats_read(tsdn, &arena->pa_shard, astats->mutex_prof_data);
    ...
    for (szind_t i = 0; i < SC_NBINS; i++) {
        for (unsigned j = 0; j < bin_infos[i].n_shards; j++) {
            bin_stats_merge(tsdn, &bstats[i], arena_get_bin(arena, i, j));
        }
    }
}
...
arena_extent_dalloc_large_prep(tsdn_t *tsdn, arena_t *arena, edata_t *edata) {
    if (config_stats) {
        LOCKEDINT_MTX_LOCK(tsdn, arena->stats.mtx);
        arena_large_dalloc_stats_update(tsdn, arena, edata_usize_get(edata));
        LOCKEDINT_MTX_UNLOCK(tsdn, arena->stats.mtx);
    }
}
...
arena_decay_ms_set(
  tsdn_t *tsdn, arena_t *arena, extent_state_t state, ssize_t decay_ms) {
    ...
    return pa_decay_ms_set(tsdn, &arena->pa_shard, state, decay_ms, eagerness);
}
...
arena_decay_impl(tsdn_t *tsdn, arena_t *arena, decay_t *decay,
  ...
  bool all) {
    if (all) {
        ...
        pac_decay_all(tsdn, &arena->pa_shard.pac, decay, decay_stats, ecache,
          /* fully_decay */ all);
...
arena_should_decay_early(tsdn_t *tsdn, arena_t *arena, decay_t *decay,
  ...
  size_t npages_new) {
    ...
    if (npages_new > 0) {
        uint64_t npurge_new =
          decay_npages_purge_in(decay, remaining_sleep, npages_new);
        info->npages_to_purge_new += npurge_new;
    }
    malloc_mutex_unlock(tsdn, &decay->mtx);
    return info->npages_to_purge_new > ARENA_DEFERRED_PURGE_NPAGES_THRESHOLD;
}
...
arena_maybe_do_deferred_work(
  tsdn_t *tsdn, arena_t *arena, decay_t *decay, size_t npages_new) {
    ...
    } else if (arena_should_decay_early(
                 tsdn, arena, decay, info, &remaining_sleep, npages_new)) {
...
arena_reset(tsd_t *tsd, arena_t *arena) {
    ...
    for (edata_t *edata = edata_list_active_first(&arena->large); edata != NULL;
      edata = edata_list_active_first(&arena->large)) {
...
arena_cache_bin_fill_small(tsdn_t *tsdn, arena_t *arena, cache_bin_t *cache_bin,
  cache_bin_info_t *cache_bin_info, szind_t binind, const unsigned nfill) {
    assert(cache_bin_ncached_get_local(cache_bin, cache_bin_info) == 0);

    const bin_info_t *bin_info = &bin_infos[binind];

    CACHE_BIN_PTR_ARRAY_DECLARE(ptrs, nfill);
    cache_bin_init_ptr_array_for_fill(cache_bin, cache_bin_info, &ptrs, nfill);
    /*
...
arena_dalloc_bin(tsdn_t *tsdn, arena_t *arena, edata_t *edata, void *ptr) {
    ...
    bool ret =
      arena_dalloc_bin_locked_step(tsdn, arena, bin, &info, binind, edata, ptr);
...
arena_new(tsdn_t *tsdn, unsigned ind, const arena_config_t *config) {
    ...
    if (ind == 0) {
        ...
        base =
          base_new(tsdn, ind, config->extent_hooks, config->metadata_use_hooks);
        if (base == NULL) {
            return NULL;
        }
    }
    ...
    if (opt_hpa && ehooks_are_default(base_ehooks_get(base)) && ind != 0) {
        ...
        if (pa_shard_enable_hpa(
              tsdn, &arena->pa_shard, &hpa_shard_opts, &opt_hpa_sec_opts)) {
            goto label_error;
        }
    }
