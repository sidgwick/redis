...
static const ctl_named_node_t thread_node[] = {
  ...
  {NAME("prof"), CHILD(named, thread_prof)}, {NAME("idle"), CTL(thread_idle)}};
...
static const ctl_named_node_t opt_node[] = {{NAME("abort"), CTL(opt_abort)},
  ...
  {NAME("tcache"), CTL(opt_tcache)}, {NAME("tcache_max"), CTL(opt_tcache_max)},
...
static const ctl_named_node_t arena_i_node[] = {
  ...
  {NAME("reset"), CTL(arena_i_reset)}, {NAME("destroy"), CTL(arena_i_destroy)},
...
#define MUTEX_PROF_DATA_NODE(prefix)                                           \
    static const ctl_named_node_t stats_##prefix##_node[] = {                  \
      ...
      {NAME("max_num_thds"),                                                   \
        CTL(stats_##prefix##_max_num_thds)} /* Note that # of current waiting  \
                                               thread not provided. */         \
    };
...
static const ctl_named_node_t stats_arenas_i_hpa_shard_full_slabs_node[] = {
  ...
  {NAME("nactive_huge"), CTL(stats_arenas_i_hpa_shard_full_slabs_nactive_huge)},
  {NAME("ndirty_nonhuge"),
    CTL(stats_arenas_i_hpa_shard_full_slabs_ndirty_nonhuge)},
  {NAME("ndirty_huge"), CTL(stats_arenas_i_hpa_shard_full_slabs_ndirty_huge)}};
...
static const ctl_named_node_t stats_arenas_i_hpa_shard_empty_slabs_node[] = {
  ...
  {NAME("ndirty_huge"), CTL(stats_arenas_i_hpa_shard_empty_slabs_ndirty_huge)}};
...
ctl_arena_clear(ctl_arena_t *ctl_arena) {
    ...
    if (config_stats) {
        ...
        memset(
          ctl_arena->astats->bstats, 0, SC_NBINS * sizeof(bin_stats_data_t));
        memset(ctl_arena->astats->lstats, 0,
          (SC_NSIZES - SC_NBINS) * sizeof(arena_stats_large_t));
        memset(ctl_arena->astats->estats, 0, SC_NPSIZES * sizeof(pac_estats_t));
        memset(&ctl_arena->astats->hpastats, 0, sizeof(hpa_shard_stats_t));
        memset(&ctl_arena->astats->secstats, 0, sizeof(sec_stats_t));
    }
}
...
ctl_arena_stats_amerge(tsdn_t *tsdn, ctl_arena_t *ctl_arena, arena_t *arena) {
    ...
    if (config_stats) {
        ...
        for (i = 0; i < SC_NBINS; i++) {
            bin_stats_t *bstats = &ctl_arena->astats->bstats[i].stats_data;
            ctl_arena->astats->allocated_small +=
              bstats->curregs * sz_index2size(i);
...
#undef OP
        if (!destroyed) {
            ...
            assert(
              atomic_load_zu(&astats->astats.internal, ATOMIC_RELAXED) == 0);
        }
...
ctl_arena_init(tsd_t *tsd, const arena_config_t *config) {
    ...
    if ((ctl_arena = ql_last(&ctl_arenas->destroyed, destroyed_link)) != NULL) {
...
ctl_background_thread_stats_read(tsdn_t *tsdn) {
    ...
    if (!have_background_thread || background_thread_stats_read(tsdn, stats)) {
...
ctl_refresh(tsdn_t *tsdn) {
    ...
    if (config_stats) {
        ...
        ctl_stats->retained =
          ctl_sarena->astats->astats.pa_shard_stats.pac_stats.retained;
...
#define READ_GLOBAL_MUTEX_PROF_DATA(i, mtx)                                    \
    ...
    malloc_mutex_unlock(tsdn, &mtx);
        ...
        if (config_prof && opt_prof) {
            ...
            READ_GLOBAL_MUTEX_PROF_DATA(
              global_prof_mutex_prof_recent_alloc, prof_recent_alloc_mtx);
            READ_GLOBAL_MUTEX_PROF_DATA(
              global_prof_mutex_prof_recent_dump, prof_recent_dump_mtx);
            READ_GLOBAL_MUTEX_PROF_DATA(
        ...
        if (have_background_thread) {
            ...
            memset(
              &ctl_stats->mutex_prof_data[global_prof_mutex_background_thread],
...
ctl_byname(tsd_t *tsd, const char *name, void *oldp, size_t *oldlenp,
  void *newp, size_t newlen) {
    ...
    depth = CTL_MAX_DEPTH;
    ret = ctl_lookup(tsd_tsdn(tsd), super_root_node, name, &node, mib, &depth);
    if (ret != 0) {
        goto label_return;
    }
...
ctl_nametomib(tsd_t *tsd, const char *name, size_t *mibp, size_t *miblenp) {
    ...
    ret = ctl_lookup(tsd_tsdn(tsd), super_root_node, name, NULL, mibp, miblenp);
label_return:
    return (ret);
}
...
ctl_mibnametomib(
  tsd_t *tsd, size_t *mib, size_t miblen, const char *name, size_t *miblenp) {
    ...
    *miblenp -= miblen;
    ret = ctl_lookup(tsd_tsdn(tsd), node, name, NULL, mib + miblen, miblenp);
    *miblenp += miblen;
label_return:
    return (ret);
}
...
ctl_bymibname(tsd_t *tsd, size_t *mib, size_t miblen, const char *name,
  size_t *miblenp, void *oldp, size_t *oldlenp, void *newp, size_t newlen) {
    ...
    *miblenp -= miblen;
     ...
     */
    ret = ctl_lookup(tsd_tsdn(tsd), node, name, &node, mib + miblen, miblenp);
    *miblenp += miblen;
    if (ret != 0) {
        goto label_return;
    }
    ...
    if (node != NULL && node->ctl) {
        ret = node->ctl(tsd, mib, *miblenp, oldp, oldlenp, newp, newlen);
    } else {
        /* The name refers to a partial path through the ctl tree. */
        ret = ENOENT;
    }
  ...
  size_t *oldlenp, void *newp, size_t newlen) {
    ...
    if (newind != oldind) {
        ...
        if (tcache_available(tsd)) {
            tcache_arena_reassociate(tsd_tsdn(tsd), tsd_tcache_slowp_get(tsd),
...
arena_i_decay_ms_ctl_impl(tsd_t *tsd, const size_t *mib, size_t miblen,
  void *oldp, size_t *oldlenp, void *newp, size_t newlen, bool dirty) {
    ...
    if (newp != NULL) {
        ...
        if (arena_decay_ms_set(tsd_tsdn(tsd), arena, state, *(ssize_t *)newp)) {
            ret = EFAULT;
            goto label_return;
        }
    }
  ...
  void *oldp, size_t *oldlenp, void *newp, size_t newlen) {
    ...
    if (arena_ind < narenas_total_get()) {
        ...
        if (arena == NULL) {
            ...
            old_extent_hooks = (extent_hooks_t *)&ehooks_default_extent_hooks;
            ...
            if (newp != NULL) {
                ...
                arena = arena_init(tsd_tsdn(tsd), arena_ind, &config);
                if (arena == NULL) {
                    ret = EFAULT;
                    goto label_return;
                }
            ...
            if (newp != NULL) {
                ...
                WRITE(new_extent_hooks, extent_hooks_t *);
                old_extent_hooks =
                  arena_set_extent_hooks(tsd, arena, new_extent_hooks);
                READ(old_extent_hooks, extent_hooks_t *);
            } else {
                old_extent_hooks =
                  ehooks_get_extent_hooks_ptr(arena_get_ehooks(arena));
                READ(old_extent_hooks, extent_hooks_t *);
            }
        }
    } else {
...
prof_thread_active_init_ctl(tsd_t *tsd, const size_t *mib, size_t miblen,
  void *oldp, size_t *oldlenp, void *newp, size_t newlen) {
    ...
    if (newp != NULL) {
        ...
        oldval = opt_prof ? prof_thread_active_init_get(tsd_tsdn(tsd)) : false;
    }
    READ(oldval, bool);
...
#define RO_MUTEX_CTL_GEN(n, l)                                                 \
    ...
    CTL_RO_CGEN(config_stats, stats_##n##_num_wait, l.n_wait_times, uint64_t)  \
    CTL_RO_CGEN(                                                               \
    ...
    CTL_RO_CGEN(config_stats, stats_##n##_max_num_thds, l.max_n_thds, uint32_t)
  ...
  size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen) {
    ...
    for (i = 0; i < len; ++i) {
        inspect_extent_util_stats_get(tsd_tsdn(tsd), ptrs[i],
          &util_stats[i].nfree, &util_stats[i].nregs, &util_stats[i].size);
    }
    ret = 0;
...
experimental_batch_alloc_ctl(tsd_t *tsd, const size_t *mib, size_t miblen,
  void *oldp, size_t *oldlenp, void *newp, size_t newlen) {
    ...
    size_t filled = batch_alloc(batch_alloc_packet.ptrs, batch_alloc_packet.num,
      batch_alloc_packet.size, batch_alloc_packet.flags);
    READ(filled, size_t);
