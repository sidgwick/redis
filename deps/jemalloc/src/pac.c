...
pac_alloc_real(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, size_t size,
  size_t alignment, bool zero, bool guarded) {
    ...
    edata_t *edata = ecache_alloc(tsdn, pac, ehooks, &pac->ecache_dirty, NULL,
      size, alignment, zero, guarded);

    if (edata == NULL && pac_may_have_muzzy(pac)) {
        edata = ecache_alloc(tsdn, pac, ehooks, &pac->ecache_muzzy, NULL, size,
          alignment, zero, guarded);
    }
    if (edata == NULL) {
        edata = ecache_alloc_grow(tsdn, pac, ehooks, &pac->ecache_retained,
...
pac_alloc_new_guarded(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, size_t size,
  size_t alignment, bool zero, bool frequent_reuse) {
    ...
    if (san_bump_enabled() && frequent_reuse) {
        edata = san_bump_alloc(tsdn, &pac->sba, pac, ehooks, size, zero);
    } else {
        ...
        if (edata != NULL) {
            ...
            san_guard_pages_two_sided(tsdn, ehooks, edata, pac->emap, true);
        }
    }
    assert(edata == NULL
...
pac_alloc_impl(tsdn_t *tsdn, pai_t *self, size_t size, size_t alignment,
  bool zero, bool guarded, bool frequent_reuse, bool *deferred_work_generated) {
    ...
    if (!guarded || frequent_reuse) {
        edata =
          pac_alloc_real(tsdn, pac, ehooks, size, alignment, zero, guarded);
    }
...
pac_expand_impl(tsdn_t *tsdn, pai_t *self, edata_t *edata, size_t old_size,
  size_t new_size, bool zero, bool *deferred_work_generated) {
    ...
    edata_t *trail = ecache_alloc(tsdn, pac, ehooks, &pac->ecache_dirty, edata,
      expand_amount, PAGE, zero, /* guarded*/ false);
    if (trail == NULL) {
        trail = ecache_alloc(tsdn, pac, ehooks, &pac->ecache_muzzy, edata,
          expand_amount, PAGE, zero, /* guarded*/ false);
    }
    if (trail == NULL) {
        trail = ecache_alloc_grow(tsdn, pac, ehooks, &pac->ecache_retained,
...
pac_shrink_impl(tsdn_t *tsdn, pai_t *self, edata_t *edata, size_t old_size,
  size_t new_size, bool *deferred_work_generated) {
    ...
    edata_t *trail = extent_split_wrapper(tsdn, pac, ehooks, edata, new_size,
      shrink_amount, /* holding_core_locks */ false);
    if (trail == NULL) {
        return true;
    }
  ...
  tsdn_t *tsdn, pai_t *self, edata_t *edata, bool *deferred_work_generated) {
    ...
    if (edata_guarded_get(edata)) {
        ...
        if (!edata_slab_get(edata) || !maps_coalesce) {
            assert(
              edata_size_get(edata) >= SC_LARGE_MINCLASS || !maps_coalesce);
...
pac_stash_decayed(tsdn_t *tsdn, pac_t *pac, ecache_t *ecache,
  size_t npages_limit, size_t npages_decay_max, edata_list_inactive_t *result) {
    ...
    while (nstashed < npages_decay_max) {
        edata_t *edata = ecache_evict(tsdn, pac, ehooks, ecache, npages_limit);
        if (edata == NULL) {
            break;
        }
...
pac_decay_stashed(tsdn_t *tsdn, pac_t *pac, decay_t *decay,
  ...
  edata_list_inactive_t *decay_extents) {
    ...
    bool try_muzzy =
      !fully_decay && pac_decay_ms_get(pac, extent_state_muzzy) != 0;
      ...
      edata != NULL; edata = edata_list_inactive_first(decay_extents)) {
        ...
        case extent_state_dirty:
            if (try_muzzy) {
                ...
                if (!err) {
                    ecache_dalloc(tsdn, pac, ehooks, &pac->ecache_muzzy, edata);
                    break;
                }
            }
    ...
    if (config_stats) {
        ...
        locked_inc_u64(
          tsdn, LOCKEDINT_MTX(*pac->stats_mtx), &decay_stats->npurge, 1);
        locked_inc_u64(tsdn, LOCKEDINT_MTX(*pac->stats_mtx),
          &decay_stats->nmadvise, nmadvise);
        locked_inc_u64(
          tsdn, LOCKEDINT_MTX(*pac->stats_mtx), &decay_stats->purged, npurged);
        LOCKEDINT_MTX_UNLOCK(tsdn, *pac->stats_mtx);
        atomic_fetch_sub_zu(
          &pac->stats->pac_mapped, nunmapped << LG_PAGE, ATOMIC_RELAXED);
    }
  ...
  size_t npages_limit) {
    if (current_npages > npages_limit) {
        pac_decay_to_limit(tsdn, pac, decay, decay_stats, ecache,
          /* fully_decay */ false, npages_limit, current_npages - npages_limit);
    }
}
...
pac_maybe_decay_purge(tsdn_t *tsdn, pac_t *pac, decay_t *decay,
  ...
  pac_purge_eagerness_t eagerness) {
    ...
    bool epoch_advanced =
      decay_maybe_advance_epoch(decay, &time, npages_current);
    if (eagerness == PAC_PURGE_ALWAYS
      || (epoch_advanced && eagerness == PAC_PURGE_ON_EPOCH_ADVANCE)) {
        size_t npages_limit = decay_npages_limit_get(decay);
        pac_decay_try_purge(
          tsdn, pac, decay, decay_stats, ecache, npages_current, npages_limit);
    }
...
pac_destroy(tsdn_t *tsdn, pac_t *pac) {
    ...
    ehooks_t *ehooks = pac_ehooks_get(pac);
    edata_t *edata;
    while ((edata = ecache_evict(tsdn, pac, ehooks, &pac->ecache_retained, 0))
      != NULL) {
