...
const char *thp_mode_names[] = {"default", "always", "never", "not supported"};
 ...
 *
 *     <jemalloc>: ../contrib/jemalloc/src/extent.c:1195: Failed assertion:
 * "p[i] == 0"
 *
...
os_pages_commit(void *addr, size_t size, bool commit) {
...
#else
    {
        int prot = commit ? PAGES_PROT_COMMIT : PAGES_PROT_DECOMMIT;
        void *result =
          mmap(addr, size, prot, mmap_flags | MAP_FIXED, PAGES_FD_TAG, 0);
...
pages_mark_guards(void *head, void *tail) {
    ...
    assert(head == NULL || tail == NULL || (uintptr_t)head < (uintptr_t)tail);
...
pages_unmark_guards(void *head, void *tail) {
    ...
    size_t range =
      head_and_tail ? (uintptr_t)tail - (uintptr_t)head + PAGE : SIZE_T_MAX;
    /*
...
pages_set_thp_state(void *ptr, size_t size) {
    ...
    if (opt_thp == thp_mode_always && init_system_thp_mode != thp_mode_never) {
