...
hpdata_reserve_alloc(hpdata_t *hpdata, size_t sz) {
     ...
     */
    size_t new_dirty =
      fb_ucount(hpdata->touched_pages, HUGEPAGE_PAGES, result, npages);
    ...
    hpdata_assert_consistent(hpdata);
    return (void *)((uintptr_t)hpdata_addr_get(hpdata) + (result << LG_PAGE));
}
...
hpdata_unreserve(hpdata_t *hpdata, void *addr, size_t sz) {
    ...
    assert((sz & PAGE_MASK) == 0);
    size_t begin =
      ((uintptr_t)addr - (uintptr_t)hpdata_addr_get(hpdata)) >> LG_PAGE;
    ...
    size_t new_begin =
      (fb_fls(hpdata->active_pages, HUGEPAGE_PAGES, begin) + 1);
    size_t new_end =
      fb_ffs(hpdata->active_pages, HUGEPAGE_PAGES, begin + npages - 1);
...
hpdata_purge_begin(hpdata_t *hpdata, hpdata_purge_state_t *purge_state) {
    ...
    fb_bit_and(dirty_pages, dirty_pages, hpdata->touched_pages, HUGEPAGE_PAGES);

    fb_init(purge_state->to_purge, HUGEPAGE_PAGES);
    size_t next_bit = 0;
    while (next_bit < HUGEPAGE_PAGES) {
        size_t next_dirty = fb_ffs(dirty_pages, HUGEPAGE_PAGES, next_bit);
        /* Recall that fb_ffs returns nbits if no set bit is found. */
        if (next_dirty == HUGEPAGE_PAGES) {
            break;
        }
        size_t next_active =
          fb_ffs(hpdata->active_pages, HUGEPAGE_PAGES, next_dirty);
        /*
         ...
         */
        ssize_t last_dirty =
          fb_fls(dirty_pages, HUGEPAGE_PAGES, next_active - 1);
        assert(last_dirty >= 0);
    ...
    assert(ndirty
      <= fb_scount(purge_state->to_purge, HUGEPAGE_PAGES, 0, HUGEPAGE_PAGES));
    assert(ndirty == fb_scount(dirty_pages, HUGEPAGE_PAGES, 0, HUGEPAGE_PAGES));
...
hpdata_purge_next(hpdata_t *hpdata, hpdata_purge_state_t *purge_state,
  void **r_purge_addr, size_t *r_purge_size) {
    ...
    *r_purge_addr =
      (void *)((uintptr_t)hpdata_addr_get(hpdata) + purge_begin * PAGE);
...
hpdata_purge_end(hpdata_t *hpdata, hpdata_purge_state_t *purge_state) {
    ...
    fb_bit_not(purge_state->to_purge, purge_state->to_purge, HUGEPAGE_PAGES);
