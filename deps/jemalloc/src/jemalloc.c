            ...
            if (arena_get(tsd_tsdn(tsd), i, false) != NULL) {
                ...
                for (j = 0; j < 2; j++) {
                    if (arena_nthreads_get(
                          arena_get(tsd_tsdn(tsd), i, false), !!j)
                      < arena_nthreads_get(
            ...
            if (arena_nthreads_get(
              ...
              || first_null == narenas_auto) {
                ...
                arena = arena_init_locked(
                  tsd_tsdn(tsd), choose[j], &arena_config_default);
                if (arena == NULL) {
    ...
    if (config_stats) {
        ...
        for (i = 0, narenas = narenas_total_get(); i < narenas; i++) {
            ...
            if (arena != NULL) {
                ...
                ql_foreach (tcache_slow, &arena->tcache_ql, link) {
...
#define CONF_HANDLE_T(t, max_t, o, n, min, max, check_min, check_max, clip)    \
    if (CONF_MATCH(n)) {                                                       \
        ...
        if (CONF_VALUE_READ_FAIL()) {                                          \
            ...
            if (check_min(mv, (t)(min)) || check_max(mv, (t)(max))) {          \
                CONF_ERROR(                                                    \
                  "Out-of-range "                                              \
                  "conf value",                                                \
                  k, klen, v, vlen);                                           \
...
#define CONF_HANDLE_INT64_T(o, n, min, max, check_min, check_max, clip)        \
    CONF_HANDLE_T_SIGNED(int64_t, o, n, min, max, check_min, check_max, clip)
        ...
        CONF_CONTINUE;                                                         \
            ...
            if (strncmp("metadata_thp", k, klen) == 0) {
                ...
                for (m = 0; m < metadata_thp_mode_limit; m++) {
                    if (strncmp(metadata_thp_mode_names[m], v, vlen) == 0) {
                        opt_metadata_thp = m;
                        match = true;
                        break;
                    }
            ...
            if (CONF_MATCH("narenas_ratio")) {
                char *end;
                bool err = fxp_parse(&opt_narenas_ratio, v, &end);
            ...
            if (CONF_MATCH("bin_shards")) {
                ...
                do {
                    ...
                    size_t nshards;
                    bool err =
                      malloc_conf_multi_sizes_next(&bin_shards_segment_cur,
            ...
            CONF_HANDLE_INT64_T(opt_mutex_max_spin, "mutex_max_spin", -1,
              INT64_MAX, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, false);
            ...
            if (CONF_MATCH("stats_print_opts")) {
                init_opt_stats_opts(v, vlen, opt_stats_print_opts);
                CONF_CONTINUE;
            }
            CONF_HANDLE_INT64_T(opt_stats_interval, "stats_interval", -1,
              INT64_MAX, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, false)
            if (CONF_MATCH("stats_interval_opts")) {
                init_opt_stats_opts(v, vlen, opt_stats_interval_opts);
                CONF_CONTINUE;
            }
            if (config_fill) {
                if (CONF_MATCH("junk")) {
            ...
            if (config_enable_cxx) {
                CONF_HANDLE_BOOL(opt_experimental_infallible_new,
                  "experimental_infallible_new")
            }

            CONF_HANDLE_BOOL(opt_tcache, "tcache")
            CONF_HANDLE_SIZE_T(opt_tcache_max, "tcache_max", 0,
              TCACHE_MAXCLASS_LIMIT, CONF_DONT_CHECK_MIN, CONF_CHECK_MAX,
              /* clip */ true)
             ...
             */
            CONF_HANDLE_SSIZE_T(
              opt_lg_tcache_nslots_mul, "lg_tcache_nslots_mul", -16, 16)
            ...
            CONF_HANDLE_UNSIGNED(opt_tcache_nslots_small_max,
              ...
              CONF_CHECK_MAX, /* clip */ true)
            CONF_HANDLE_UNSIGNED(opt_tcache_nslots_large, "tcache_nslots_large",
              1, 2048, CONF_CHECK_MIN, CONF_CHECK_MAX, /* clip */ true)
            CONF_HANDLE_SIZE_T(opt_tcache_gc_incr_bytes, "tcache_gc_incr_bytes",
              1024, SIZE_T_MAX, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX,
              /* clip */ true)
            CONF_HANDLE_SIZE_T(opt_tcache_gc_delay_bytes,
              "tcache_gc_delay_bytes", 0, SIZE_T_MAX, CONF_DONT_CHECK_MIN,
              CONF_DONT_CHECK_MAX,
              /* clip */ false)
            ...
            if (strncmp("percpu_arena", k, klen) == 0) {
                ...
                for (int m = percpu_arena_mode_names_base;
                  m < percpu_arena_mode_names_limit; m++) {
                    if (strncmp(percpu_arena_mode_names[m], v, vlen) == 0) {
            ...
            CONF_HANDLE_SIZE_T(opt_hpa_opts.slab_max_alloc,
              "hpa_slab_max_alloc", PAGE, HUGEPAGE, CONF_CHECK_MIN,
              CONF_CHECK_MAX, true);
            ...
            CONF_HANDLE_SIZE_T(opt_hpa_opts.hugification_threshold,
              "hpa_hugification_threshold", PAGE, HUGEPAGE, CONF_CHECK_MIN,
              CONF_CHECK_MAX, true);
            ...
            CONF_HANDLE_UINT64_T(opt_hpa_opts.min_purge_interval_ms,
              "hpa_min_purge_interval_ms", 0, 0, CONF_DONT_CHECK_MIN,
              CONF_DONT_CHECK_MAX, false);
            ...
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.nshards, "hpa_sec_nshards", 0,
              0, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.max_alloc, "hpa_sec_max_alloc",
              PAGE, 0, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.max_bytes, "hpa_sec_max_bytes",
              PAGE, 0, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.bytes_after_flush,
              "hpa_sec_bytes_after_flush", PAGE, 0, CONF_CHECK_MIN,
              CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.batch_fill_extra,
              "hpa_sec_batch_fill_extra", 0, HUGEPAGE_PAGES, CONF_CHECK_MIN,
              CONF_CHECK_MAX, true);
            ...
            if (config_prof) {
                ...
                CONF_HANDLE_SIZE_T(opt_lg_prof_sample, "lg_prof_sample", 0,
                  (sizeof(uint64_t) << 3) - 1, CONF_DONT_CHECK_MIN,
                  CONF_CHECK_MAX, true)
                CONF_HANDLE_BOOL(opt_prof_accum, "prof_accum")
                CONF_HANDLE_SSIZE_T(opt_lg_prof_interval, "lg_prof_interval",
                  -1, (sizeof(uint64_t) << 3) - 1)
                CONF_HANDLE_BOOL(opt_prof_gdump, "prof_gdump")
                CONF_HANDLE_BOOL(opt_prof_final, "prof_final")
                CONF_HANDLE_BOOL(opt_prof_leak, "prof_leak")
                CONF_HANDLE_BOOL(opt_prof_leak_error, "prof_leak_error")
                CONF_HANDLE_BOOL(opt_prof_log, "prof_log")
                CONF_HANDLE_SSIZE_T(opt_prof_recent_alloc_max,
                  "prof_recent_alloc_max", -1, SSIZE_MAX)
                CONF_HANDLE_BOOL(opt_prof_stats, "prof_stats")
                CONF_HANDLE_BOOL(
                  opt_prof_sys_thread_name, "prof_sys_thread_name")
                if (CONF_MATCH("prof_time_resolution")) {
                    if (CONF_MATCH_VALUE("default")) {
                        opt_prof_time_res = prof_time_res_default;
                        ...
                        if (!config_high_res_timer) {
                            ...
                            opt_prof_time_res = prof_time_res_high;
                        }
                    } else {
                        CONF_ERROR("Invalid conf value", k, klen, v, vlen);
                    }
            ...
            if (config_log) {
                if (CONF_MATCH("log")) {
                    size_t cpylen = (vlen <= sizeof(log_var_names)
                        ? vlen
            ...
            if (CONF_MATCH("thp")) {
                ...
                for (int m = 0; m < thp_mode_names_limit; m++) {
                    if (strncmp(thp_mode_names[m], v, vlen) == 0) {
            ...
            if (CONF_MATCH("zero_realloc")) {
                if (CONF_MATCH_VALUE("alloc")) {
                    opt_zero_realloc_action = zero_realloc_action_alloc;
                } else if (CONF_MATCH_VALUE("free")) {
                    opt_zero_realloc_action = zero_realloc_action_free;
                } else if (CONF_MATCH_VALUE("abort")) {
                    opt_zero_realloc_action = zero_realloc_action_abort;
                } else {
                    CONF_ERROR("Invalid conf value", k, klen, v, vlen);
                }
                CONF_CONTINUE;
            }
            if (config_uaf_detection && CONF_MATCH("lg_san_uaf_align")) {
                ssize_t a;
            ...
            CONF_HANDLE_SIZE_T(opt_san_guard_small, "san_guard_small", 0,
              SIZE_T_MAX, CONF_DONT_CHECK_MIN, CONF_DONT_CHECK_MAX, false)
            CONF_HANDLE_SIZE_T(opt_san_guard_large, "san_guard_large", 0,
              SIZE_T_MAX, CONF_DONT_CHECK_MIN, CONF_DONT_CHECK_MAX, false)
...
malloc_conf_init(sc_data_t *sc_data, unsigned bin_shard_sizes[SC_NBINS]) {
    ...
    malloc_conf_init_helper(sc_data, bin_shard_sizes, false, opts_cache, NULL);
...
malloc_init_hard_a0_locked() {
    ...
    if (opt_hpa && !hpa_supported()) {
        ...
        if (pa_shard_enable_hpa(
              TSDN_NULL, &a0->pa_shard, &hpa_shard_opts, &opt_hpa_sec_opts)) {
            return true;
        }
    }
...
malloc_init_hard_recursible(void) {
    ...
    if (opt_percpu_arena != percpu_arena_disabled) {
        bool cpu_count_is_deterministic = malloc_cpu_count_is_deterministic();
...
#define UNLOCK_RETURN(tsdn, ret, reentrancy)                                   \
    ...
    witness_assert_lockless(witness_tsd_tsdn(tsd_witness_tsdp_get_unsafe(tsd)));
    malloc_tsd_boot1();
...
tcache_get_from_ind(tsd_t *tsd, unsigned tcache_ind, bool slow, bool is_alloc) {
    ...
    if (tcache_ind == TCACHE_IND_AUTOMATIC) {
        ...
        } else if (is_alloc || likely(tsd_reentrancy_level_get(tsd) == 0)) {
...
imalloc_no_sample(static_opts_t *sopts, dynamic_opts_t *dopts, tsd_t *tsd,
  size_t size, size_t usize, szind_t ind) {
    ...
    return iallocztm(
      tsd_tsdn(tsd), size, ind, dopts->zero, tcache, false, arena, sopts->slow);
}
...
imalloc_body(static_opts_t *sopts, dynamic_opts_t *dopts, tsd_t *tsd) {
    ...
    if (config_prof && opt_prof) {
        ...
        if (likely((uintptr_t)tctx == (uintptr_t)1U)) {
            ...
            allocation =
              imalloc_no_sample(sopts, dopts, tsd, usize, usize, ind);
...
static inline void *
je_malloc_internal(size_t size, size_t *usize) {
...
JEMALLOC_ATTR(nonnull(1))
  je_posix_memalign(void **memptr, size_t alignment, size_t size) {
    ...
    LOG("core.posix_memalign.exit", "result: %d, alloc ptr: %p", ret, *memptr);
...
static void *
je_calloc_internal(size_t num, size_t size, size_t *usize) {
    void *ret;
    static_opts_t sopts;
    dynamic_opts_t dopts;
...
ifree(tsd_t *tsd, void *ptr, tcache_t *tcache, bool slow_path, size_t *usable) {
    ...
    emap_alloc_ctx_t alloc_ctx;
    emap_alloc_ctx_lookup(tsd_tsdn(tsd), &arena_emap_global, ptr, &alloc_ctx);
...
maybe_check_alloc_ctx(tsd_t *tsd, void *ptr, emap_alloc_ctx_t *alloc_ctx) {
    if (config_opt_size_checks) {
        ...
        emap_alloc_ctx_lookup(tsd_tsdn(tsd), &arena_emap_global, ptr, &dbg_ctx);
    ...
    if (!config_prof) {
        ...
        if (likely(!prof_sample_aligned(ptr))) {
            ...
            if (config_opt_safety_checks) {
                ...
                if (unlikely(alloc_ctx.szind != sz_size2index(usize))) {
                    safety_check_fail_sized_dealloc(
                      /* current_dealloc */ true, ptr,
                      /* true_size */ sz_index2size(alloc_ctx.szind),
                      /* input_size */ usize);
                }
            }
        } else {
...
free_default(void *ptr, size_t *usize) {
    ...
    if (likely(ptr != NULL)) {
        ...
        if (likely(tsd_fast(tsd))) {
            tcache_t *tcache =
              tcache_get_from_ind(tsd, TCACHE_IND_AUTOMATIC, /* slow */ false,
                /* is_alloc */ false);
            ifree(tsd, ptr, tcache, /* slow */ false, usize);
        } else {
            tcache_t *tcache =
              tcache_get_from_ind(tsd, TCACHE_IND_AUTOMATIC, /* slow */ true,
                /* is_alloc */ false);
...
bool
free_fastpath(void *ptr, size_t size, bool size_hint, size_t *usable_size) {
...
static inline void
je_free_internal(void *ptr, size_t *usize) {
...
JEMALLOC_EXPORT
JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN smallocx_return_t JEMALLOC_NOTHROW
/*
...
do_rallocx(void *ptr, size_t size, int flags, bool is_realloc,
  size_t *old_usable_size, size_t *new_usable_size) {
    void *p;
    tsd_t *tsd;
    size_t usize;
    size_t old_usize;
    ...
    if (config_fill && unlikely(opt_junk_alloc) && usize > old_usize && !zero) {
...
JEMALLOC_ALLOC_SIZE(2) je_rallocx(void *ptr, size_t size, int flags) {
    LOG(
      "core.rallocx.entry", "ptr: %p, size: %zu, flags: %d", ptr, size, flags);
...
do_realloc_nonnull_zero(void *ptr, size_t *old_usize, size_t *new_usize) {
    ...
    if (opt_zero_realloc_action == zero_realloc_action_alloc) {
         ...
         */
        return do_rallocx(
          ptr, 1, MALLOCX_TCACHE_NONE, true, old_usize, new_usize);
...
ixallocx_prof_sample(tsdn_t *tsdn, void *ptr, size_t old_usize, size_t size,
  size_t extra, size_t alignment, bool zero, prof_tctx_t *tctx) {
    ...
    return ixallocx_helper(tsdn, ptr, old_usize, size, extra, alignment, zero);
}
...
ixallocx_prof(tsd_t *tsd, void *ptr, size_t old_usize, size_t size,
  size_t extra, size_t alignment, bool zero, emap_alloc_ctx_t *alloc_ctx) {
     ...
     */
    size_t usize_max;
    if (aligned_usize_get(size + extra, alignment, &usize_max, NULL, false)) {
        /*
    ...
    if (usize == old_usize) {
        ...
        prof_realloc(tsd, ptr, size, usize, tctx, prof_active, ptr, old_usize,
          &prof_info, sample_event);
    }
...
je_xallocx(void *ptr, size_t size, size_t extra, int flags) {
     ...
     */
    edata_t *old_edata =
      emap_edata_lookup(tsd_tsdn(tsd), &arena_emap_global, ptr);
     ...
     */
    assert(
      emap_edata_lookup(tsd_tsdn(tsd), &arena_emap_global, ptr) == old_edata);
...
je_sdallocx(void *ptr, size_t size, int flags) {
    LOG(
      "core.sdallocx.entry", "ptr: %p, size: %zu, flags: %d", ptr, size, flags);
...
je_malloc_stats_print(
  void (*write_cb)(void *, const char *), void *cbopaque, const char *opts) {
    ...
    if (config_debug) {
        ...
        buf_writer_init(
          tsdn, &buf_writer, write_cb, cbopaque, NULL, STATS_PRINT_BUFSIZE);
...
batch_alloc_prof_sample_assert(tsd_t *tsd, size_t batch, size_t usize) {
    ...
    bool prof_sample_event = te_prof_sample_event_lookahead(tsd, batch * usize);
...
batch_alloc(void **ptrs, size_t num, size_t size, int flags) {
    LOG("core.batch_alloc.entry", "ptrs: %p, num: %zu, size: %zu, flags: %d",
      ptrs, num, size, flags);
    ...
    while (filled < num) {
        ...
        if (likely(ind < SC_NBINS) && batch >= nregs) {
            ...
            size_t n = arena_fill_small_fresh(
              tsd_tsdn(tsd), arena, ind, ptrs + filled, arena_batch, zero);
            progress += n;
            filled += n;
        }
        ...
        if (likely(ind < nhbins) && progress < batch) {
            if (bin == NULL) {
                unsigned tcache_ind = mallocx_tcache_get(flags);
                tcache_t *tcache =
                  tcache_get_from_ind(tsd, tcache_ind, /* slow */ true,
                    /* is_alloc */ true);
                if (tcache != NULL) {
                    bin = &tcache->bins[ind];
                }
            ...
            if (bin != NULL) {
                 ...
                 */
                size_t n = cache_bin_alloc_batch(bin, bin_batch, ptrs + filled);
                if (config_stats) {
                ...
                if (config_prof && opt_prof && unlikely(ind >= SC_NBINS)) {
                    for (size_t i = 0; i < n; ++i) {
                        prof_tctx_reset_sampled(tsd, ptrs[filled + i]);
                    }
                }
                progress += n;
                filled += n;
...
/* Helps the application decide if a pointer is worth re-allocating in order to
 * reduce fragmentation. returns 1 if the allocation should be moved, and 0 if
 * the allocation be kept. If the application decides to re-allocate it should
 * use MALLOCX_TCACHE_NONE when doing so. */
JEMALLOC_EXPORT int JEMALLOC_NOTHROW
...
JEMALLOC_EXPORT
JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *
JEMALLOC_ALLOC_SIZE(2) realloc_with_usize(
  void *ptr, size_t size, size_t *old_usize, size_t *new_usize) {
