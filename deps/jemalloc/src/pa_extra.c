...
pa_shard_stats_merge(tsdn_t *tsdn, pa_shard_t *shard,
  ...
  size_t *resident) {
    ...
    pa_shard_stats_out->edata_avail +=
      atomic_load_zu(&shard->edata_cache.count, ATOMIC_RELAXED);
    ...
    *resident += (resident_pgs << LG_PAGE);
    ...
    for (pszind_t i = 0; i < SC_NPSIZES; i++) {
        ...
        retained_bytes = ecache_nbytes_get(&shard->pac.ecache_retained, i);
...
pa_shard_mtx_stats_read(tsdn_t *tsdn, pa_shard_t *shard,
  mutex_prof_data_t mutex_prof_data[mutex_prof_num_arena_mutexes]) {
    ...
    if (shard->ever_used_hpa) {
        ...
        pa_shard_mtx_stats_read_single(tsdn, mutex_prof_data,
          &shard->hpa_shard.grow_mtx, arena_prof_mutex_hpa_shard_grow);
        sec_mutex_stats_read(
          tsdn, &shard->hpa_sec, &mutex_prof_data[arena_prof_mutex_hpa_sec]);
    }
}
