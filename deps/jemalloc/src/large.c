...
large_palloc(
  tsdn_t *tsdn, arena_t *arena, size_t usize, size_t alignment, bool zero) {
    ...
    if (unlikely(arena == NULL)
      || (edata = arena_extent_alloc_large(tsdn, arena, usize, alignment, zero))
        == NULL) {
        return NULL;
    }
...
large_ralloc_no_move_shrink(tsdn_t *tsdn, edata_t *edata, size_t usize) {
    ...
    bool err = pa_shrink(tsdn, &arena->pa_shard, edata, old_size,
      usize + sz_large_pad, sz_size2index(usize), &deferred_work_generated);
    if (err) {
        return true;
    }
  ...
  tsdn_t *tsdn, edata_t *edata, size_t usize, bool zero) {
    ...
    if (zero) {
        if (opt_cache_oblivious) {
             ...
             */
            void *zbase =
              (void *)((uintptr_t)edata_addr_get(edata) + old_usize);
...
large_ralloc_no_move(
  tsdn_t *tsdn, edata_t *edata, size_t usize_min, size_t usize_max, bool zero) {
