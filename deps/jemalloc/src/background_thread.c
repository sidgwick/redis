...
background_work_sleep_once(
  tsdn_t *tsdn, background_thread_info_t *info, unsigned ind) {
    ...
    for (unsigned i = ind; i < narenas; i += max_background_threads) {
        ...
        uint64_t ns_arena_deferred =
          pa_shard_time_until_deferred_work(tsdn, &arena->pa_shard);
    ...
    if (ns_until_deferred == BACKGROUND_THREAD_DEFERRED_MAX) {
        ...
        sleep_ns = (ns_until_deferred < BACKGROUND_THREAD_MIN_INTERVAL_NS)
...
background_work(tsd_t *tsd, unsigned ind) {
    ...
    if (ind == 0) {
        ...
        while (info->state != background_thread_stopped) {
            if (background_thread_pause_check(tsd_tsdn(tsd), info)) {
                continue;
            }
...
#    endif
    ...
    assert(
      pthread_equal(pthread_self(), background_thread_info[thread_ind].thread));
...
background_thread_create_locked(tsd_t *tsd, unsigned arena_ind) {
    ...
    malloc_mutex_lock(tsd_tsdn(tsd), &info->mtx);
    need_new_thread =
      background_thread_enabled() && (info->state == background_thread_stopped);
    if (need_new_thread) {
...
background_thread_boot1(tsdn_t *tsdn, base_t *base) {
    ...
    background_thread_info = (background_thread_info_t *)base_alloc(tsdn, base,
      opt_max_background_threads * sizeof(background_thread_info_t), CACHELINE);
