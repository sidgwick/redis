...
prof_tdata_count(void) {
    ...
    tdata_tree_iter(&tdatas, NULL, prof_tdata_count_iter, (void *)&tdata_count);
...
prof_thread_name_alloc(tsd_t *tsd, const char *thread_name) {
    ...
    ret = iallocztm(tsd_tsdn(tsd), size, sz_size2index(size), false, NULL, true,
...
prof_unbias_map_init() {
    ...
    for (szind_t i = 0; i < SC_NSIZES; i++) {
        ...
        prof_shifted_unbiased_cnt[i] = (size_t)round(shifted_unbiased_cnt);
    }
#else
    unreachable();
#endif
...
prof_do_unbias(uint64_t c_out_shifted_i, uint64_t s_out_i, uint64_t *r_c_in,
  uint64_t *r_s_in) {
     ...
     * shifted form.
     */
    double c_out = (double)c_out_shifted_i / (double)(ZU(1) << SC_LG_TINY_MIN);
...
prof_dump_print_cnts(
  write_cb_t *prof_dump_write, void *cbopaque, const prof_cnt_t *cnts) {
    ...
    uint64_t accumbytes;
    if (opt_prof_unbias) {
        prof_do_unbias(cnts->curobjs_shifted_unbiased, cnts->curbytes_unbiased,
          &curobjs, &curbytes);
    ...
    prof_dump_printf(prof_dump_write, cbopaque,
      "%" FMTu64 ": %" FMTu64 " [%" FMTu64 ": %" FMTu64 "]", curobjs, curbytes,
      accumobjs, accumbytes);
}
...
prof_gctx_merge_iter(prof_gctx_tree_t *gctxs, prof_gctx_t *gctx, void *opaque) {
    ...
    tctx_tree_iter(&gctx->tctxs, NULL, prof_tctx_merge_iter, (void *)arg->tsdn);
...
prof_gctx_finish(tsd_t *tsd, prof_gctx_tree_t *gctxs) {
    ...
    while ((gctx = gctx_tree_first(gctxs)) != NULL) {
        ...
        malloc_mutex_lock(tsd_tsdn(tsd), gctx->lock);
        {
            ...
            next = NULL;
            do {
                prof_tctx_t *to_destroy = tctx_tree_iter(&gctx->tctxs, next,
                  prof_tctx_finish_iter, (void *)tsd_tsdn(tsd));
                if (to_destroy != NULL) {
                    next = tctx_tree_next(&gctx->tctxs, to_destroy);
                    tctx_tree_remove(&gctx->tctxs, to_destroy);
                    idalloctm(
                      tsd_tsdn(tsd), to_destroy, NULL, NULL, true, true);
                } else {
                    next = NULL;
                }
            } while (next != NULL);
...
prof_tdata_merge_iter(
  prof_tdata_tree_t *tdatas_ptr, prof_tdata_t *tdata, void *opaque) {
    prof_tdata_merge_iter_arg_t *arg = (prof_tdata_merge_iter_arg_t *)opaque;
    ...
    if (!tdata->expired) {
        ...
        arg->cnt_all->curbytes_unbiased += tdata->cnt_summed.curbytes_unbiased;
        if (opt_prof_accum) {
...
prof_tdata_dump_iter(
  prof_tdata_tree_t *tdatas_ptr, prof_tdata_t *tdata, void *opaque) {
    ...
    prof_dump_iter_arg_t *arg = (prof_dump_iter_arg_t *)opaque;
    prof_dump_printf(
      arg->prof_dump_write, arg->cbopaque, "  t%" FMTu64 ": ", tdata->thr_uid);
    prof_dump_print_cnts(
...
prof_dump_gctx(prof_dump_iter_arg_t *arg, prof_gctx_t *gctx,
  const prof_bt_t *bt, prof_gctx_tree_t *gctxs) {
    ...
    for (unsigned i = 0; i < bt->len; i++) {
        prof_dump_printf(arg->prof_dump_write, arg->cbopaque, " %#" FMTxPTR,
          (uintptr_t)bt->vec[i]);
    }
...
prof_tctx_destroy(tsd_t *tsd, prof_tctx_t *tctx) {
...

    {
        ...
        bool destroy_tdata =
          prof_tdata_should_destroy(tsd_tsdn(tsd), tdata, false);
        malloc_mutex_unlock(tsd_tsdn(tsd), tdata->lock);
        if (destroy_tdata) {
