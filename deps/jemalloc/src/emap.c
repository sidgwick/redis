...
emap_try_acquire_edata_neighbor_impl(tsdn_t *tsdn, emap_t *emap, edata_t *edata,
  ...
  bool expanding) {
    ...
    void *neighbor_addr =
      forward ? edata_past_get(edata) : edata_before_get(edata);
    /*
    ...
    EMAP_DECLARE_RTREE_CTX;
    rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, &emap->rtree, rtree_ctx,
      (uintptr_t)neighbor_addr, /* dependent*/ false,
      /* init_missing */ false);
    if (elm == NULL) {
        return NULL;
    }

    rtree_contents_t neighbor_contents =
      rtree_leaf_elm_read(tsdn, &emap->rtree, elm, /* dependent */ true);
    if (!extent_can_acquire_neighbor(
          edata, neighbor_contents, pai, expected_state, forward, expanding)) {
        return NULL;
    }
...
emap_try_acquire_edata_neighbor(tsdn_t *tsdn, emap_t *emap, edata_t *edata,
  extent_pai_t pai, extent_state_t expected_state, bool forward) {
    return emap_try_acquire_edata_neighbor_impl(
      tsdn, emap, edata, pai, expected_state, forward, /* expand */ false);
}
...
emap_rtree_write_acquired(tsdn_t *tsdn, emap_t *emap, rtree_leaf_elm_t *elm_a,
  rtree_leaf_elm_t *elm_b, edata_t *edata, szind_t szind, bool slab) {
    ...
    contents.metadata.is_head =
      (edata == NULL) ? false : edata_is_head_get(edata);
  ...
  tsdn_t *tsdn, emap_t *emap, edata_t *edata, szind_t szind) {
    ...
    if (config_debug) {
        ...
        bool err = emap_rtree_leaf_elms_lookup(tsdn, emap, rtree_ctx, edata,
          /* dependent */ true, /* init_missing */ false, &elm_a, &elm_b);
        assert(!err);
...
emap_deregister_boundary(tsdn_t *tsdn, emap_t *emap, edata_t *edata) {
    ...
    emap_rtree_write_acquired(tsdn, emap, elm_a, elm_b, NULL, SC_NSIZES, false);
}
  ...
  tsdn_t *tsdn, emap_t *emap, edata_t *edata, szind_t szind, bool slab) {
    ...
    if (szind != SC_NSIZES) {
        ...
        if (slab && edata_size_get(edata) > PAGE) {
            uintptr_t key = (uintptr_t)edata_past_get(edata) - (uintptr_t)PAGE;
            rtree_write(tsdn, &emap->rtree, rtree_ctx, key, contents);
        }
    }
}
