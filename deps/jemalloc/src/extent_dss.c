...
const char *dss_prec_names[] = {"disabled", "primary", "secondary", "N/A"};
  ...
  size_t alignment, bool *zero, bool *commit) {
    ...
    if (!atomic_load_b(&dss_exhausted, ATOMIC_ACQUIRE)) {
        ...
        while (true) {
            ...
            bool head_state = opt_retain ? EXTENT_IS_HEAD : EXTENT_NOT_HEAD;
            /*
            ...
            if (gap_size_page != 0) {
                edata_init(gap, arena_ind_get(arena), gap_addr_page,
                  gap_size_page, false, SC_NSIZES,
                  extent_sn_next(&arena->pa_shard.pac), extent_state_active,
                  false, true, EXTENT_PAI_PAC, head_state);
            }
            /*
            ...
            if (dss_prev == max_cur) {
                ...
                if (gap_size_page != 0) {
                    ...
                    extent_dalloc_gap(tsdn, &arena->pa_shard.pac, ehooks, gap);
                } else {
                ...
                if (*zero && *commit) {
                    ...
                    edata_init(&edata, arena_ind_get(arena), ret, size, size,
                      false, SC_NSIZES, extent_state_active, false, true,
                      EXTENT_PAI_PAC, head_state);
                    if (extent_purge_forced_wrapper(
                          tsdn, ehooks, &edata, 0, size)) {
                        memset(ret, 0, size);
                    }
                }
                return ret;
...
extent_in_dss(void *addr) {
    ...
    return extent_in_dss_helper(addr, atomic_load_p(&dss_max, ATOMIC_ACQUIRE));
}
