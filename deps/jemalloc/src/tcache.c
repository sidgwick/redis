...
tcache_gc_small(
  tsd_t *tsd, tcache_slow_t *tcache_slow, tcache_t *tcache, szind_t szind) {
    ...
    cache_bin_sz_t ncached =
      cache_bin_ncached_get_local(cache_bin, &tcache_bin_info[szind]);
    cache_bin_sz_t low_water =
      cache_bin_low_water_get(cache_bin, &tcache_bin_info[szind]);
...
tcache_alloc_small_hard(tsdn_t *tsdn, arena_t *arena, tcache_t *tcache,
  cache_bin_t *cache_bin, szind_t binind, bool *tcache_success) {
    ...
    arena_cache_bin_fill_small(
      tsdn, arena, cache_bin, &tcache_bin_info[binind], binind, nfill);
...
tcache_bin_flush_match(
  edata_t *edata, unsigned cur_arena_ind, unsigned cur_binshard, bool small) {
    if (small) {
  ...
  szind_t binind, cache_bin_ptr_array_t *ptrs, unsigned nflush, bool small) {
    ...
    while (nflush > 0) {
        ...
        if (small) {
            ...
            cur_bin = arena_get_bin(cur_arena, binind, cur_binshard);
         ...
         * flush them.
         */
        if (config_stats && tcache_arena == cur_arena && !merged_stats) {
            merged_stats = true;
            if (small) {
        ...
        if (!small) {
            for (unsigned i = 0; i < nflush; i++) {
                ...
                if (tcache_bin_flush_match(
                      edata, cur_arena_ind, cur_binshard, small)) {
        ...
        for (unsigned i = 0; i < nflush; i++) {
            ...
            if (small) {
                if (arena_dalloc_bin_locked_step(tsdn, cur_arena, cur_bin,
                      &dalloc_bin_info, binind, edata, ptr)) {
    ...
    if (config_stats && !merged_stats) {
        if (small) {
            ...
            arena_stats_large_flush_nrequests_add(
              tsdn, &tcache_arena->stats, binind, cache_bin->tstats.nrequests);
            cache_bin->tstats.nrequests = 0;
        }
    }
}
...
tcache_bin_flush_bottom(tsd_t *tsd, tcache_t *tcache, cache_bin_t *cache_bin,
  szind_t binind, unsigned rem, bool small) {
    ...
    cache_bin_sz_t ncached =
      cache_bin_ncached_get_local(cache_bin, &tcache_bin_info[binind]);
    ...
    cache_bin_init_ptr_array_for_flush(
      cache_bin, &tcache_bin_info[binind], &ptrs, nflush);

    tcache_bin_flush_impl(tsd, tcache, cache_bin, binind, &ptrs, nflush, small);

    cache_bin_finish_flush(
      cache_bin, &tcache_bin_info[binind], &ptrs, ncached - rem);
}
...
tcache_bin_flush_stashed(tsd_t *tsd, tcache_t *tcache, cache_bin_t *cache_bin,
  szind_t binind, bool is_small) {
    ...
    cache_bin_sz_t orig_cached = cache_bin_ncached_get_local(cache_bin, info);
    ...
    cache_bin_init_ptr_array_for_stashed(
      cache_bin, binind, info, &ptrs, nstashed);
...
static void
tcache_init(
  tsd_t *tsd, tcache_slow_t *tcache_slow, tcache_t *tcache, void *mem) {
    ...
    size_t cur_offset = 0;
    cache_bin_preincrement(tcache_bin_info, nhbins, mem, &cur_offset);
    for (unsigned i = 0; i < nhbins; i++) {
        ...
        cache_bin_init(cache_bin, &tcache_bin_info[i], mem, &cur_offset);
    }
    /*
    ...
    for (unsigned i = nhbins; i < SC_NBINS; i++) {
        ...
        cache_bin_init(
          cache_bin, &tcache_bin_info[i], fake_stack, &fake_offset);
        assert(tcache_small_bin_disabled(i, cache_bin));
    }

    cache_bin_postincrement(tcache_bin_info, nhbins, mem, &cur_offset);
...
tsd_tcache_data_init(tsd_t *tsd) {
    ...
    if (!malloc_initialized()) {
        ...
        tcache_arena_associate(tsd_tsdn(tsd), tcache_slow, tcache, arena);
    } else {
...
tcache_create_explicit(tsd_t *tsd) {
     ...
     */
    size_t size =
      tcache_bin_alloc_size + sizeof(tcache_t) + sizeof(tcache_slow_t);
...
tcache_destroy(tsd_t *tsd, tcache_t *tcache, bool tsd_tcache) {
    ...
    idalloctm(tsd_tsdn(tsd), tcache_slow->dyn_alloc, NULL, NULL, true, true);
    ...
    if (arena_nthreads_get(arena, false) == 0 && !background_thread_enabled()) {
...
tcache_stats_merge(tsdn_t *tsdn, tcache_t *tcache, arena_t *arena) {
    ...
    for (unsigned i = 0; i < nhbins; i++) {
        ...
        if (i < SC_NBINS) {
            ...
            arena_stats_large_flush_nrequests_add(
              tsdn, &arena->stats, i, cache_bin->tstats.nrequests);
        }
        cache_bin->tstats.nrequests = 0;
    }
}
...
tcache_boot(tsdn_t *tsdn, base_t *base) {
    ...
    tcache_bin_info =
      (cache_bin_info_t *)base_alloc(tsdn, base, size, CACHELINE);
