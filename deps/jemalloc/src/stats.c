...
#define OP(counter, counter_type, human, derived, base_counter)                \
    ...
    if (!derived) {                                                            \
        CTL_LEAF(mib, miblen_name, #counter, (counter_type *)&dst->bool_val,   \
          counter_type);                                                       \
    } else {                                                                   \
        emitter_col_t *base =                                                  \
          &col_##counter_type[mutex_counter_##base_counter];                   \
        dst->counter_type##_val =                                              \
          (counter_type)rate_per_second(base->counter_type##_val, uptime);     \
    }
    MUTEX_PROF_COUNTERS
#undef OP
...
#define OP(counter, counter_type, human, derived, base_counter)                \
    ...
    dst->type = EMITTER_TYPE_##counter_type;                                   \
    if (!derived) {                                                            \
        CTL_LEAF(mib, miblen_mutex, #counter, (counter_type *)&dst->bool_val,  \
          counter_type);                                                       \
    } else {                                                                   \
        emitter_col_t *base =                                                  \
          &col_##counter_type[mutex_counter_##base_counter];                   \
...
#define COL_HDR(row_name, column_name, human, left_or_right, col_width, etype) \
    ...
    COL_HDR_INIT(row_name, column_name, human, left_or_right, col_width, etype)
...
stats_arena_bins_print(
  emitter_t *emitter, bool mutex, unsigned i, uint64_t uptime) {
    ...
    bool prof_stats_on =
      config_prof && opt_prof && opt_prof_stats && i == MALLCTL_ARENAS_ALL;
    ...
    for (j = 0, in_gap = false; j < nbins; j++) {
        ...
        if (prof_stats_on) {
            prof_stats_mib[3] = j;
            CTL_LEAF(prof_stats_mib, 4, "live", &prof_live, prof_stats_t);
            CTL_LEAF(prof_stats_mib, 4, "accum", &prof_accum, prof_stats_t);
        }
        ...
        CTL_LEAF(stats_arenas_mib, 5, "nrequests", &nrequests, uint64_t);
        ...
        CTL_LEAF(stats_arenas_mib, 5, "nonfull_slabs", &nonfull_slabs, size_t);

        if (mutex) {
            mutex_stats_read_arena_bin(
              stats_arenas_mib, 5, col_mutex64, col_mutex32, uptime);
        }
        ...
        if (prof_stats_on) {
            emitter_json_kv(emitter, "prof_live_requested", emitter_type_uint64,
              &prof_live.req_sum);
            emitter_json_kv(emitter, "prof_live_count", emitter_type_uint64,
              &prof_live.count);
            emitter_json_kv(emitter, "prof_accum_requested",
              emitter_type_uint64, &prof_accum.req_sum);
            emitter_json_kv(emitter, "prof_accum_count", emitter_type_uint64,
              &prof_accum.count);
        }
        ...
        if (prof_stats_on) {
            ...
            col_prof_accum_requested.uint64_val = prof_accum.req_sum;
...
stats_arena_lextents_print(emitter_t *emitter, unsigned i, uint64_t uptime) {
    ...
    for (j = 0, in_gap = false; j < nlextents; j++) {
        ...
        CTL_LEAF(stats_arenas_mib, 5, "curlextents", &curlextents, size_t);
...
stats_arena_extents_print(emitter_t *emitter, unsigned i) {
    ...
    for (j = 0; j < SC_NPSIZES; j++) {
        ...
        CTL_LEAF(stats_arenas_mib, 5, "dirty_bytes", &dirty_bytes, size_t);
        CTL_LEAF(stats_arenas_mib, 5, "muzzy_bytes", &muzzy_bytes, size_t);
        CTL_LEAF(
          stats_arenas_mib, 5, "retained_bytes", &retained_bytes, size_t);
...
stats_arena_hpa_shard_print(emitter_t *emitter, unsigned i, uint64_t uptime) {
    ...
    CTL_M2_GET(
      "stats.arenas.0.hpa_shard.npurge_passes", i, &npurge_passes, uint64_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.npurges", i, &npurges, uint64_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.nhugifies", i, &nhugifies, uint64_t);
    CTL_M2_GET(
    ...
    emitter_table_printf(emitter,
      ...
      " / sec)\n"
      "\n",
      npurge_passes, rate_per_second(npurge_passes, uptime), npurges,
      rate_per_second(npurges, uptime), nhugifies,
      rate_per_second(nhugifies, uptime), ndehugifies,
    ...
    emitter_json_kv(emitter, "ndehugifies", emitter_type_uint64, &ndehugifies);
    ...
    nretained_nonhuge =
      npageslabs_nonhuge * HUGEPAGE_PAGES - nactive_nonhuge - ndirty_nonhuge;
    ...
    emitter_json_kv(emitter, "nactive_huge", emitter_type_size, &nactive_huge);
    emitter_json_kv(emitter, "nactive_huge", emitter_type_size, &nactive_huge);
    emitter_json_kv(
      emitter, "npageslabs_nonhuge", emitter_type_size, &npageslabs_nonhuge);
    emitter_json_kv(
    ...
    for (pszind_t j = 0; j < PSSET_NPSIZES && j < SC_NPSIZES; j++) {
        ...
        CTL_LEAF(
          stats_arenas_mib, 6, "npageslabs_huge", &npageslabs_huge, size_t);
        CTL_LEAF(stats_arenas_mib, 6, "nactive_huge", &nactive_huge, size_t);
        CTL_LEAF(stats_arenas_mib, 6, "ndirty_huge", &ndirty_huge, size_t);

        CTL_LEAF(stats_arenas_mib, 6, "npageslabs_nonhuge", &npageslabs_nonhuge,
          size_t);
        CTL_LEAF(
          stats_arenas_mib, 6, "nactive_nonhuge", &nactive_nonhuge, size_t);
        CTL_LEAF(
          stats_arenas_mib, 6, "ndirty_nonhuge", &ndirty_nonhuge, size_t);
...
stats_arena_mutexes_print(
  emitter_t *emitter, unsigned arena_ind, uint64_t uptime) {
    ...
    for (mutex_prof_arena_ind_t i = 0; i < mutex_prof_num_arena_mutexes; i++) {
...
#define OPT_WRITE_CHAR_P(name) OPT_WRITE(name, cpv, cpsz, emitter_type_string)
    ...
    if (je_mallctl("opt.hpa_dirty_mult", (void *)&u32v, &u32sz, NULL, 0) == 0) {
        /*
        ...
        if (u32v == (uint32_t)-1) {
            const char *neg1 = "-1";
            emitter_kv(emitter, "hpa_dirty_mult", "opt.hpa_dirty_mult",
...
stats_print_helper(emitter_t *emitter, bool merged, bool destroyed,
  bool unmerged, bool bins, bool large, bool mutex, bool extents, bool hpa) {
    ...
    emitter_json_kv(emitter, "metadata_thp", emitter_type_size, &metadata_thp);
    ...
    if (mutex) {
        ...
        for (int i = 0; i < mutex_prof_num_global_mutexes; i++) {
            mutex_stats_read_global(stats_mutexes_mib, 2, global_mutex_names[i],
    ...
    if (merged || destroyed || unmerged) {
        ...
        if (merged && (ninitialized > 1 || !unmerged)) {
            ...
            stats_arena_print(
              emitter, MALLCTL_ARENAS_ALL, bins, large, mutex, extents, hpa);
        ...
        if (destroyed_initialized && destroyed) {
            ...
            stats_arena_print(emitter, MALLCTL_ARENAS_DESTROYED, bins, large,
              mutex, extents, hpa);
        ...
        if (unmerged) {
            for (i = j = 0; i < narenas; i++) {
                if (initialized[i]) {
                    ...
                    malloc_snprintf(
                      arena_ind_str, sizeof(arena_ind_str), "%u", i);
                    emitter_json_object_kv_begin(emitter, arena_ind_str);
                    emitter_table_printf(
                      emitter, "arenas[%s]:\n", arena_ind_str);
                    stats_arena_print(
                      emitter, i, bins, large, mutex, extents, hpa);
                    /* Close "<arena-ind>". */
                    emitter_json_object_end(emitter);
                }
            }
...
#undef OPTION
    ...
    if (config_stats) {
        stats_print_helper(&emitter, merged, destroyed, unmerged, bins, large,
          mutex, extents, hpa);
    }
...
stats_interval_event_handler(tsd_t *tsd, uint64_t elapsed) {
    ...
    if (counter_accum(tsd_tsdn(tsd), &stats_interval_accumulated, elapsed)) {
...
stats_boot(void) {
    ...
    if (opt_stats_interval < 0) {
        ...
        stats_interval = (opt_stats_interval > 0) ? opt_stats_interval : 1;
