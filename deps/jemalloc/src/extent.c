...
extent_try_delayed_coalesce(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks,
  ecache_t *ecache, edata_t *edata) {
    ...
    bool coalesced;
    edata = extent_try_coalesce(tsdn, pac, ehooks, ecache, edata, &coalesced);
...
ecache_alloc_grow(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, ecache_t *ecache,
  ...
  bool guarded) {
    ...
    bool commit = true;
    edata_t *edata = extent_alloc_retained(
      tsdn, pac, ehooks, expand_edata, size, alignment, zero, &commit, guarded);
    if (edata == NULL) {
        ...
        void *new_addr =
          (expand_edata == NULL) ? NULL : edata_past_get(expand_edata);
...
extents_abandon_vm(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, ecache_t *ecache,
  edata_t *edata, bool growing_retained) {
    ...
    if (config_stats) {
        atomic_fetch_add_zu(&pac->stats->abandoned_vm, sz, ATOMIC_RELAXED);
    }
    /*
    ...
    if (ecache->state == extent_state_dirty) {
        if (extent_purge_lazy_impl(
            ...
            extent_purge_forced_impl(
              tsdn, ehooks, edata, 0, edata_size_get(edata), growing_retained);
        }
    }
...
extent_deactivate_locked_impl(
  tsdn_t *tsdn, pac_t *pac, ecache_t *ecache, edata_t *edata) {
    ...
    emap_update_edata_state(tsdn, pac->emap, edata, ecache->state);
    eset_t *eset =
      edata_guarded_get(edata) ? &ecache->guarded_eset : &ecache->eset;
...
extent_recycle_extract(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks,
  ...
  bool guarded) {
    ...
    if (expand_edata != NULL) {
        edata = emap_try_acquire_edata_neighbor_expand(
        ...
        if (edata != NULL) {
            ...
            if (edata_size_get(edata) < size) {
                emap_release_edata(tsdn, pac->emap, edata, ecache->state);
                edata = NULL;
            }
        }
    } else {
...
extent_split_interior(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks,
  ...
  size_t alignment) {
    ...
    *to_salvage = NULL;
    ...
    if (trailsize != 0) {
        assert(!edata_guarded_get(*edata));
        *trail = extent_split_impl(tsdn, pac, ehooks, *edata, size, trailsize,
...
extent_recycle_split(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks,
  ...
  edata_t *edata, bool growing_retained) {
    ...
    extent_split_interior_result_t result =
      extent_split_interior(tsdn, pac, ehooks, &edata, &lead, &trail, &to_leak,
        &to_salvage, expand_edata, size, alignment);
    ...
    if (result == extent_split_interior_ok) {
        ...
        if (to_leak != NULL) {
            ...
            malloc_mutex_unlock(tsdn, &ecache->mtx);
            extents_abandon_vm(
              tsdn, pac, ehooks, ecache, to_leak, growing_retained);
            malloc_mutex_lock(tsdn, &ecache->mtx);
        }
        return NULL;
    }
...
extent_recycle(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, ecache_t *ecache,
  edata_t *expand_edata, size_t size, size_t alignment, bool zero, bool *commit,
  bool growing_retained, bool guarded) {
    witness_assert_depth_to_rank(
      tsdn_witness_tsdp_get(tsdn), WITNESS_RANK_CORE, growing_retained ? 1 : 0);
    ...
    edata = extent_recycle_split(tsdn, pac, ehooks, ecache, expand_edata, size,
      alignment, edata, growing_retained);
    malloc_mutex_unlock(tsdn, &ecache->mtx);
    if (edata == NULL) {
        return NULL;
    }
...
extent_grow_retained(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, size_t size,
  size_t alignment, bool zero, bool *commit) {
    ...
    void *ptr =
      ehooks_alloc(tsdn, ehooks, NULL, alloc_size, PAGE, &zeroed, &committed);
    ...
    edata_init(edata, ecache_ind_get(&pac->ecache_retained), ptr, alloc_size,
      false, SC_NSIZES, extent_sn_next(pac), extent_state_active, zeroed,
      committed, EXTENT_PAI_PAC, EXTENT_IS_HEAD);
    ...
    extent_split_interior_result_t result =
      extent_split_interior(tsdn, pac, ehooks, &edata, &lead, &trail, &to_leak,
        &to_salvage, NULL, size, alignment);

    if (result == extent_split_interior_ok) {
        if (lead != NULL) {
            extent_record(tsdn, pac, ehooks, &pac->ecache_retained, lead);
        }
        if (trail != NULL) {
            extent_record(tsdn, pac, ehooks, &pac->ecache_retained, trail);
        }
    } else {
        /*
        ...
        if (to_salvage != NULL) {
            ...
            extent_record(tsdn, pac, ehooks, &pac->ecache_retained, to_salvage);
        }
        if (to_leak != NULL) {
            extent_deregister_no_gdump_sub(tsdn, pac, to_leak);
            extents_abandon_vm(
              tsdn, pac, ehooks, &pac->ecache_retained, to_leak, true);
        }
        goto label_err;
    }
    ...
    if (zero && !edata_zeroed_get(edata)) {
        ehooks_zero(tsdn, ehooks, edata_base_get(edata), edata_size_get(edata));
    }
    return edata;
label_err:
    malloc_mutex_unlock(tsdn, &pac->grow_mtx);
...
extent_alloc_retained(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks,
  ...
  bool guarded) {
    ...
    edata_t *edata = extent_recycle(tsdn, pac, ehooks, &pac->ecache_retained,
      expand_edata, size, alignment, zero, commit,
...
extent_try_coalesce_impl(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks,
  ecache_t *ecache, edata_t *edata, bool *coalesced) {
    ...
    do {
        ...
        edata_t *next = emap_try_acquire_edata_neighbor(tsdn, pac->emap, edata,
          EXTENT_PAI_PAC, ecache->state, /* forward */ true);
        if (next != NULL) {
            if (!extent_coalesce(
        ...
        edata_t *prev = emap_try_acquire_edata_neighbor(tsdn, pac->emap, edata,
          EXTENT_PAI_PAC, ecache->state, /* forward */ false);
        if (prev != NULL) {
            if (!extent_coalesce(
                  tsdn, pac, ehooks, ecache, edata, prev, false)) {
                edata = prev;
                if (ecache->delay_coalesce) {
                    /* Do minimal coalescing. */
                    *coalesced = true;
...
extent_record(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, ecache_t *ecache,
  edata_t *edata) {
    ...
    if (!ecache->delay_coalesce) {
        edata = extent_try_coalesce(tsdn, pac, ehooks, ecache, edata, NULL);
        ...
        do {
            assert(edata_state_get(edata) == extent_state_active);
            edata = extent_try_coalesce_large(
              tsdn, pac, ehooks, ecache, edata, &coalesced);
        } while (coalesced);
...
extent_alloc_wrapper(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, void *new_addr,
  ...
  bool growing_retained) {
    ...
    void *addr =
      ehooks_alloc(tsdn, ehooks, new_addr, size, palignment, &zero, commit);
    if (addr == NULL) {
        edata_cache_put(tsdn, pac->edata_cache, edata);
        return NULL;
    }
    edata_init(edata, ecache_ind_get(&pac->ecache_dirty), addr, size,
      /* slab */ false, SC_NSIZES, extent_sn_next(pac), extent_state_active,
      zero, *commit, EXTENT_PAI_PAC,
  ...
  tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, edata_t *edata) {
    ...
    if (!ehooks_dalloc_will_fail(ehooks)) {
        ...
        if (edata_guarded_get(edata)) {
            ...
            san_unguard_pages_two_sided(tsdn, ehooks, edata, pac->emap);
        }
        /*
...
extent_destroy_wrapper(
  tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, edata_t *edata) {
    ...
    ehooks_destroy(tsdn, ehooks, edata_base_get(edata), edata_size_get(edata),
...
extent_purge_lazy_wrapper(tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata,
  size_t offset, size_t length) {
    return extent_purge_lazy_impl(tsdn, ehooks, edata, offset, length, false);
}
...
extent_purge_forced_wrapper(tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata,
  size_t offset, size_t length) {
    return extent_purge_forced_impl(tsdn, ehooks, edata, offset, length, false);
}
...
extent_split_impl(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, edata_t *edata,
  size_t size_a, size_t size_b, bool holding_core_locks) {
    ...
    edata_init(trail, edata_arena_ind_get(edata),
      ...
      /* slab */ false, SC_NSIZES, edata_sn_get(edata), edata_state_get(edata),
      edata_zeroed_get(edata), edata_committed_get(edata), EXTENT_PAI_PAC,
      EXTENT_NOT_HEAD);
    ...
    emap_split_commit(tsdn, pac->emap, &prepare, edata, size_a, trail, size_b);
...
extent_merge_impl(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, edata_t *a,
  edata_t *b, bool holding_core_locks) {
    ...
    bool err = ehooks_merge(tsdn, ehooks, edata_base_get(a), edata_size_get(a),
      edata_base_get(b), edata_size_get(b), edata_committed_get(a));
    ...
    edata_sn_set(a,
      (edata_sn_get(a) < edata_sn_get(b)) ? edata_sn_get(a) : edata_sn_get(b));
...
extent_commit_zero(tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, bool commit,
  bool zero, bool growing_retained) {
    ...
    if (commit && !edata_committed_get(edata)) {
        if (extent_commit_impl(tsdn, ehooks, edata, 0, edata_size_get(edata),
              growing_retained)) {
            return true;
        }
    }
