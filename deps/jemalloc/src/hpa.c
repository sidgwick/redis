...
hpdata_t *
hpa_central_extract(
  tsdn_t *tsdn, hpa_central_t *central, size_t size, bool *oom) {
    ...
    *oom = false;
    ...
    if (central->eden == NULL) {
        ...
        void *new_eden = pages_map(NULL, HPA_EDEN_SIZE, HUGEPAGE, &commit);
        if (new_eden == NULL) {
            *oom = true;
            malloc_mutex_unlock(tsdn, &central->grow_mtx);
            return NULL;
...
hpa_shard_init(hpa_shard_t *shard, hpa_central_t *central, emap_t *emap,
  ...
  const hpa_shard_opts_t *opts) {
    ...
    err = malloc_mutex_init(&shard->mtx, "hpa_shard", WITNESS_RANK_HPA_SHARD,
...
hpa_good_hugification_candidate(hpa_shard_t *shard, hpdata_t *ps) {
     ...
     * HUGEPAGE.
     */
    return hpdata_nactive_get(ps) * PAGE >= shard->opts.hugification_threshold;
}
...
hpa_ndirty_max(tsdn_t *tsdn, hpa_shard_t *shard) {
    ...
    return fxp_mul_frac(psset_nactive(&shard->psset), shard->opts.dirty_mult);
}
...
hpa_hugify_blocked_by_ndirty(tsdn_t *tsdn, hpa_shard_t *shard) {
    ...
    return hpa_adjusted_ndirty(tsdn, shard) + hpdata_nretained_get(to_hugify)
...
hpa_update_purge_hugify_eligibility(
  tsdn_t *tsdn, hpa_shard_t *shard, hpdata_t *ps) {
    ...
    if (hpa_good_hugification_candidate(shard, ps) && !hpdata_huge_get(ps)) {
        nstime_t now;
...
hpa_try_purge(tsdn_t *tsdn, hpa_shard_t *shard) {
    ...
    if (dehugify) {
        shard->central->hooks.dehugify(hpdata_addr_get(to_purge), HUGEPAGE);
    }
    ...
    size_t purge_size;
    while (
      hpdata_purge_next(to_purge, &purge_state, &purge_addr, &purge_size)) {
...
hpa_alloc(tsdn_t *tsdn, pai_t *self, size_t size, size_t alignment, bool zero,
  bool guarded, bool frequent_reuse, bool *deferred_work_generated) {
    ...
    size_t nallocs = hpa_alloc_batch(
...
hpa_time_until_deferred_work(tsdn_t *tsdn, pai_t *self) {
    ...
    if (hpa_should_purge(tsdn, shard)) {
        ...
        uint64_t since_last_purge_ms =
          shard->central->hooks.ms_since(&shard->last_purge);

        if (since_last_purge_ms < shard->opts.min_purge_interval_ms) {
            uint64_t until_purge_ns;
            until_purge_ns =
              shard->opts.min_purge_interval_ms - since_last_purge_ms;
...
hpa_assert_empty(tsdn_t *tsdn, hpa_shard_t *shard, psset_t *psset) {
    ...
    for (int huge = 0; huge <= 1; huge++) {
        ...
        for (pszind_t i = 0; i < PSSET_NPSIZES; i++) {
            hpa_shard_assert_stats_empty(&psset->stats.nonfull_slabs[i][huge]);
        }
    }
}
