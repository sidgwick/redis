...
bool
pa_central_init(
  pa_central_t *central, base_t *base, bool hpa, hpa_hooks_t *hpa_hooks) {
    bool err;
    if (hpa) {
...
pa_shard_init(tsdn_t *tsdn, pa_shard_t *shard, pa_central_t *central,
  ...
  malloc_mutex_t *stats_mtx, nstime_t *cur_time, size_t pac_oversize_threshold,
  ssize_t dirty_decay_ms, ssize_t muzzy_decay_ms) {
    ...
    if (pac_init(tsdn, &shard->pac, base, emap, &shard->edata_cache, cur_time,
          pac_oversize_threshold, dirty_decay_ms, muzzy_decay_ms,
...
pa_alloc(tsdn_t *tsdn, pa_shard_t *shard, size_t size, size_t alignment,
  ...
  bool *deferred_work_generated) {
    ...
    if (!guarded && pa_shard_uses_hpa(shard)) {
        edata = pai_alloc(tsdn, &shard->hpa_sec.pai, size, alignment, zero,
     ...
     */
    if (edata == NULL) {
        edata = pai_alloc(tsdn, &shard->pac.pai, size, alignment, zero, guarded,
...
pa_expand(tsdn_t *tsdn, pa_shard_t *shard, edata_t *edata, size_t old_size,
  size_t new_size, szind_t szind, bool zero, bool *deferred_work_generated) {
    ...
    bool error = pai_expand(
      tsdn, pai, edata, old_size, new_size, zero, deferred_work_generated);
    if (error) {
        return true;
    }
...
pa_shrink(tsdn_t *tsdn, pa_shard_t *shard, edata_t *edata, size_t old_size,
  size_t new_size, szind_t szind, bool *deferred_work_generated) {
    ...
    pai_t *pai = pa_get_pai(shard, edata);
    bool error =
      pai_shrink(tsdn, pai, edata, old_size, new_size, deferred_work_generated);
    if (error) {
        return true;
    }
...
bool
pa_shard_retain_grow_limit_get_set(
  tsdn_t *tsdn, pa_shard_t *shard, size_t *old_limit, size_t *new_limit) {
    return pac_retain_grow_limit_get_set(
      tsdn, &shard->pac, old_limit, new_limit);
}
