...
eset_stats_add(eset_t *eset, pszind_t pind, size_t sz) {
    size_t cur =
      atomic_load_zu(&eset->bin_stats[pind].nextents, ATOMIC_RELAXED);
    atomic_store_zu(&eset->bin_stats[pind].nextents, cur + 1, ATOMIC_RELAXED);
    cur = atomic_load_zu(&eset->bin_stats[pind].nbytes, ATOMIC_RELAXED);
    atomic_store_zu(&eset->bin_stats[pind].nbytes, cur + sz, ATOMIC_RELAXED);
}
...
eset_insert(eset_t *eset, edata_t *edata) {
    ...
    if (edata_heap_empty(&eset->bins[pind].heap)) {
         ...
         */
        if (edata_cmp_summary_comp(edata_cmp_summary, eset->bins[pind].heap_min)
          < 0) {
            eset->bins[pind].heap_min = edata_cmp_summary;
        }
    }
    ...
    atomic_store_zu(&eset->npages, cur_eset_npages + npages, ATOMIC_RELAXED);
}
...
eset_remove(eset_t *eset, edata_t *edata) {
    ...
    if (edata_heap_empty(&eset->bins[pind].heap)) {
        ...
        if (edata_cmp_summary_comp(edata_cmp_summary, eset->bins[pind].heap_min)
          == 0) {
            eset->bins[pind].heap_min =
              edata_cmp_summary_get(edata_heap_first(&eset->bins[pind].heap));
        }
    }
     ...
     */
    size_t cur_extents_npages = atomic_load_zu(&eset->npages, ATOMIC_RELAXED);
    assert(cur_extents_npages >= npages);
    atomic_store_zu(
      &eset->npages, cur_extents_npages - (size >> LG_PAGE), ATOMIC_RELAXED);
}
  ...
  eset_t *eset, size_t size, bool exact_only, unsigned lg_max_fit) {
    ...
    for (pszind_t i =
      ...
      i = (pszind_t)fb_ffs(eset->bitmap, ESET_NPSIZES, (size_t)i + 1)) {
        ...
        if (ret == NULL
          || edata_cmp_summary_comp(eset->bins[i].heap_min, ret_summ) < 0) {
            /*
            ...
            assert(ret == NULL
              || edata_cmp_summary_comp(
                   eset->bins[i].heap_min, edata_cmp_summary_get(edata))
                == 0);
            ret = edata;
