...
prof_dump_flush(void *opaque, const char *s) {
    ...
    if (!arg->error) {
        ssize_t err = prof_dump_write_file(arg->prof_dump_fd, s, strlen(s));
...
prof_dump_filename(tsd_t *tsd, char *filename, char v, uint64_t vseq) {
    ...
    if (vseq != VSEQ_INVALID) {
        ...
        malloc_snprintf(filename, DUMP_FILENAME_BUFSIZE,
          "%s.%d.%" FMTu64 ".%c.heap", prefix, prof_getpid(), prof_dump_seq, v);
    }
    prof_dump_seq++;
}
...
prof_get_default_filename(tsdn_t *tsdn, char *filename, uint64_t ind) {
    malloc_mutex_lock(tsdn, &prof_dump_filename_mtx);
    malloc_snprintf(filename, PROF_DUMP_FILENAME_LEN, "%s.%d.%" FMTu64 ".json",
      prof_prefix_get(tsdn), prof_getpid(), ind);
    malloc_mutex_unlock(tsdn, &prof_dump_filename_mtx);
}
...
prof_prefix_set(tsdn_t *tsdn, const char *prefix) {
    ...
    if (prof_prefix == NULL) {
        ...
        char *buffer =
          base_alloc(tsdn, prof_base, PROF_DUMP_FILENAME_LEN, QUANTUM);
        if (buffer == NULL) {
            return true;
        }
        malloc_mutex_lock(tsdn, &prof_dump_filename_mtx);
