...
base_map(tsdn_t *tsdn, ehooks_t *ehooks, unsigned ind, size_t size) {
    ...
    if (ehooks_are_default(ehooks)) {
        ...
        addr =
          ehooks_alloc(tsdn, ehooks, NULL, size, alignment, &zero, &commit);
    }
...
base_auto_thp_switch(tsdn_t *tsdn, base_t *base) {
    ...
    while (block != NULL) {
        ...
        if (config_stats) {
            base->n_thp +=
              HUGEPAGE_CEILING(block->size - edata_bsize_get(&block->edata))
              >> LG_HUGEPAGE;
        }
        block = block->next;
...
base_extent_bump_alloc_post(
  base_t *base, edata_t *edata, size_t gap_size, void *addr, size_t size) {
    if (edata_bsize_get(edata) > 0) {
        /*
         * Compute the index for the largest size class that does not
         * exceed extent's size.
         */
        szind_t index_floor = sz_size2index(edata_bsize_get(edata) + 1) - 1;
...
base_extent_bump_alloc(
  base_t *base, edata_t *edata, size_t size, size_t alignment) {
    void *ret;
    size_t gap_size;
...
base_block_alloc(tsdn_t *tsdn, base_t *base, ehooks_t *ehooks, unsigned ind,
  pszind_t *pind_last, size_t *extent_sn_next, size_t size, size_t alignment) {
    ...
    base_block_t *block =
      (base_block_t *)base_map(tsdn, ehooks, ind, block_size);
    if (block == NULL) {
        return NULL;
    }
...
base_extent_alloc(tsdn_t *tsdn, base_t *base, size_t size, size_t alignment) {
    ...
    malloc_mutex_unlock(tsdn, &base->mtx);
    base_block_t *block =
      base_block_alloc(tsdn, base, ehooks, base_ind_get(base), &base->pind_last,
...
base_new(tsdn_t *tsdn, unsigned ind, const extent_hooks_t *extent_hooks,
  bool metadata_use_hooks) {
    ...
    if (malloc_mutex_init(
          &base->mtx, "base", WITNESS_RANK_BASE, malloc_mutex_rank_exclusive)) {
    ...
    base_extent_bump_alloc_post(base, &block->edata, gap_size, base, base_size);
...
base_delete(tsdn_t *tsdn, base_t *base) {
    ...
    do {
        ...
        next = block->next;
        base_unmap(tsdn, ehooks, base_ind_get(base), block, block->size);
    } while (next != NULL);
}
...
base_alloc_edata(tsdn_t *tsdn, base_t *base) {
    size_t esn;
    edata_t *edata =
      base_alloc_impl(tsdn, base, sizeof(edata_t), EDATA_ALIGNMENT, &esn);
    if (edata == NULL) {
        return NULL;
    }
