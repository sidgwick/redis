...
psset_bin_stats_insert_remove(
  psset_t *psset, psset_bin_stats_t *binstats, hpdata_t *ps, bool insert) {
    ...
    if (config_debug) {
        ...
        for (size_t huge = 0; huge <= 1; huge++) {
            psset_bin_stats_accum(&check_stats, &psset->stats.full_slabs[huge]);
            psset_bin_stats_accum(
              &check_stats, &psset->stats.empty_slabs[huge]);
            for (pszind_t pind = 0; pind < PSSET_NPSIZES; pind++) {
                psset_bin_stats_accum(
                  &check_stats, &psset->stats.nonfull_slabs[pind][huge]);
            }
        }
        assert(psset->merged_stats.npageslabs == check_stats.npageslabs);
...
static void
psset_bin_stats_insert(
  psset_t *psset, psset_bin_stats_t *binstats, hpdata_t *ps) {
    psset_bin_stats_insert_remove(psset, binstats, ps, true);
}

static void
psset_bin_stats_remove(
  psset_t *psset, psset_bin_stats_t *binstats, hpdata_t *ps) {
...
psset_stats_insert(psset_t *psset, hpdata_t *ps) {
    if (hpdata_empty(ps)) {
        ...
        pszind_t pind =
          sz_psz2ind(sz_psz_quantize_floor(longest_free_range << LG_PAGE));
        assert(pind < PSSET_NPSIZES);

        psset_bin_stats_insert(psset, psset->stats.nonfull_slabs[pind], ps);
    }
}
...
psset_stats_remove(psset_t *psset, hpdata_t *ps) {
    if (hpdata_empty(ps)) {
        ...
        psset_bin_stats_remove(psset, psset->stats.nonfull_slabs[pind], ps);
    }
}
...
psset_pick_alloc(psset_t *psset, size_t size) {
    ...
    pszind_t pind =
      (pszind_t)fb_ffs(psset->pageslab_bitmap, PSSET_NPSIZES, (size_t)min_pind);
...
psset_pick_purge(psset_t *psset) {
    ssize_t ind_ssz =
      fb_fls(psset->purge_bitmap, PSSET_NPURGE_LISTS, PSSET_NPURGE_LISTS - 1);
    if (ind_ssz < 0) {
        return NULL;
    }
