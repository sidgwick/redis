...
static void test_reply_reader(void)
{
    ...
    test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_ARRAY &&
    ...
    ret = redisReaderGetReply(reader, &reply);
    test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_INTEGER &&
    ...
    ret = redisReaderGetReply(reader, &reply);
    test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_VERB &&
    ...
    redisReaderFeed(reader, (char *)">2\r\n$6\r\nLOLWUT\r\n:42\r\n", 21);
    ret = redisReaderGetReply(reader, &reply);
    test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_PUSH &&
              ((redisReply *)reply)->elements == 2 && ((redisReply *)reply)->element[0]->type == REDIS_REPLY_STRING &&
    ...
    test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_DOUBLE &&
              ...
              ((redisReply *)reply)->len == 22 && strcmp(((redisReply *)reply)->str, "3.14159265358979323846") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
    ...
    ret = redisReaderGetReply(reader, &reply);
    test_cond(
        ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_MAP && ((redisReply *)reply)->elements == 4 &&
        ...
        ((redisReply *)reply)->element[2]->type == REDIS_REPLY_STRING && ((redisReply *)reply)->element[2]->len == 6 &&
    ...
    ret = redisReaderGetReply(reader, &reply);
    test_cond(
        ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_SET && ((redisReply *)reply)->elements == 5 &&
        ...
        ((redisReply *)reply)->element[1]->type == REDIS_REPLY_STRING && ((redisReply *)reply)->element[1]->len == 5 &&
    ...
    ret = redisReaderGetReply(reader, &reply);
    test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_BIGNUM &&
    ...
    ret = redisReaderGetReply(reader, &reply);
    test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_ARRAY &&
              ((redisReply *)reply)->elements == 1 && ((redisReply *)reply)->element[0]->type == REDIS_REPLY_DOUBLE &&
...
static void test_blocking_connection(struct config config)
{
    ...
    test_cond(reply->type == REDIS_REPLY_ARRAY && reply->elements == 2 &&
              reply->element[0]->type == REDIS_REPLY_ARRAY && reply->element[0]->elements == 2 &&
...
void subscribe_cb(redisAsyncContext *ac, void *r, void *privdata)
{
    ...
    assert(reply != NULL && reply->type == (state->resp3 ? REDIS_REPLY_PUSH : REDIS_REPLY_ARRAY) &&
           reply->elements == 3);
...
void subscribe_channel_a_cb(redisAsyncContext *ac, void *r, void *privdata)
{
    ...
    if (strcmp(reply->element[0]->str, "subscribe") == 0) {
        ...
        redisAsyncCommand(ac, unexpected_cb, (void *)"unsubscribe should not call unexpected_cb()",
