...
static void counts_inc_normalised(struct hdr_histogram *h, int32_t index, int64_t value)
{
...
static void counts_inc_normalised_atomic(struct hdr_histogram *h, int32_t index, int64_t value)
{
    int32_t normalised_index = normalize_index(h, index);
...
{
    int32_t pow2ceiling =
        64 - count_leading_zeros_64(value | h->sub_bucket_mask); /* smallest power of 2 containing value */
...
static int64_t size_of_equivalent_value_range_given_bucket_indices(const struct hdr_histogram *h, int32_t bucket_index,
...
static int64_t lowest_equivalent_value_given_bucket_indices(const struct hdr_histogram *h, int32_t bucket_index,
                                                            int32_t sub_bucket_index)
{
    return value_from_index(bucket_index, sub_bucket_index, h->unit_magnitude);
}
...
int hdr_calculate_bucket_config(int64_t lowest_discernible_value, int64_t highest_trackable_value,
                                int significant_figures, struct hdr_histogram_bucket_config *cfg)
{
    ...
    cfg->bucket_count =
        buckets_needed_to_cover_value(highest_trackable_value, cfg->sub_bucket_count, (int32_t)cfg->unit_magnitude);
...
int hdr_init(int64_t lowest_discernible_value, int64_t highest_trackable_value, int significant_figures,
...
bool hdr_record_corrected_values_atomic(struct hdr_histogram *h, int64_t value, int64_t count,
...
int64_t hdr_add_while_correcting_for_coordinated_omission(struct hdr_histogram *h, struct hdr_histogram *from,
                                                          int64_t expected_interval)
{
    struct hdr_iter iter;
    int64_t dropped = 0;
...
int hdr_value_at_percentiles(const struct hdr_histogram *h, const double *percentiles, int64_t *values, size_t length)
{
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        const int64_t count_at_percentile = (int64_t)(((requested_percentile / 100) * total_count) + 0.5);
...
static bool move_next(struct hdr_iter *iter)
{
    ...
    const int64_t size_of_equivalent_value_range =
...
static bool next_value_greater_than_reporting_level_upper_bound(struct hdr_iter *iter,
                                                                int64_t reporting_level_upper_bound)
{
...
static bool iter_linear_next(struct hdr_iter *iter)
{
    ...
    if (has_next(iter) || next_value_greater_than_reporting_level_upper_bound(
...
void hdr_iter_log_init(struct hdr_iter *iter, const struct hdr_histogram *h, int64_t value_units_first_bucket,
                       double log_base)
{
    hdr_iter_init(iter, h);
...
int hdr_percentiles_print(struct hdr_histogram *h, FILE *stream, int32_t ticks_per_half_distance, double value_scale,
                          format_type format)
{
    char line_format[25];
    ...
    if (CLASSIC == format) {
        ...
        if (fprintf(stream, CLASSIC_FOOTER, mean, stddev, max, h->total_count, h->bucket_count, h->sub_bucket_count) <
            0) {
            rc = EIO;
            goto cleanup;
        }
