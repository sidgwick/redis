...
void updateCurIncrAofEndOffset(void)
{
    ...
    if (listLength(server.aof_manifest->incr_aof_list) == 0)
        return;
...
void updateReplOffsetAndResetEndOffset(void)
{
    ...
    if (ai->end_offset != -1) {
        ...
        server.master_repl_offset =
            (ai->start_offset == -1 ? 0 : ai->start_offset) + getAppendOnlyFileSize(ai->file_name, NULL);
    }
}
...
int aofRewriteLimited(void)
{
    ...
    serverLog(LL_WARNING,
              "Background AOF rewrite has repeatedly failed and triggered the limit, will retry in "
              "%d minutes",
              next_delay_minutes);
    return 1;
}
...
int startAppendOnly(void)
{
    ...
    if (hasActiveChildProcess() && server.child_type != CHILD_TYPE_AOF) {
        ...
        serverLog(LL_NOTICE, "AOF was enabled but there is already another background operation. "
                             "An AOF background was scheduled to start when possible.");
    } else if (server.in_exec) {
        server.aof_rewrite_scheduled = 1;
        serverLog(LL_NOTICE, "AOF was enabled during a transaction. An AOF background was "
        ...
        if (server.child_type == CHILD_TYPE_AOF) {
            serverLog(LL_NOTICE, "AOF was enabled but there is already an AOF rewriting in "
        ...
        if (rewriteAppendOnlyFileBackground() == C_ERR) {
            server.aof_state = AOF_OFF;
            serverLog(LL_WARNING, "Redis needs to enable the AOF but can't trigger a background AOF rewrite "
...
void flushAppendOnlyFile(int force)
{
    ...
    if (sdslen(server.aof_buf) == 0) {
        if (server.aof_last_incr_fsync_offset == server.aof_last_incr_size) {
            ...
            if (server.aof_fsync == AOF_FSYNC_EVERYSEC && server.mstime - server.aof_last_fsync >= 1000 &&
    ...
    if (server.aof_fsync == AOF_FSYNC_EVERYSEC && !force) {
        ...
        if (sync_in_progress) {
            ...
            serverLog(LL_NOTICE, "Asynchronous AOF fsync is taking too long (disk is busy?). Writing the AOF "
    ...
    if (nwritten != (ssize_t)sdslen(server.aof_buf)) {
        ...
        if (server.aof_fsync == AOF_FSYNC_ALWAYS) {
            ...
            serverLog(LL_WARNING, "Can't recover from AOF write error when the AOF fsync policy is "
                                  "'always'. Exiting...");
            exit(1);
        } else {
...
uxeof: /* Unexpected AOF end of file. */
    ...
    serverLog(LL_WARNING,
              ...
              "2) Alternatively you can set the 'aof-load-truncated' configuration option to yes and "
              "restart the server.",
              filename);
    ret = AOF_FAILED;
    goto cleanup;
...
int rewriteHashObject(rio *r, robj *key, robj *o)
{
    ...
    if (!isHFE) {
        ...
        while (hashTypeNext(hi, 0) != C_ERR) {
            ...
            char hmsetCmd[] = "*4\r\n$5\r\nHMSET\r\n";
            if ((!rioWrite(r, hmsetCmd, sizeof(hmsetCmd) - 1)) || (!rioWriteBulkObject(r, key)) ||
            ...
            if (hi->expire_time != EB_EXPIRE_TIME_INVALID) {
                char cmd[] = "*6\r\n$10\r\nHPEXPIREAT\r\n";
                if ((!rioWrite(r, cmd, sizeof(cmd) - 1)) || (!rioWriteBulkObject(r, key)) ||
                    (!rioWriteBulkLongLong(r, hi->expire_time)) || (!rioWriteBulkString(r, "FIELDS", 6)) ||
                    (!rioWriteBulkString(r, "1", 1)) || (!rioWriteHashIteratorCursor(r, hi, OBJ_HASH_KEY)))
                    goto reHashEnd;
            }
        }
    }
...
int rioWriteStreamEmptyConsumer(rio *r, robj *key, const char *groupname, size_t groupname_len,
