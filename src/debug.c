...
void debugCommand(client *c)
{
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
        const char *help[] = {"AOF-FLUSH-SLEEP <microsec>",
                              ...
                              "INTERNAL_SECRET",
                              "    Return the cluster internal secret (hashed with crc16) or error if not in cluster "
                              "mode.",
                              ...
                              "    Graceful restart: save config, db, restart after a <milliseconds> delay (default "
                              "0).",
                              "SDSLEN <key>",
                              ...
                              "    In case RESET is provided the peak reset time will be restored to the default "
                              "value",
        ...
        *p = 'x';
        ...
        addReplyStatusFormat(c,
                             ...
                             (void *)kv, kv->refcount, strenc, rdbSavedObjectLen(kv, c->argv[2], c->db->id), kv->lru,
                             estimateObjectIdleTime(kv) / 1000, extra);
        ...
        if (kv->type != OBJ_STRING || !sdsEncodedObject(val)) {
            ...
            addReplyStatusFormat(c,
                                 ...
                                 (long long)sdslen(val->ptr), (long long)sdsavail(val->ptr), (long long)val_alloc);
        }
    } else if (!strcasecmp(c->argv[1]->ptr, "listpack") && c->argc == 3) {
        kvobj *o;

        if ((o = kvobjCommandLookupOrReply(c, c->argv[2], shared.nokeyerr)) == NULL)
            return;
        ...
        replicationFeedSlaves(server.slaves, -1, c->argv + 2, c->argc - 2);
        addReply(c, shared.ok);
...
#else
static void setupStacktracePipe(void)
{ /* we don't need a pipe to write the stacktraces */
}
#endif
#ifdef HAVE_BACKTRACE
...
__attribute__((noinline)) static void writeStacktraces(int fd, int uplevel)
{
    ...
    if (!ThreadsManager_runOnThreads(tids, len_tids, collect_stacktrace_data))
        return;
...
void doFastMemoryTest(void)
{
    ...
    if (server.memcheck_enabled) {
        ...
        if (memtest_test_linux_anonymous_maps()) {
            ...
            serverLogRaw(LL_WARNING | LL_RAW,
                         "Fast memory test PASSED, however your memory can still be broken. Please "
                         "run a memory test for several hours if possible.\n");
        }
    }
#endif /* HAVE_PROC_MAPS */
}
...
__attribute__((noinline)) static void sigsegvHandler(int sig, siginfo_t *info, void *secret)
{
    ...
    bugReportStart();
    serverLog(LL_WARNING, "Redis %s crashed by signal: %d, si_code: %d", REDIS_VERSION, sig, info->si_code);
...
void bugReportEnd(int killViaSignal, int sig)
{
    ...
    if (server.daemonize && server.supervised == 0 && server.pidfile)
...
static size_t get_ready_to_signal_threads_tids(int sig_num, pid_t tids[TIDS_MAX_SIZE])
{
    ...
    while ((nread = syscall(SYS_getdents64, dir, buff, PATH_MAX))) {
        ...
        for (long pos = 0; pos < nread;) {
            ...
            if (!is_thread_ready_to_signal(path_buff, entry->d_name, sig_num))
                continue;
