...
static int moduleValidateCommandArgs(RedisModuleCommandArg *args, const RedisModuleCommandInfoVersion *version);
...
static void moduleFreeKeyIterator(RedisModuleKey *key)
{
    ...
    case OBJ_STREAM:
        ...
        break;
    default:
        serverAssert(0); /* No key->iter for other types. */
    }
    key->iter = NULL;
}
...
{
    if (!(key->mode & REDISMODULE_WRITE) || key->kv == NULL)
        return 0;
    int isempty;
    robj *o = key->kv;
...
{
    if (!(ctx->flags & REDISMODULE_CTX_KEYS_POS_REQUEST) || !ctx->keys_result)
        return;
    if (pos <= 0)
        return;
...
{
    if (!(ctx->flags & REDISMODULE_CTX_CHANNELS_POS_REQUEST) || !ctx->keys_result)
        return;
    if (pos <= 0)
        return;
    ...
    int new_flags = 0;
    if (flags & REDISMODULE_CMD_CHANNEL_SUBSCRIBE)
        new_flags |= CMD_CHANNEL_SUBSCRIBE;
    if (flags & REDISMODULE_CMD_CHANNEL_UNSUBSCRIBE)
        new_flags |= CMD_CHANNEL_UNSUBSCRIBE;
    if (flags & REDISMODULE_CMD_CHANNEL_PUBLISH)
        new_flags |= CMD_CHANNEL_PUBLISH;
    if (flags & REDISMODULE_CMD_CHANNEL_PATTERN)
        new_flags |= CMD_CHANNEL_PATTERN;
...
int64_t commandFlagsFromString(char *s)
{
    ...
    for (j = 0; j < count; j++) {
        ...
        } else if (!strcasecmp(t, "blocking"))
            flags |= CMD_BLOCKING;
        else if (!strcasecmp(t, "allow-stale"))
            flags |= CMD_STALE;
        else if (!strcasecmp(t, "no-monitor"))
            flags |= CMD_SKIP_MONITOR;
        else if (!strcasecmp(t, "no-slowlog"))
            flags |= CMD_SKIP_SLOWLOG;
        else if (!strcasecmp(t, "fast"))
            flags |= CMD_FAST;
        else if (!strcasecmp(t, "no-auth"))
            flags |= CMD_NO_AUTH;
        else if (!strcasecmp(t, "may-replicate"))
            flags |= CMD_MAY_REPLICATE;
        else if (!strcasecmp(t, "getkeys-api"))
            flags |= CMD_MODULE_GETKEYS;
        else if (!strcasecmp(t, "getchannels-api"))
            flags |= CMD_MODULE_GETCHANNELS;
        else if (!strcasecmp(t, "no-cluster"))
            flags |= CMD_MODULE_NO_CLUSTER;
        else if (!strcasecmp(t, "no-mandatory-keys"))
            flags |= CMD_NO_MANDATORY_KEYS;
        else if (!strcasecmp(t, "allow-busy"))
            flags |= CMD_ALLOW_BUSY;
        else if (!strcasecmp(t, "internal"))
            flags |= (CMD_INTERNAL | CMD_NOSCRIPT); /* We also disallow internal commands in scripts. */
        else
            break;
    }
...
RedisModuleCommand *moduleCreateCommandProxy(struct RedisModule *module, sds declared_name, sds fullname,
                                             RedisModuleCmdFunc cmdfunc, int64_t flags, int firstkey, int lastkey,
                                             int keystep);
                     ...
                     int firstkey, int lastkey, int keystep)
{
    ...
    RedisModuleCommand *cp = moduleCreateCommandProxy(ctx->module, declared_name, sdsdup(declared_name), cmdfunc, flags,
 ...
 */
int RM_CreateSubcommand(RedisModuleCommand *parent, const char *name, RedisModuleCmdFunc cmdfunc, const char *strflags,
                        int firstkey, int lastkey, int keystep)
{
...
static RedisModuleCommandHistoryEntry *moduleCmdHistoryEntryAt(const RedisModuleCommandInfoVersion *version,
...
static RedisModuleCommandKeySpec *moduleCmdKeySpecAt(const RedisModuleCommandInfoVersion *version,
...
static RedisModuleCommandArg *moduleCmdArgAt(const RedisModuleCommandInfoVersion *version,
...
int RM_SetCommandInfo(RedisModuleCommand *command, const RedisModuleCommandInfo *info)
{
    ...
    if (info->history) {
        ...
        for (size_t j = 0; j < count; j++) {
            RedisModuleCommandHistoryEntry *entry = moduleCmdHistoryEntryAt(version, info->history, j);
    ...
    if (info->key_specs) {
        ...
        for (size_t j = 0; j < count; j++) {
            RedisModuleCommandKeySpec *spec = moduleCmdKeySpecAt(version, info->key_specs, j);
...
static int moduleValidateCommandInfo(const RedisModuleCommandInfo *info)
{
    ...
    if (info->key_specs) {
        for (size_t j = 0; moduleCmdKeySpecAt(version, info->key_specs, j)->begin_search_type; j++) {
            ...
            uint64_t key_flags =
                REDISMODULE_CMD_KEY_RO | REDISMODULE_CMD_KEY_RW | REDISMODULE_CMD_KEY_OW | REDISMODULE_CMD_KEY_RM;
            uint64_t write_flags = REDISMODULE_CMD_KEY_INSERT | REDISMODULE_CMD_KEY_DELETE | REDISMODULE_CMD_KEY_UPDATE;
...
{
    int64_t out = 0;
    int64_t map[][2] = {{REDISMODULE_CMD_KEY_RO, CMD_KEY_RO},
...
static int moduleValidateCommandArgs(RedisModuleCommandArg *args, const RedisModuleCommandInfoVersion *version)
{
    ...
    for (size_t j = 0; moduleCmdArgAt(version, args, j)->name != NULL; j++) {
        ...
        if (arg->type == REDISMODULE_ARG_TYPE_ONEOF || arg->type == REDISMODULE_ARG_TYPE_BLOCK) {
            if (arg->subargs == NULL) {
                serverLog(LL_WARNING,
                          "Invalid command info: Argument \"%s\": "
            ...
            if (!moduleValidateCommandArgs(arg->subargs, version))
                return 0;
        } else {
            if (arg->subargs != NULL) {
                serverLog(LL_WARNING,
...
{
    size_t count = 0;
    while (moduleCmdArgAt(version, args, count)->name)
        count++;
    ...
    for (size_t j = 0; j < count; j++) {
        ...
        if (arg->token)
            realargs[j].token = zstrdup(arg->token);
        if (arg->summary)
            realargs[j].summary = zstrdup(arg->summary);
        if (arg->since)
            realargs[j].since = zstrdup(arg->since);
        if (arg->deprecated_since)
            realargs[j].deprecated_since = zstrdup(arg->deprecated_since);
        if (arg->display_text)
            realargs[j].display_text = zstrdup(arg->display_text);
        realargs[j].flags = moduleConvertArgFlags(arg->flags);
        if (arg->subargs)
            realargs[j].subargs = moduleCopyCommandArgs(arg->subargs, version);
    }
    return realargs;
}
...
int moduleListConfigMatch(void *config, void *name)
{
    ...
    return strcasecmp(mc->name, (char *)name) == 0 || ((mc->alias) && strcasecmp(mc->alias, (char *)name) == 0);
}
...
void autoMemoryCollect(RedisModuleCtx *ctx)
{
    ...
    for (j = 0; j < ctx->amqueue_used; j++) {
        ...
        case REDISMODULE_AM_INFO:
            ...
            break;
        case REDISMODULE_AM_CONFIG:
            RM_ConfigIteratorRelease(NULL, ptr);
            break;
        }
    }
    ctx->flags |= REDISMODULE_CTX_AUTO_MEMORY;
...
int RM_GetContextFlags(RedisModuleCtx *ctx)
{
    ...
    if (server.in_fork_child)
        flags |= REDISMODULE_CTX_FLAGS_IS_CHILD;
...
int RM_StringTruncate(RedisModuleKey *key, size_t newlen)
{
    if (!(key->mode & REDISMODULE_WRITE))
        return REDISMODULE_ERR;
    if (key->kv && key->kv->type != OBJ_STRING)
        return REDISMODULE_ERR;
...
int RM_ListPush(RedisModuleKey *key, int where, RedisModuleString *ele)
{
    ...
    listTypePush(key->kv, ele, (where == REDISMODULE_LIST_HEAD) ? LIST_HEAD : LIST_TAIL);
...
RedisModuleString *RM_ListPop(RedisModuleKey *key, int where)
{
    ...
    if (key->iter)
        moduleFreeKeyIterator(key);
    robj *ele = listTypePop(key->kv, (where == REDISMODULE_LIST_HEAD) ? LIST_HEAD : LIST_TAIL);
...
int RM_ListInsert(RedisModuleKey *key, long index, RedisModuleString *value)
{
    if (!value) {
        errno = EINVAL;
        return REDISMODULE_ERR;
    } else if (key != NULL && key->kv == NULL && (index == 0 || index == -1)) {
        /* Insert in empty key => push. */
        return RM_ListPush(key, REDISMODULE_LIST_TAIL, value);
    } else if (key != NULL && key->kv != NULL && key->kv->type == OBJ_LIST &&
...
int RM_ListDelete(RedisModuleKey *key, long index)
{
    if (moduleListIteratorSeek(key, index, REDISMODULE_WRITE)) {
        ...
        listTypeTryConversion(key->kv, LIST_CONV_SHRINKING, moduleFreeListIterator, key);
        if (!key->iter)
            return REDISMODULE_OK; /* Return ASAP if iterator has been freed */
...
int moduleZsetAddFlagsToCoreFlags(int flags)
{
    ...
    if (flags & REDISMODULE_ZADD_NX)
        retflags |= ZADD_IN_NX;
    if (flags & REDISMODULE_ZADD_GT)
        retflags |= ZADD_IN_GT;
    if (flags & REDISMODULE_ZADD_LT)
        retflags |= ZADD_IN_LT;
    return retflags;
}
...
{
    int retflags = 0;
    if (flags & ZADD_OUT_ADDED)
        retflags |= REDISMODULE_ZADD_ADDED;
    if (flags & ZADD_OUT_UPDATED)
        retflags |= REDISMODULE_ZADD_UPDATED;
    if (flags & ZADD_OUT_NOP)
...
int RM_ZsetAdd(RedisModuleKey *key, double score, RedisModuleString *ele, int *flagsptr)
{
    ...
    if (!(key->mode & REDISMODULE_WRITE))
        return REDISMODULE_ERR;
    if (key->kv && key->kv->type != OBJ_ZSET)
        return REDISMODULE_ERR;
    if (key->kv == NULL)
...
int RM_ZsetScore(RedisModuleKey *key, RedisModuleString *ele, double *score)
{
    if (key->kv == NULL)
        return REDISMODULE_ERR;
    if (key->kv->type != OBJ_ZSET)
        return REDISMODULE_ERR;
    if (zsetScore(key->kv, ele->ptr, score) == C_ERR)
        return REDISMODULE_ERR;
    return REDISMODULE_OK;
}
...
int zsetInitScoreRange(RedisModuleKey *key, double min, double max, int minex, int maxex, int first)
{
    ...
    if (key->kv->encoding == OBJ_ENCODING_LISTPACK) {
        key->u.zset.current = first ? zzlFirstInRange(key->kv->ptr, zrs) : zzlLastInRange(key->kv->ptr, zrs);
...
int zsetInitLexRange(RedisModuleKey *key, RedisModuleString *min, RedisModuleString *max, int first)
{
    ...
    if (key->kv->encoding == OBJ_ENCODING_LISTPACK) {
        key->u.zset.current = first ? zzlFirstInLexRange(key->kv->ptr, zlrs) : zzlLastInLexRange(key->kv->ptr, zlrs);
    } else if (key->kv->encoding == OBJ_ENCODING_SKIPLIST) {
        zset *zs = key->kv->ptr;
        zskiplist *zsl = zs->zsl;
...
int RM_ZsetRangeNext(RedisModuleKey *key)
{
    if (!key->kv || key->kv->type != OBJ_ZSET)
        return 0;
    if (!key->u.zset.type || !key->u.zset.current)
    ...
    if (key->kv->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (next)
            next = lpNext(zl, next); /* Skip score. */
        if (next == NULL) {
            key->u.zset.er = 1;
            return 0;
        } else {
        ...
        if (next == NULL) {
            ...
            /* Are we still within the range? */
            if (key->u.zset.type == REDISMODULE_ZSET_RANGE_SCORE && !zslValueLteMax(next->score, &key->u.zset.rs)) {
                key->u.zset.er = 1;
                return 0;
            } else if (key->u.zset.type == REDISMODULE_ZSET_RANGE_LEX) {
...
int RM_ZsetRangePrev(RedisModuleKey *key)
{
    ...
    if (key->kv->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (prev)
            prev = lpPrev(zl, prev); /* Back to previous ele. */
        if (prev == NULL) {
            key->u.zset.er = 1;
            return 0;
        } else {
        ...
        if (prev == NULL) {
            ...
            /* Are we still within the range? */
            if (key->u.zset.type == REDISMODULE_ZSET_RANGE_SCORE && !zslValueGteMin(prev->score, &key->u.zset.rs)) {
                key->u.zset.er = 1;
                return 0;
            } else if (key->u.zset.type == REDISMODULE_ZSET_RANGE_LEX) {
...
{
    va_list ap;
    if (!key || (flags & ~(REDISMODULE_HASH_NX | REDISMODULE_HASH_XX | REDISMODULE_HASH_CFIELDS |
    ...
    while (1) {
        ...
        if (flags & (REDISMODULE_HASH_XX | REDISMODULE_HASH_NX)) {
            ...
            if (((flags & REDISMODULE_HASH_XX) && !exists) || ((flags & REDISMODULE_HASH_NX) && exists)) {
                if (flags & REDISMODULE_HASH_CFIELDS)
                    decrRefCount(field);
                continue;
            }
    ...
    va_end(ap);
    updateKeysizesHist(key->db, getKeySlot(key->key->ptr), OBJ_HASH, oldlen, (int64_t)hashTypeLength(key->kv, 0));
        ...
        if (flags & REDISMODULE_HASH_EXISTS) {
            ...
            *expireptr = REDISMODULE_NO_EXPIRE;
            if (key->kv) {
                ...
                int res = hashTypeGetValueObject(key->db, key->kv, field->ptr, hfeFlags, NULL, &expireTime, NULL);
                /* If field has expiration time */
                if (res && expireTime != 0)
                    *expireptr = expireTime;
            }
        } else {
            valueptr = va_arg(ap, RedisModuleString **);
            if (key->kv) {
                hashTypeGetValueObject(key->db, key->kv, field->ptr, hfeFlags, valueptr, NULL, NULL);
...
int RM_StreamAdd(RedisModuleKey *key, int flags, RedisModuleStreamID *id, RedisModuleString **argv, long numfields)
{
    ...
    if (!key || (numfields != 0 && !argv) ||                 /* invalid key or argv */
        ...
        return REDISMODULE_ERR;
    } else if (!(flags & REDISMODULE_STREAM_ADD_AUTOID) && id->ms == 0 && id->seq == 0) {
...
int RM_StreamDelete(RedisModuleKey *key, RedisModuleStreamID *id)
{
    if (!key || !id) {
        ...
        errno = ENOTSUP; /* wrong type */
        return REDISMODULE_ERR;
    } else if (!(key->mode & REDISMODULE_WRITE) || key->iter != NULL) {
...
{
    /* check args */
    if (!key || (flags & ~(REDISMODULE_STREAM_ITERATOR_EXCLUSIVE | REDISMODULE_STREAM_ITERATOR_REVERSE))) {
    ...
    if (start)
        lower = (streamID){start->ms, start->seq};
    if (end)
        upper = (streamID){end->ms, end->seq};
...
int RM_CallReplyMapElement(RedisModuleCallReply *reply, size_t idx, RedisModuleCallReply **key,
 ...
 * The `key` and `value` arguments are used to return by reference, and may be
 * NULL if not required. */
int RM_CallReplyAttributeElement(RedisModuleCallReply *reply, size_t idx, RedisModuleCallReply **key,
                                 RedisModuleCallReply **val)
{
...
void RM_CallReplyPromiseSetUnblockHandler(RedisModuleCallReply *reply, RedisModuleOnUnblocked on_unblock,
                                          void *private_data)
{
    RedisModuleAsyncRMCallPromise *promise = callReplyGetPrivateData(reply);
...
int RM_CallReplyPromiseAbort(RedisModuleCallReply *reply, void **private_data)
{
    RedisModuleAsyncRMCallPromise *promise = callReplyGetPrivateData(reply);
    if (!promise->c)
        return REDISMODULE_ERR; /* Promise can not be aborted, either already aborted or already finished. */
    if (!(promise->c->flags & CLIENT_BLOCKED))
        return REDISMODULE_ERR; /* Client is not blocked anymore, can not abort it. */

    /* Client is still blocked, remove it from any blocking state and release it. */
    if (private_data)
        *private_data = promise->private_data;
...
robj **moduleCreateArgvFromUserFormat(const char *cmdname, const char *fmt, int *argcp, int *flags, va_list ap)
{
    ...
    while (*p) {
        if (*p == 'c') {
            ...
            if (flags)
                (*flags) |= REDISMODULE_ARGV_RESP_3;
        } else if (*p == '0') {
            if (flags)
                (*flags) |= REDISMODULE_ARGV_RESP_AUTO;
        } else if (*p == 'C') {
            if (flags)
            ...
            if (flags)
                (*flags) |= REDISMODULE_ARGV_NO_WRITES;
        } else if (*p == 'M') {
            if (flags)
                (*flags) |= REDISMODULE_ARGV_RESPECT_DENY_OOM;
        } else if (*p == 'E') {
            if (flags)
                (*flags) |= REDISMODULE_ARGV_CALL_REPLIES_AS_ERRORS;
        } else if (*p == 'D') {
            if (flags)
                (*flags) |= (REDISMODULE_ARGV_DRY_RUN | REDISMODULE_ARGV_CALL_REPLIES_AS_ERRORS);
        } else if (*p == 'K') {
            if (flags)
                (*flags) |= REDISMODULE_ARGV_ALLOW_BLOCK;
        } else {
            goto fmterr;
        }
        p++;
...
RedisModuleCallReply *RM_Call(RedisModuleCtx *ctx, const char *cmdname, const char *fmt, ...)
{
    ...
    user *user = NULL;
    if ((flags & REDISMODULE_ARGV_RUN_AS_USER) && !(ctx->client->flags & CLIENT_INTERNAL)) {
    ...
    if (c->cmd && (c->cmd->flags & CMD_INTERNAL) && (flags & REDISMODULE_ARGV_RUN_AS_USER) &&
    ...
    if (server.cluster_enabled && !mustObeyClient(ctx->client)) {
        ...
        if (getNodeByQuery(c, c->cmd, c->argv, c->argc, NULL, cmd_flags, &error_code) != getMyClusterNode()) {
            sds msg = NULL;
            if (error_code == CLUSTER_REDIR_DOWN_RO_STATE) {
                if (error_as_call_replies) {
                    msg = sdscatfmt(sdsempty(),
                                    "Can not execute a write command '%S' while the cluster is "
                                    "down and readonly",
                                    c->cmd->fullname);
                }
                errno = EROFS;
...
{
    if (key == NULL || key->kv == NULL || RM_KeyType(key) != REDISMODULE_KEYTYPE_MODULE)
        return NULL;
...
int moduleAllDatatypesHandleErrors(void)
{
    ...
    while ((de = dictNext(&di)) != NULL) {
        struct RedisModule *module = dictGetVal(de);
        if (listLength(module->types) && !(module->options & REDISMODULE_OPTIONS_HANDLE_IO_ERRORS)) {
            dictResetIterator(&di);
            return 0;
        }
    }
...
RedisModuleBlockedClient *moduleBlockClient(RedisModuleCtx *ctx, RedisModuleCmdFunc reply_callback,
                                            ...
                                            RedisModuleString **keys, int numkeys, void *privdata, int flags)
{
    client *c = ctx->client;
...
{
    if (!listLength(moduleAuthCallbacks))
...
RedisModuleBlockedClient *RM_BlockClientOnKeysWithFlags(RedisModuleCtx *ctx, RedisModuleCmdFunc reply_callback,
                                                        RedisModuleCmdFunc timeout_callback,
                                                        void (*free_privdata)(RedisModuleCtx *, void *),
                                                        long long timeout_ms, RedisModuleString **keys, int numkeys,
...
int RM_UnblockClient(RedisModuleBlockedClient *bc, void *privdata)
{
    if (bc->blocked_on_keys) {
        ...
        if (bc->unblocked)
            return REDISMODULE_OK;
        if (bc->client)
            bc->blocked_on_keys_explicit_unblock = 1;
    }
...
RedisModuleCtx *RM_GetThreadSafeContext(RedisModuleBlockedClient *bc)
{
     ...
     * a new client */
    if (!bc)
        flags |= REDISMODULE_CTX_NEW_CLIENT;
...
RedisModuleCtx *RM_GetDetachedThreadSafeContext(RedisModuleCtx *ctx)
{
    ...
    moduleCreateContext(new_ctx, ctx->module, REDISMODULE_CTX_THREAD_SAFE | REDISMODULE_CTX_NEW_CLIENT);
    return new_ctx;
}
...
int RM_AddPostNotificationJob(RedisModuleCtx *ctx, RedisModulePostNotificationJobFunc callback, void *privdata,
...
int RM_GetClusterNodeInfo(RedisModuleCtx *ctx, const char *id, char *ip, char *master_id, int *port, int *flags)
{
    ...
    if (ip)
        redis_strlcpy(ip, clusterNodeIp(node), NET_IP_STR_LEN);
    ...
    if (port)
        *port = getNodeDefaultClientPort(node);
...
int RM_EventLoopAdd(int fd, int mask, RedisModuleEventLoopFunc func, void *user_data)
{
    ...
    if (!func || mask & ~(REDISMODULE_EVENTLOOP_READABLE | REDISMODULE_EVENTLOOP_WRITABLE)) {
        errno = EINVAL;
        return REDISMODULE_ERR;
    }
...
int RM_EventLoopDel(int fd, int mask)
{
    ...
    if (mask & ~(REDISMODULE_EVENTLOOP_READABLE | REDISMODULE_EVENTLOOP_WRITABLE)) {
        errno = EINVAL;
        return REDISMODULE_ERR;
    }
    ...
    if (err) {
        if (error) {
            *error = createObject(OBJ_STRING, err);
            if (ctx != NULL)
                autoMemoryAdd(ctx, REDISMODULE_AM_STRING, *error);
        } else {
            sdsfree(err);
        }
...
{
    const int allow_mask = (REDISMODULE_CMD_KEY_ACCESS | REDISMODULE_CMD_KEY_INSERT | REDISMODULE_CMD_KEY_DELETE |
...
{
    const int allow_mask = (REDISMODULE_CMD_CHANNEL_PUBLISH | REDISMODULE_CMD_CHANNEL_SUBSCRIBE |
                            REDISMODULE_CMD_CHANNEL_UNSUBSCRIBE | REDISMODULE_CMD_CHANNEL_PATTERN);
...
int moduleGetACLLogEntryReason(RedisModuleACLLogEntryReason reason)
{
    int acl_reason = 0;
    switch (reason) {
    case REDISMODULE_ACL_LOG_AUTH:
        acl_reason = ACL_DENIED_AUTH;
        break;
    case REDISMODULE_ACL_LOG_KEY:
        acl_reason = ACL_DENIED_KEY;
        break;
    case REDISMODULE_ACL_LOG_CHANNEL:
        acl_reason = ACL_DENIED_CHANNEL;
        break;
    case REDISMODULE_ACL_LOG_CMD:
        acl_reason = ACL_DENIED_CMD;
        break;
    default:
        break;
    }
...
int RM_ACLAddLogEntry(RedisModuleCtx *ctx, RedisModuleUser *user, RedisModuleString *object,
 ...
 *
 * For more information about ACL log, please refer to https://redis.io/commands/acl-log */
int RM_ACLAddLogEntryByUserName(RedisModuleCtx *ctx, RedisModuleString *username, RedisModuleString *object,
                                RedisModuleACLLogEntryReason reason)
{
    int acl_reason = moduleGetACLLogEntryReason(reason);
    if (!acl_reason)
...
static int authenticateClientWithUser(RedisModuleCtx *ctx, user *user, RedisModuleUserChangedFunc callback,
                                      void *privdata, uint64_t *client_id)
{
...
int RM_AuthenticateClientWithUser(RedisModuleCtx *ctx, RedisModuleUser *module_user,
 ...
 * See authenticateClientWithUser for information about callback, client_id,
 * and general usage for authentication. */
int RM_AuthenticateClientWithACLUser(RedisModuleCtx *ctx, const char *name, size_t len,
                                     RedisModuleUserChangedFunc callback, void *privdata, uint64_t *client_id)
{
...
RedisModuleString *RM_GetClientCertificate(RedisModuleCtx *ctx, uint64_t client_id)
{
    ...
    if (ctx != NULL)
        autoMemoryAdd(ctx, REDISMODULE_AM_STRING, s);
...
RedisModuleServerInfoData *RM_GetServerInfo(RedisModuleCtx *ctx, const char *section)
{
    ...
    d->rax = raxNew();
    if (ctx != NULL)
        autoMemoryAdd(ctx, REDISMODULE_AM_INFO, d);
...
{
    if (ctx != NULL)
        autoMemoryFreed(ctx, REDISMODULE_AM_INFO, data);
...
void moduleCallCommandFilters(client *c)
{
    ...
    RedisModuleCommandFilterCtx filter = {.argv = c->argv, .argv_len = c->argv_len, .argc = c->argc, .c = c};
...
static void moduleScanKeyCallback(void *privdata, const dictEntry *de, dictEntryLink plink)
{
    ...
    if (kv->type == OBJ_SET) {
        ...
        if ((!(data->key->mode & REDISMODULE_OPEN_KEY_ACCESS_EXPIRED)) && (hfieldIsExpired(key)))
            return;
    ...
    if (ht) {
        ...
        while (p) {
            ...
            if (hfe) {
                ...
                /* Skip expired fields */
                if ((!(key->mode & REDISMODULE_OPEN_KEY_ACCESS_EXPIRED)) && (hashTypeIsExpired(kv, vllExpire)))
                    continue;
            }

            robj *value = (pValue != NULL) ? createStringObject((char *)pValue, lenValue)
                                           : createStringObjectFromLongLongWithSds(vllValue);

            robj *field = (pField != NULL) ? createStringObject((char *)pField, lenField)
...
int TerminateModuleForkChild(int child_pid, int wait)
{
    ...
    if (server.child_type != CHILD_TYPE_MODULE || server.child_pid != child_pid)
        return C_ERR;
...
int RM_SubscribeToServerEvent(RedisModuleCtx *ctx, RedisModuleEvent event, RedisModuleEventCallback callback)
{
    ...
    if (ctx->module == NULL)
        return REDISMODULE_ERR;
    if (event.id >= _REDISMODULE_EVENT_NEXT)
        return REDISMODULE_ERR;
    if (event.dataver > moduleEventVersions[event.id])
        return REDISMODULE_ERR; /* Module compiled with a newer redismodule.h than we support */
...
void processModuleLoadingProgressEvent(int is_aof)
{
    ...
    if (now >= next_event) {
        ...
        RedisModuleLoadingProgressV1 fi = {REDISMODULE_LOADING_PROGRESS_VERSION, server.hz, progress};
        moduleFireServerEvent(
            REDISMODULE_EVENT_LOADING_PROGRESS,
            is_aof ? REDISMODULE_SUBEVENT_LOADING_PROGRESS_AOF : REDISMODULE_SUBEVENT_LOADING_PROGRESS_RDB, &fi);
        /* decide when the next event should fire. */
...
void moduleLoadFromQueue(void)
{
    ...
    if (dictSize(server.module_configs_queue)) {
        ...
        dictResetIterator(&di);
        serverLog(LL_WARNING, "Module Configuration detected without loadmodule directive or no "
                              "ApplyConfig call: aborting");
        exit(1);
    }
}
...
int moduleFreeCommand(struct RedisModule *module, struct redisCommand *cmd)
{
    ...
    if (cmd->subcommands_dict) {
        ...
        while ((de = dictNext(&di)) != NULL) {
            ...
            if (moduleFreeCommand(module, sub) != C_OK)
                continue;
...
void moduleUnregisterCommands(struct RedisModule *module)
{
    ...
    while ((de = dictNext(&di)) != NULL) {
        ...
        if (moduleFreeCommand(module, cmd) != C_OK)
            continue;
...
int parseLoadexArguments(RedisModuleString ***module_argv, int *module_argc)
{
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (!strcasecmp(arg_val, "CONFIG")) {
            ...
            if (!dictReplace(server.module_configs_queue, name, value))
                sdsfree(name);
            i += 2;
 ...
 * C_ERR is returned. */
int moduleOnLoad(int (*onload)(void *, void **, int), const char *path, void *handle, void **module_argv,
                 int module_argc, int is_loadex)
{
    RedisModuleCtx ctx;
    ...
    if (is_loadex && dictSize(server.module_configs_queue)) {
        serverLogRaw(LL_WARNING, "Loadex configurations were not applied, likely due to invalid "
                                 "arguments. Unloading the module.");
        post_load_err = 1;
    }
    ...
    moduleFireServerEvent(REDISMODULE_EVENT_MODULE_CHANGE, REDISMODULE_SUBEVENT_MODULE_LOADED, ctx.module);
...
int moduleUnload(sds name, const char **errmsg, int forced_unload)
{
    ...
    moduleFireServerEvent(REDISMODULE_EVENT_MODULE_CHANGE, REDISMODULE_SUBEVENT_MODULE_UNLOADED, module);
...
{
    if ((flags & ~(REDISMODULE_CONFIG_DEFAULT | REDISMODULE_CONFIG_IMMUTABLE | REDISMODULE_CONFIG_SENSITIVE |
                   REDISMODULE_CONFIG_HIDDEN | REDISMODULE_CONFIG_PROTECTED | REDISMODULE_CONFIG_DENY_LOADING |
                   REDISMODULE_CONFIG_BITFLAGS | REDISMODULE_CONFIG_MEMORY | REDISMODULE_CONFIG_UNPREFIXED))) {
    ...
    for (size_t i = 0; nameAlias[i] != '\0'; i++) {
        ...
        if (((*delim) == NULL) && (ch == '|')) {
            ...
            *delim = &nameAlias[i];
            ...
            if (++dot_count > 1) {
                serverLog(LL_WARNING, "Invalid character sequence in Module configuration name or alias: %s",
                          nameAlias);
                return REDISMODULE_ERR;
            }
        } else {
...
int loadModuleDefaultConfigs(RedisModule *module)
{
    ...
    while ((ln = listNext(&li))) {
        ...
        if (!performModuleConfigSetDefaultFromName(module_config->name, &err)) {
            serverLog(LL_WARNING, "Issue attempting to set default value of configuration %s : %s", module_config->name,
                      err);
            return REDISMODULE_ERR;
        }
    }
...
{
    if (module_config->apply_fn == NULL)
        return 1;
...
int moduleConfigApplyConfig(list *module_configs, const char **err, const char **err_arg_name)
{
    ...
    while ((ln = listNext(&li))) {
        ...
        if (!moduleConfigApplyInternal(module_config, err)) {
            if (err_arg_name)
                *err_arg_name = module_config->name;
            return 0;
        }
    }
    return 1;
...
ModuleConfig *createModuleConfig(const char *name, RedisModuleConfigApplyFunc apply_fn, void *privdata,
                                 RedisModule *module, unsigned int flags)
{
...
int moduleConfigValidityCheck(RedisModule *module, const char *name, unsigned int flags, configType type)
{
    ...
    if (flags & REDISMODULE_CONFIG_UNPREFIXED) {
        ...
        if (delim) {
            ...
            isdup = configExists(ar[0]) || configExists(ar[1]) || (sdscmp(ar[0], ar[1]) == 0);
...
unsigned int maskModuleConfigFlags(unsigned int flags)
{
    ...
    if (flags & REDISMODULE_CONFIG_DEFAULT)
        new_flags |= MODIFIABLE_CONFIG;
    if (flags & REDISMODULE_CONFIG_IMMUTABLE)
        new_flags |= IMMUTABLE_CONFIG;
    if (flags & REDISMODULE_CONFIG_HIDDEN)
        new_flags |= HIDDEN_CONFIG;
    if (flags & REDISMODULE_CONFIG_PROTECTED)
        new_flags |= PROTECTED_CONFIG;
    if (flags & REDISMODULE_CONFIG_DENY_LOADING)
        new_flags |= DENY_LOADING_CONFIG;
    return new_flags;
}
...
{
    unsigned int new_flags = 0;
    if (flags & REDISMODULE_CONFIG_MEMORY)
        new_flags |= MEMORY_CONFIG;
    return new_flags;
}
...
int RM_RegisterStringConfig(RedisModuleCtx *ctx, const char *name, const char *default_val, unsigned int flags,
                            ...
                            RedisModuleConfigApplyFunc applyfn, void *privdata)
{
    ...
    addModuleStringConfig(sdsdup(mc->name), (mc->alias) ? sdsdup(mc->alias) : NULL, cflags, mc,
                          default_val ? sdsnew(default_val) : NULL);
    return REDISMODULE_OK;
}
...
int RM_RegisterBoolConfig(RedisModuleCtx *ctx, const char *name, int default_val, unsigned int flags,
                          ...
                          RedisModuleConfigApplyFunc applyfn, void *privdata)
{
    ...
    listAddNodeTail(module->module_configs, mc);
    unsigned int cflags = maskModuleConfigFlags(flags);
    addModuleBoolConfig(sdsdup(mc->name), (mc->alias) ? sdsdup(mc->alias) : NULL, cflags, mc, default_val);
    return REDISMODULE_OK;
}
...
int RM_RegisterEnumConfig(RedisModuleCtx *ctx, const char *name, int default_val, unsigned int flags,
                          ...
                          RedisModuleConfigApplyFunc applyfn, void *privdata)
{
    ...
    addModuleEnumConfig(sdsdup(mc->name), (mc->alias) ? sdsdup(mc->alias) : NULL, cflags, mc, default_val, enum_vals,
                        num_enum_vals);
    return REDISMODULE_OK;
}
 ...
 * `CONFIG SET`, `CONFIG GET`, and `CONFIG REWRITE` commands. See
 * RedisModule_RegisterStringConfig for detailed information about configs. */
int RM_RegisterNumericConfig(RedisModuleCtx *ctx, const char *name, long long default_val, unsigned int flags,
                             ...
                             RedisModuleConfigSetNumericFunc setfn, RedisModuleConfigApplyFunc applyfn, void *privdata)
{
    ...
    unsigned int cflags = maskModuleConfigFlags(flags);
    addModuleNumericConfig(sdsdup(mc->name), (mc->alias) ? sdsdup(mc->alias) : NULL, cflags, mc, default_val,
                           numeric_flags, min, max);
    return REDISMODULE_OK;
}
...
RedisModuleConfigIterator *RM_ConfigIteratorCreate(RedisModuleCtx *ctx, const char *pattern)
{
    ...
    if (ctx != NULL)
        autoMemoryAdd(ctx, REDISMODULE_AM_CONFIG, iter);
    return iter;
}
...
{
    if (ctx != NULL)
        autoMemoryFreed(ctx, REDISMODULE_AM_CONFIG, iter);
    if (iter->di)
...
void moduleCommand(client *c)
{
    ...
    if (c->argc == 2 && !strcasecmp(subcmd, "help")) {
        const char *help[] = {"LIST",
                              ...
                              "    Load a module library from <path>, while passing it module configurations and "
                              "optional arguments.",
                              "UNLOAD <name>",
                              "    Unload a module.",
                              NULL};
...
RedisModuleDict *RM_DefragRedisModuleDict(RedisModuleDefragCtx *ctx, RedisModuleDict *dict,
                                          RedisModuleDefragDictValueCallback valueCB, RedisModuleString **seekTo)
{
    ...
    while (raxNext(&ri)) {
        ...
        if (ret == 1 || RM_DefragShouldStop(ctx)) {
            if (ret == 0 && !raxNext(&ri))
                goto cleanup; /* Last node and no more work needed. */
            if (*seekTo)
...
{
    dictForEach(
        modules, struct RedisModule, module, if (module->defrag_start_cb) {
...
{
    dictForEach(
        modules, struct RedisModule, module, if (module->defrag_end_cb) {
            RedisModuleDefragCtx defrag_ctx = INIT_MODULE_DEFRAG_CTX(0, NULL, NULL, -1);
