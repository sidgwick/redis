...
void clusterCommandShards(client *c)
{
    ...
    for (void *shard_handle = clusterNextShardHandle(shard_it); shard_handle != NULL;
         shard_handle = clusterNextShardHandle(shard_it)) {
...
{
    const char *help[] = {"COUNTKEYSINSLOT <slot>",
                          ...
                          "    <id> <ip:port@bus-port[,hostname]> <flags> <master> <pings> <pongs> <epoch> <link> "
                          "<slot> ...",
                          "REPLICAS <node-id>",
    ...
    if (c->flags & CLIENT_BLOCKED && (c->bstate.btype == BLOCKED_LIST || c->bstate.btype == BLOCKED_ZSET ||
        ...
        if ((de = dictNext(&di)) != NULL) {
            ...
            if ((c->flags & CLIENT_READONLY) && !(c->lastcmd->flags & CMD_WRITE) && clusterNodeIsSlave(myself) &&
                clusterNodeGetSlaveof(myself) == node) {
                node = myself;
            }
...
void sflushCommand(client *c)
{
    ...
    if (inSlotRange)
        sflush->ranges[sflush->numRanges++].last = CLUSTER_SLOTS - 1;
