 ...
 *    +-----------+------------------+------------------------+----------------------------+
 */
static kvobj *kvobjCreateEmbedString(const char *val_ptr, size_t val_len, const sds key, int hasExpire)
...
void decrRefCount(robj *o)
{
    ...
    if (unlikely(o->refcount <= 0)) {
        serverPanic("illegal decrRefCount for object with: type %u, encoding %u, refcount %d", o->type, o->encoding,
                    o->refcount);
    }
...
size_t getObjectLength(robj *o)
{
    ...
    case OBJ_ZSET:
        return zsetLength(o);
    case OBJ_HASH:
        return hashTypeLength(o, 0);
    case OBJ_STREAM:
        return streamLength(o);
    default:
        return 0;
    }
}
...
int equalStringObjects(robj *a, robj *b)
{
    if (a->encoding == OBJ_ENCODING_INT && b->encoding == OBJ_ENCODING_INT) {
        ...
        if (sdsEncodedObject(a) && sdsEncodedObject(b) && sdslen(a->ptr) != sdslen(b->ptr)) {
            return 0;
        }
...
struct redisMemOverhead *getMemoryOverheadData(void)
{
    ...
    size_t frag_smallbins_bytes = server.cron_malloc_stats.allocator_frag_smallbins_bytes -
                                  server.cron_malloc_stats.lua_allocator_frag_smallbins_bytes;
    size_t allocated = server.cron_malloc_stats.allocator_allocated - server.cron_malloc_stats.lua_allocator_allocated;
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        if (!kvstoreNumAllocatedDicts(db->keys))
            continue;
...
sds getMemoryDoctorReport(void)
{
    ...
    if (num_reports == 0) {
        ...
        if (big_peak) {
            s = sdscat(s, " * Peak memory: In the past this instance used more than 150% the memory "
                          "that is currently using. The allocator is normally not able to release "
                          "memory after a peak, so you can expect to see a big fragmentation ratio, "
                          "however this is actually harmless and is only due to the memory peak, and "
                          "if the Redis instance Resident Set Size (RSS) is currently bigger than "
                          "expected, the memory will be used as soon as you fill the Redis instance "
                          "with more data. If the memory peak was only occasional and you want to try "
                          "to reclaim memory, please try the MEMORY PURGE command, otherwise the only "
                          "other option is to shutdown and restart the instance.\n\n");
        }
        if (high_frag) {
            s = sdscatprintf(s,
                             " * High total RSS: This instance has a memory fragmentation and RSS overhead "
                             "greater than 1.4 (this means that the Resident Set Size of the Redis process is "
                             "much larger than the sum of the logical allocations Redis performed). This "
                             "problem is usually due either to a large peak memory (check if there is a peak "
                             "memory entry above in the report) or may result from a workload that causes the "
                             "allocator to fragment memory a lot. If the problem is a large peak memory, then "
                             "there is no issue. Otherwise, make sure you are using the Jemalloc allocator and "
                             "not the default libc malloc. Note: The currently used allocator is \"%s\".\n\n",
                             ZMALLOC_LIB);
        }
        if (high_alloc_frag) {
            s = sdscatprintf(s, " * High allocator fragmentation: This instance has an allocator external "
                                "fragmentation greater than 1.1. This problem is usually due either to a large "
                                "peak memory (check if there is a peak memory entry above in the report) or may "
                                "result from a workload that causes the allocator to fragment memory a lot. You "
                                "can try enabling 'activedefrag' config option.\n\n");
        }
        if (high_alloc_rss) {
            s = sdscatprintf(s, " * High allocator RSS overhead: This instance has an RSS memory overhead is "
                                "greater than 1.1 (this means that the Resident Set Size of the allocator is "
                                "much larger than the sum what the allocator actually holds). This problem is "
                                "usually due to a large peak memory (check if there is a peak memory entry "
                                "above in the report), you can try the MEMORY PURGE command to reclaim it.\n\n");
        }
        if (high_proc_rss) {
            s = sdscatprintf(s, " * High process RSS overhead: This instance has non-allocator RSS memory "
                                "overhead is greater than 1.1 (this means that the Resident Set Size of the "
                                "Redis process is much larger than the RSS the allocator holds). This problem "
                                "may be due to Lua scripts or Modules.\n\n");
        }
        if (big_slave_buf) {
            s = sdscat(s, " * Big replica buffers: The replica output buffers in this instance are greater "
                          "than 10MB for each replica (on average). This likely means that there is some "
                          "replica instance that is struggling receiving data, either because it is too slow "
                          "or because of networking issues. As a result, data piles on the master output "
                          "buffers. Please try to identify what replica is not receiving data correctly and "
                          "why. You can use the INFO output in order to check the replicas delays and the "
                          "CLIENT LIST command to check the output buffers of each replica.\n\n");
        }
        if (big_client_buf) {
            s = sdscat(s, " * Big client buffers: The clients output buffers in this instance are greater "
                          "than 200K per client (on average). This may result from different causes, like "
                          "Pub/Sub clients subscribed to channels bot not receiving data fast enough, so "
                          "that data piles on the Redis instance output buffer, or clients sending "
                          "commands with large replies or very large sequences of commands in the same "
                          "pipeline. Please use the CLIENT LIST command in order to investigate the issue "
                          "if it causes problems in your instance, or to understand better why certain "
                          "clients are using a big amount of memory.\n\n");
        }
        if (many_scripts) {
            s = sdscat(s, " * Many scripts: There seem to be many cached scripts in this instance "
                          "(more than 1000). This may be because scripts are generated and "
                          "`EVAL`ed, instead of being parameterized (with KEYS and ARGV), `SCRIPT "
                          "LOAD`ed and `EVALSHA`ed. Unless `SCRIPT FLUSH` is called periodically, "
                          "the scripts' caches may end up consuming most of your memory.\n\n");
        }
...
void objectCommand(client *c)
{
    ...
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
        ...
        if ((kv = kvobjCommandLookupOrReply(c, c->argv[2], shared.null[c->resp])) == NULL)
            return;
        ...
        if ((kv = kvobjCommandLookupOrReply(c, c->argv[2], shared.null[c->resp])) == NULL)
            return;
        if (!(server.maxmemory_policy & MAXMEMORY_FLAG_LFU)) {
            addReplyError(c, "An LFU maxmemory policy is not selected, access frequency not "
