...
int clusterLoadConfig(char *filename)
{
    ...
    if (server.cluster->myself == NULL)
        goto fmterr;
    if (!(myself->flags & (CLUSTER_NODE_MASTER | CLUSTER_NODE_SLAVE)))
        goto fmterr;
    if (nodeIsSlave(myself) && myself->slaveof == NULL)
        goto fmterr;
...
void clearNodeFailureIfNeeded(clusterNode *node)
{
    ...
    if (clusterNodeIsMaster(node) && node->numslots > 0 &&
        ...
        serverLog(LL_NOTICE,
                  "Clear FAIL state for node %.40s (%s): is reachable again and nobody is serving "
...
uint32_t writePingExt(clusterMsg *hdr, int gossipcount)
{
    ...
    if (cursor != NULL) {
        clusterMsgPingExtInternalSecret *ext =
            preparePingExt(cursor, CLUSTERMSG_EXT_TYPE_INTERNALSECRET, getInternalSecretPingExtSize());
...
void clusterProcessPingExtensions(clusterMsg *hdr, clusterLink *link)
{
    ...
    while (extensions--) {
        ...
        if (type == CLUSTERMSG_EXT_TYPE_HOSTNAME) {
            ...
            clusterMsgPingExtInternalSecret *internal_secret_ext =
                (clusterMsgPingExtInternalSecret *)&(ext->ext[0].internal_secret);
            if (memcmp(server.cluster->internal_secret, internal_secret_ext->internal_secret,
                       CLUSTER_INTERNALSECRETLEN) > 0) {
                memcpy(server.cluster->internal_secret, internal_secret_ext->internal_secret,
                       CLUSTER_INTERNALSECRETLEN);
            }
        } else {
    ...
    if (type == CLUSTERMSG_TYPE_PING || type == CLUSTERMSG_TYPE_PONG || type == CLUSTERMSG_TYPE_MEET) {
        ...
        if (!link->inbound) {
            if (nodeInHandshake(link->node)) {
                 ...
                 * address. */
                serverLog(LL_DEBUG,
                          "PONG contains mismatching sender ID. About node %.40s added %d ms ago, "
                          "having flags %d",
                    ...
                    if (master && !memcmp(master->shard_id, sender->shard_id, CLUSTER_NAMELEN)) {
                        ...
                        if (sender->configEpoch > senderConfigEpoch) {
                            ...
                            serverLog(LL_NOTICE,
                                      ...
                                      sender->shard_id, sender->name, sender->human_nodename, slots, master->name,
...
void clusterRequestFailoverAuth(void)
{
    ...
    if (server.cluster->mf_end)
        msgblock->msg[0].mflags[0] |= CLUSTERMSG_FLAG0_FORCEACK;
    clusterBroadcastMessage(msgblock);
    clusterMsgSendBlockDecrRefCount(msgblock);
}
...
void clusterFailoverReplaceYourMaster(void)
{
    ...
    if (clusterNodeIsMaster(myself) || oldmaster == NULL)
        return;
...
int verifyClusterConfigWithData(void)
{
    ...
    for (j = 1; j < server.dbnum; j++) {
        if (kvstoreSize(server.db[j].keys))
            return C_ERR;
    }
...
{
    if (!kvstoreSize(server.pubsubshard_channels))
        return;
...
const char **clusterCommandExtendedHelp(void)
{
    static const char *help[] = {
        ...
        "    Delete slots information which are between <start-slot> and <end-slot> from current "
        "node.",
        ...
        "    Output format is an array where each array element is a map containing attributes of "
        "a link",
        NULL};
