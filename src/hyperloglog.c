...
int hllSparseSet(robj *o, long index, uint8_t count)
{
    ...
    if (deltalen > 0 && sdslen(o->ptr) + deltalen > server.hll_sparse_max_bytes)
        goto promote;
...
int isHLLObjectOrReply(client *c, robj *o)
{
    ...
    if (hdr->encoding == HLL_DENSE && stringObjectLen(o) != HLL_DENSE_SIZE)
        goto invalid;
...
void pfaddCommand(client *c)
{
    ...
    if (kv == NULL) {
        ...
        if (isHLLObjectOrReply(c, kv) != C_OK)
            return;
    ...
    for (j = 2; j < c->argc; j++) {
        int retval = hllAdd(kv, (unsigned char *)c->argv[j]->ptr, sdslen(c->argv[j]->ptr));
        switch (retval) {
        case 1:
            updated++;
            break;
