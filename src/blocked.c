...
void blockClient(client *c, int btype)
{
    ...
    serverAssert(!(c->flags & CLIENT_MASTER && btype != BLOCKED_MODULE && btype != BLOCKED_LAZYFREE &&
...
int blockedClientMayTimeout(client *c)
{
    ...
    if (c->bstate.btype == BLOCKED_LIST || c->bstate.btype == BLOCKED_ZSET || c->bstate.btype == BLOCKED_STREAM ||
        c->bstate.btype == BLOCKED_WAIT || c->bstate.btype == BLOCKED_WAITAOF) {
        return 1;
    }
    return 0;
}
...
void replyToBlockedClientTimedOut(client *c)
{
    ...
    } else if (c->bstate.btype == BLOCKED_LIST || c->bstate.btype == BLOCKED_ZSET ||
