...
void putClientInPendingWriteQueue(client *c)
{
    ...
    if (!(c->flags & CLIENT_PENDING_WRITE) &&
        (c->replstate == REPL_STATE_NONE || c->replstate == SLAVE_STATE_SEND_BULK_AND_STREAM ||
...
static inline int _prepareClientToWrite(client *c)
{
    ...
    if (unlikely(_flags & (CLIENT_SCRIPT | CLIENT_MODULE)))
        return C_OK;

    /* If CLIENT_CLOSE_ASAP flag is set, we need not write anything. */
    if (unlikely(_flags & CLIENT_CLOSE_ASAP))
        return C_ERR;

    /* CLIENT REPLY OFF / SKIP handling: don't send replies.
     * CLIENT_PUSHING handling: disables the reply silencing flags. */
    if (unlikely((_flags & (CLIENT_REPLY_OFF | CLIENT_REPLY_SKIP)) && !(_flags & CLIENT_PUSHING)))
        return C_ERR;

    /* Masters don't receive replies, unless CLIENT_MASTER_FORCE_REPLY flag
     * is set. */
    if (unlikely((_flags & CLIENT_MASTER) && !(_flags & CLIENT_MASTER_FORCE_REPLY)))
        return C_ERR;
...
void addExtendedReplyHelp(client *c, const char **help, const char **extended_help)
{
    ...
    if (extended_help) {
        while (extended_help[idx])
            addReplyStatus(c, extended_help[idx++]);
    }
    blen += idx;
...
void unlinkClient(client *c)
{
    ...
    if (c->conn && server.current_client == c)
...
void freeClient(client *c)
{
    ...
    if (clientTypeIsSlave(c)) {
        ...
        serverLog(LL_NOTICE, "Connection with replica%s %s lost.", type, replicationGetSlaveName(c));
    }
...
{
    if (!(c->io_flags & CLIENT_IO_WRITE_ENABLED))
        return C_OK;
    ...
    if (unlikely(is_slave)) {
        ...
        while (_clientHasPendingRepliesNonSlave(c)) {
            ...
            if (totwritten > NET_MAX_WRITES_PER_EVENT &&
                (server.maxmemory == 0 || zmalloc_used_memory() < server.maxmemory) && is_normal_client)
                break;
        }
    ...
    while (c->multibulklen) {
        ...
        if (c->bulklen == -1) {
            ...
            if (!(c->flags & CLIENT_MASTER) && ll >= PROTO_MBULK_BIG_ARG) {
                ...
                if (querybuf_len - c->qb_pos <= (size_t)ll + 2) {
                    ...
                    if (c->querybuf_peak < (size_t)ll + 2)
                        c->querybuf_peak = ll + 2;
        ...
        if (querybuf_len - c->qb_pos < (size_t)(c->bulklen + 2)) {
            ...
            if (c->argc >= c->argv_len) {
                serverAssert(
                    c->argv_len); /* Ensure argv is not freed while the client is in the mid of parsing command. */
...
void handleClientReadError(client *c)
{
    ...
    case CLIENT_READ_MASTER_USING_INLINE_PROTOCAL:
        serverLog(LL_WARNING, "WARNING: Receiving inline protocol from master, master stream corruption? "
                              "Closing the master connection and discarding the cached master.");
...
sds catClientInfoString(sds s, client *client)
{
    ...
    int paused = 0;
    if (client->running_tid != IOTHREAD_MAIN_THREAD_ID && pthread_equal(server.main_thread_id, pthread_self()) &&
        !isCrashing()) {
        paused = 1;
    ...
    if (client->flags & CLIENT_NO_EVICT)
        *p++ = 'e';
    if (client->flags & CLIENT_NO_TOUCH)
        *p++ = 'T';
    if (client->flags & CLIENT_REPL_RDB_CHANNEL)
        *p++ = 'C';
    if (client->flags & CLIENT_INTERNAL)
        *p++ = 'I';
    if (p == flags)
        *p++ = 'N';
    *p++ = '\0';
    ...
    *p = '\0';
    ...
    sds ret = sdscatfmt(
        s,
        FMTARGS(
            "id=%U", (unsigned long long)client->id, " addr=%s", getClientPeerId(client), " laddr=%s",
            getClientSockname(client), " %s", connGetInfo(client->conn, conninfo, sizeof(conninfo)), " name=%s",
            client->name ? (char *)client->name->ptr : "", " age=%I",
            (long long)(commandTimeSnapshot() / 1000 - client->ctime), " idle=%I",
            (long long)(server.unixtime - client->lastinteraction), " flags=%s", flags, " db=%i", client->db->id,
            " sub=%i", (int)dictSize(client->pubsub_channels), " psub=%i", (int)dictSize(client->pubsub_patterns),
            " ssub=%i", (int)dictSize(client->pubsubshard_channels), " multi=%i",
            (client->flags & CLIENT_MULTI) ? client->mstate.count : -1, " watch=%i",
            (int)listLength(client->watched_keys), " qbuf=%U",
            client->querybuf ? (unsigned long long)sdslen(client->querybuf) : 0, " qbuf-free=%U",
            client->querybuf ? (unsigned long long)sdsavail(client->querybuf) : 0, " argv-mem=%U",
            (unsigned long long)client->argv_len_sum, " multi-mem=%U", (unsigned long long)client->mstate.argv_len_sums,
            " rbs=%U", (unsigned long long)client->buf_usable_size, " rbp=%U", (unsigned long long)client->buf_peak,
            " obl=%U", (unsigned long long)client->bufpos, " oll=%U",
            (unsigned long long)listLength(client->reply) + used_blocks_of_repl_buf, " omem=%U",
            (unsigned long long)obufmem, /* should not include client->buf since we want to see 0 for static clients. */
            " tot-mem=%U", (unsigned long long)total_mem, " events=%s", events, " cmd=%s",
            client->lastcmd ? client->lastcmd->fullname : "NULL", " user=%s",
            client->user ? client->user->name : "(superuser)", " redir=%I",
            (client->flags & CLIENT_TRACKING) ? (long long)client->client_tracking_redirection : -1, " resp=%i",
            client->resp, " lib-name=%s", client->lib_name ? (char *)client->lib_name->ptr : "", " lib-ver=%s",
            client->lib_ver ? (char *)client->lib_ver->ptr : "", " io-thread=%i", client->tid, " tot-net-in=%U",
            client->net_input_bytes, " tot-net-out=%U", client->net_output_bytes, " tot-cmds=%U",
            client->commands_processed));

    if (paused)
        resumeIOThread(client->running_tid);
    return ret;
}
...
sds getAllClientsInfoString(int type)
{
    ...
    int allpaused = 0;
    if (server.io_threads_num > 1 && !isCrashing() && pthread_equal(server.main_thread_id, pthread_self())) {
        allpaused = 1;
        pauseAllIOThreads();
    }
...
void clientCommand(client *c)
{
    ...
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
        const char *help[] = {"CACHING (YES|NO)",
        ...
        while ((ln = listNext(&li)) != NULL) {
            ...
            if (c == client && skipme)
                continue;
            if (max_age != 0 && (long long)(commandTimeSnapshot() / 1000 - client->ctime) < max_age)
                continue;
...
void securityWarningCommand(client *c)
{
    ...
    if (llabs(now - logged_time) > 60) {
        ...
        if (connAddrPeerName(c->conn, ip, sizeof(ip), &port) == -1) {
            serverLog(LL_WARNING, "Possible SECURITY ATTACK detected. It looks like somebody is sending POST or "
                                  "Host: commands to Redis. This is likely due to an attacker attempting to use "
                                  "Cross Protocol Scripting to compromise your Redis instance. Connection aborted.");
        } else {
            serverLog(LL_WARNING,
                      "Possible SECURITY ATTACK detected. It looks like somebody is sending POST "
                      "or Host: commands to Redis. This is likely due to an attacker attempting to "
                      "use Cross Protocol Scripting to compromise your Redis instance. Connection "
                      "from %s:%d aborted.",
                      ip, port);
        }
        logged_time = now;
...
int closeClientOnOutputBufferLimitReached(client *c, int async)
{
    ...
    if ((c->reply_bytes == 0 && !clientTypeIsSlave(c)) || c->flags & CLIENT_CLOSE_ASAP)
        return 0;
...
void flushSlavesOutputBuffers(void)
{
    ...
    while ((ln = listNext(&li))) {
        ...
        if ((slave->replstate == SLAVE_STATE_ONLINE || slave->replstate == SLAVE_STATE_SEND_BULK_AND_STREAM) &&
            !(slave->flags & CLIENT_CLOSE_ASAP) && can_receive_writes && !slave->repl_start_cmd_stream_on_ack &&
