...
void rdbReportError(int corruption_error, int linenum, char *reason, ...)
{
    ...
    if (isRestoreContext()) {
        ...
        serverLog(LL_WARNING,
                  "%s. Failure loading rdb format from socket, assuming connection error, resuming "
                  "operation.",
                  msg);
        return;
    }
...
ssize_t rdbSaveObject(rio *rdb, robj *o, robj *key, int dbid)
{
    ...
    if (o->type == OBJ_STRING) {
        ...
        if (o->encoding == OBJ_ENCODING_QUICKLIST) {
            ...
            while (node) {
                if ((n = rdbSaveLen(rdb, node->container)) == -1)
                    return -1;
                nwritten += n;
                ...
                if (quicklistNodeIsCompressed(node)) {
                    ...
                    if ((n = rdbSaveRawString(rdb, node->entry, node->sz)) == -1)
                        return -1;
                    nwritten += n;
                }
                node = node->next;
            ...
            if ((n = rdbSaveLen(rdb, 1)) == -1)
                return -1;
            nwritten += n;
            if ((n = rdbSaveLen(rdb, QUICKLIST_NODE_CONTAINER_PACKED)) == -1)
                return -1;
            nwritten += n;
        ...
        if ((o->encoding == OBJ_ENCODING_LISTPACK) || (o->encoding == OBJ_ENCODING_LISTPACK_EX)) {
            ...
            if ((n = rdbSaveRawString(rdb, lp_ptr, l)) == -1)
                return -1;
            nwritten += n;
        } else if (o->encoding == OBJ_ENCODING_HT) {
            ...
            while ((de = dictNext(&di)) != NULL) {
                ...
                /* save the key */
                if ((n = rdbSaveRawString(rdb, (unsigned char *)field, hfieldlen(field))) == -1) {
                    dictResetIterator(&di);
                    return -1;
                }
                nwritten += n;
        ...
        if ((n = rdbSaveLen(rdb, s->first_id.ms)) == -1)
            return -1;
        nwritten += n;
        if ((n = rdbSaveLen(rdb, s->first_id.seq)) == -1)
            return -1;
        nwritten += n;
        /* Save the maximal tombstone ID. */
        if ((n = rdbSaveLen(rdb, s->max_deleted_entry_id.ms)) == -1)
            return -1;
        nwritten += n;
        if ((n = rdbSaveLen(rdb, s->max_deleted_entry_id.seq)) == -1)
            return -1;
        nwritten += n;
        /* Save the offset. */
        if ((n = rdbSaveLen(rdb, s->entries_added)) == -1)
            return -1;
        nwritten += n;
...
int rdbSaveKeyValuePair(rio *rdb, robj *key, robj *val, long long expiretime, int dbid)
{
    ...
    if (rdbSaveStringObject(rdb, key) == -1)
        return -1;
    if (rdbSaveObject(rdb, val, key, dbid) == -1)
        return -1;
...
int rdbSaveInfoAuxFields(rio *rdb, int rdbflags, rdbSaveInfo *rsi)
{
    ...
    if (rdbSaveAuxFieldStrInt(rdb, "aof-base", aof_base) == -1)
        return -1;
    return 1;
}
...
ssize_t rdbSaveSingleModuleAux(rio *rdb, int when, moduleType *mt)
{
    ...
    if (rdbSaveType(&aux_save_headers_rio, RDB_OPCODE_MODULE_AUX) == -1)
        goto error;

    /* Write the "module" identifier as prefix, so that we'll be able
     * to call the right module during loading. */
    if (rdbSaveLen(&aux_save_headers_rio, mt->id) == -1)
        goto error;

    /* write the 'when' so that we can provide it on loading. add a UINT opcode
     * for backwards compatibility, everything after the MT needs to be prefixed
     * by an opcode. */
    if (rdbSaveLen(&aux_save_headers_rio, RDB_MODULE_OPCODE_UINT) == -1)
        goto error;
    if (rdbSaveLen(&aux_save_headers_rio, when) == -1)
        goto error;
...
ssize_t rdbSaveFunctions(rio *rdb)
{
    ...
    while ((entry = dictNext(&iter))) {
        ...
        if ((ret = rdbSaveRawString(rdb, (unsigned char *)li->code, sdslen(li->code))) < 0)
            goto werr;
        written += ret;
    }
...
ssize_t rdbSaveDb(rio *rdb, int dbid, int rdbflags, long *key_counter)
{
    ...
    while ((de = kvstoreIteratorNext(kvs_it)) != NULL) {
        ...
        if (server.cluster_enabled && curr_slot != last_slot) {
            if ((res = rdbSaveType(rdb, RDB_OPCODE_SLOT_INFO)) < 0)
                goto werr;
            written += res;
            if ((res = rdbSaveLen(rdb, curr_slot)) < 0)
                goto werr;
            written += res;
            if ((res = rdbSaveLen(rdb, kvstoreDictSize(db->keys, curr_slot))) < 0)
                goto werr;
            written += res;
            if ((res = rdbSaveLen(rdb, kvstoreDictSize(db->expires, curr_slot))) < 0)
                goto werr;
            written += res;
            last_slot = curr_slot;
        }
        ...
        if ((res = rdbSaveKeyValuePair(rdb, &key, kv, expire, dbid)) < 0)
            goto werr;
        written += res;
...
int rdbSaveRioWithEOFMark(int req, rio *rdb, int *error, rdbSaveInfo *rsi)
{
    ...
    if (rioWrite(rdb, "\r\n", 2) == 0)
        goto werr;
    if (rdbSaveRio(req, rdb, error, RDBFLAGS_REPLICATION, rsi) == C_ERR)
        goto werr;
    if (rioWrite(rdb, eofmark, RDB_EOF_MARK_SIZE) == 0)
        goto werr;
    stopSaving(1);
...
static int _lpEntryValidation(unsigned char *p, unsigned int head_count, void *userdata)
{
    ...
    if (data->count % data->tuple_len == 2) {
        long long expire_at;
        /* Must be an integer. */
        if (!lpGetIntegerValue(p, &expire_at))
            return 0;
        /* Must be less than EB_EXPIRE_TIME_MAX. */
        if (expire_at < 0 || (unsigned long long)expire_at > EB_EXPIRE_TIME_MAX)
            return 0;
        ...
        if (expire_at != 0 && (data->last_expireat == 0 || expire_at < data->last_expireat))
            return 0;
...
robj *rdbLoadObject(int rdbtype, rio *rdb, sds key, int dbid, int *error)
{
    ...
    if (rdbtype == RDB_TYPE_STRING) {
        ...
        while (o->encoding == OBJ_ENCODING_LISTPACK && len > 0) {
            ...
            if (hfieldlen(field) > server.hash_max_listpack_value || sdslen(value) > server.hash_max_listpack_value ||
        ...
        while (len > 0) {
            ...
            if (o->encoding == OBJ_ENCODING_LISTPACK_EX) {
                ...
                if (hfieldlen(field) > server.hash_max_listpack_value ||
                    ...
                    !lpSafeToAdd(((listpackEx *)o->ptr)->lp,
                                 hfieldlen(field) + sdslen(value) + lpEntrySizeInteger(expireAt))) {
                    /* convert to hash */
                    hashTypeConvert(o, OBJ_ENCODING_HT, NULL);

                    if (len >
                        DICT_HT_INITIAL_SIZE) { /* TODO: this is NOT the original len, but this is also the case for simple hash, is this a bug? */
                        if (dictTryExpand(o->ptr, len) != DICT_OK) {
                            rdbReportCorruptRDB("OOM in dictTryExpand %llu", (unsigned long long)len);
                            decrRefCount(o);
                            if (dupSearchDict != NULL)
                    ...
                    listpackExAddNew(o, field, hfieldlen(field), value, sdslen(value), expireAt);
                    hfieldFree(field);
                    sdsfree(value);
                }
            }
        ...
        listTypeTryConversion(o, LIST_CONV_AUTO, NULL, NULL);
               ...
               rdbtype == RDB_TYPE_HASH_LISTPACK || rdbtype == RDB_TYPE_HASH_LISTPACK_EX_PRE_GA ||
        ...
        case RDB_TYPE_HASH_LISTPACK_EX:
            /* listpack-encoded hash with TTL requires its own struct
                 * pointed to by o->ptr */
            o->type = OBJ_HASH;
            if ((rdbtype == RDB_TYPE_HASH_LISTPACK_EX) || (rdbtype == RDB_TYPE_HASH_LISTPACK_EX_PRE_GA)) {
            ...
            if (deep_integrity_validation)
                server.stat_dump_payload_sanitizations++;
            if (!lpValidateIntegrityAndDups(encoded, encoded_len, deep_integrity_validation, tuple_len)) {
...
void stopLoading(int success)
{
    ...
    moduleFireServerEvent(REDISMODULE_EVENT_LOADING,
                          success ? REDISMODULE_SUBEVENT_LOADING_ENDED : REDISMODULE_SUBEVENT_LOADING_FAILED, NULL);
}
        ...
        if (type == RDB_OPCODE_EXPIRETIME) {
            ...
            if (!rdbCheckMode && mt == NULL) {
                ...
                if (!mt->aux_load) {
                    ...
                    serverLog(LL_WARNING,
                              "The RDB file contains module AUX data, but the module '%s' doesn't "
                              "seem to support it.",
                              name);
                    exit(1);
                }
                ...
                if (rc != REDISMODULE_OK || io.error) {
                    moduleTypeNameByID(name, moduleid);
                    serverLog(LL_WARNING,
                              "The RDB file contains module AUX data for the module type '%s', "
                              "that the responsible module is not able to load. Check for modules "
                              "log above for additional clues.",
                              name);
                    goto eoferr;
                }
                uint64_t eof = rdbLoadLen(rdb, NULL);
                if (eof != RDB_MODULE_OPCODE_EOF) {
                    serverLog(LL_WARNING,
                              "The RDB file contains module AUX data for the module '%s' that is "
                              "not terminated by the proper module value EOF marker",
                              name);
                    goto eoferr;
                }
                continue;
