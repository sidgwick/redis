...
static inline uint32_t lpCurrentEncodedSizeBytes(const unsigned char encoding)
{
    ...
    if (LP_ENCODING_IS_6BIT_STR(encoding))
        return 1;
    if (LP_ENCODING_IS_13BIT_INT(encoding))
        return 1;
    if (LP_ENCODING_IS_16BIT_INT(encoding))
        return 1;
    if (LP_ENCODING_IS_24BIT_INT(encoding))
        return 1;
    if (LP_ENCODING_IS_32BIT_INT(encoding))
        return 1;
    if (LP_ENCODING_IS_64BIT_INT(encoding))
        return 1;
...
static inline unsigned char *lpGetWithSize(unsigned char *p, int64_t *count, unsigned char *intbuf,
                                           uint64_t *entry_size)
{
    int64_t val;
    ...
    if (LP_ENCODING_IS_7BIT_UINT(p[0])) {
        ...
        *count = LP_ENCODING_12BIT_STR_LEN(p);
        if (entry_size)
            *entry_size = 2 + *count + lpEncodeBacklenBytes(*count + 2);
        return p + 2;
    } else if (LP_ENCODING_IS_32BIT_STR(p[0])) {
        *count = LP_ENCODING_32BIT_STR_LEN(p);
        if (entry_size)
            *entry_size = 5 + *count + lpEncodeBacklenBytes(*count + 5);
        return p + 5;
    } else {
...
static inline unsigned char *lpFindCbInternal(unsigned char *lp, unsigned char *p, void *user, lpCmp cmp,
                                              unsigned int skip)
{
    int skipcnt = 0;
    unsigned char *value;
...
static inline int lpFindCmp(const unsigned char *lp, unsigned char *p, void *user, unsigned char *s, long long slen)
{
    (void)lp;
    (void)p;
...
{
    struct lpFindArg arg = {.s = s, .slen = slen};
...
unsigned char *lpInsert(unsigned char *lp, unsigned char *elestr, unsigned char *eleint, uint32_t size,
 ...
 * holding the new element is returned (and the old pointer passed is no longer
 * considered valid). */
unsigned char *lpBatchInsert(unsigned char *lp, unsigned char *p, int where, listpackEntry *entries, unsigned int len,
                             unsigned char **newp)
{
    ...
    for (unsigned int i = 0; i < len; i++) {
        ...
        if (e->sval) {
            ...
            * Whatever the returned encoding is, 'enclen' is populated with the
            * length of the encoded element. */
            enc[i].enctype = lpEncodeGetType(e->sval, e->slen, enc[i].intenc, &enc[i].enclen);
        } else {
...
unsigned char *lpInsertString(unsigned char *lp, unsigned char *s, uint32_t slen, unsigned char *p, int where,
                              unsigned char **newp)
{
 ...
 *     }
 */
unsigned char *lpNextRandom(unsigned char *lp, unsigned char *p, unsigned int *index, unsigned int remaining,
                            int tuple_len)
{
    assert(tuple_len > 0);
...
int listpackTest(int argc, char *argv[], int flags)
{
    ...
    TEST("Batch append")
    {
        listpackEntry ent[6] = {{.sval = (unsigned char *)mixlist[0], .slen = strlen(mixlist[0])},
    ...
    TEST("lpNextRandom corner cases")
    {
        ...
        i = 0;
        assert(lpNextRandom(lp, lpFirst(lp), &i, 0, 1) == NULL);

        /* Pick all returns all. */
        i = 0;
        assert(lpNextRandom(lp, p0, &i, 3, 1) == p0 && i == 0);
        i = 1;
        assert(lpNextRandom(lp, p1, &i, 2, 1) == p1 && i == 1);
        i = 2;
        assert(lpNextRandom(lp, p2, &i, 1, 1) == p2 && i == 2);

        /* Pick more than one when there's only one left returns the last one. */
        i = 2;
        assert(lpNextRandom(lp, p2, &i, 42, 1) == p2 && i == 2);

        /* Pick all even elements returns p0 and p2. */
        i = 0;
        assert(lpNextRandom(lp, p0, &i, 10, 2) == p0 && i == 0);
        i = 1;
        assert(lpNextRandom(lp, p1, &i, 10, 2) == p2 && i == 2);
    ...
    {
        listpackEntry ent[2] = {{.sval = (unsigned char *)mixlist[0], .slen = strlen(mixlist[0])},
                                {.sval = (unsigned char *)mixlist[1], .slen = strlen(mixlist[1])}};
