...
usage:
    ...
    printf("%s%s%s", /* Split to avoid strings longer than 4095 (-Woverlength-strings). */
           ...
           " --seed <num>       Set the seed for random number generator. Default seed is based on "
           "time.\n",
           tls_usage,
...
int main(int argc, char **argv)
{
    ...
    if (config.cluster_mode) {
        ...
        config.redis_config = getRedisConfig(config.conn_info.hostip, config.conn_info.hostport, config.hostsocket);
    ...
    if (config.csv) {
        printf("\"test\",\"rps\",\"avg_latency_ms\",\"min_latency_ms\",\"p50_latency_ms\",\"p95_"
               "latency_ms\",\"p99_latency_ms\",\"max_latency_ms\"\n");
    }
