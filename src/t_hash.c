...
typedef enum ExpireSetCond { HFE_NX = 1 << 0, HFE_XX = 1 << 1, HFE_GT = 1 << 2, HFE_LT = 1 << 3 } ExpireSetCond;
...
int hashTypeSetExInit(robj *key, kvobj *kvo, client *c, redisDb *db, ExpireSetCond expireSetCond, HashTypeSetEx *ex);
 ...
 * to its expiry time.*/
static int cbFindInListpack(const unsigned char *lp, unsigned char *p, void *user, unsigned char *s, long long slen)
{
    (void)lp;
...
void listpackExAddNew(robj *o, char *field, size_t flen, char *value, size_t vlen, uint64_t expireAt)
{
    listpackEntry ent[3] = {{.sval = (unsigned char *)field, .slen = flen},
 ...
 * by expiry time. */
static void listpackExUpdateExpiry(robj *o, sds field, unsigned char *fptr, unsigned char *vptr, uint64_t expire_at)
{
    unsigned int slen = 0;
    long long val = 0;
...
SetExRes hashTypeSetExpiryListpack(HashTypeSetEx *ex, sds field, unsigned char *fptr, unsigned char *vptr,
                                   unsigned char *tptr, uint64_t expireAt)
{
    ...
    if (prevExpire == EB_EXPIRE_TIME_INVALID) {
        ...
        if (((ex->expireSetCond == HFE_GT) && (prevExpire >= expireAt)) ||
            ((ex->expireSetCond == HFE_LT) && (prevExpire <= expireAt)) || (ex->expireSetCond == HFE_NX))
            return HSETEX_NO_CONDITION_MET;
...
GetFieldRes hashTypeGetFromListpack(robj *o, sds field, unsigned char **vstr, unsigned int *vlen, long long *vll,
                                    uint64_t *expiredAt)
{
 ...
 */
GetFieldRes hashTypeGetValue(redisDb *db, kvobj *o, sds field, unsigned char **vstr, unsigned int *vlen, long long *vll,
                             int hfeFlags, uint64_t *expiredAt)
{
    ...
    if ((server.loading) || (server.allow_access_expired) || (hfeFlags & HFE_LAZY_AVOID_FIELD_DEL) ||
 ...
 */
int hashTypeGetValueObject(redisDb *db, kvobj *o, sds field, int hfeFlags, robj **val, uint64_t *expireTime,
                           int *isHashDeleted)
{
    unsigned char *vstr;
    unsigned int vlen;
    ...
    if (val)
        *val = NULL;
    GetFieldRes res = hashTypeGetValue(db, o, field, &vstr, &vlen, &vll, hfeFlags, expireTime);
    ...
    if (res == GETF_OK) {
        ...
        if (val) {
            if (vstr)
                *val = createStringObject((char *)vstr, vlen);
            else
                *val = createStringObjectFromLongLong(vll);
        }
        return 1;
    }
...
int hashTypeSet(redisDb *db, robj *o, sds field, sds value, int flags)
{
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (!update)
            listpackExAddNew(o, field, sdslen(field), value, sdslen(value), HASH_LP_NO_TTL);
    ...
    if (!hfieldIsExpireAttached(hfOld)) {
        ...
        if (prevExpire != EB_EXPIRE_TIME_INVALID) {
            if (((exInfo->expireSetCond == HFE_GT) && (prevExpire >= expireAt)) ||
                ((exInfo->expireSetCond == HFE_LT) && (prevExpire <= expireAt)) || (exInfo->expireSetCond == HFE_NX))
                return HSETEX_NO_CONDITION_MET;
...
int hashTypeSetExInit(robj *key, kvobj *o, client *c, redisDb *db, ExpireSetCond expireSetCond, HashTypeSetEx *ex)
{
    dict *ht = o->ptr;
...
void hashTypeSetExDone(HashTypeSetEx *ex)
{
     ...
     */
    uint64_t diff = (ex->minExpire > newMinExpire) ? (ex->minExpire - newMinExpire) : (newMinExpire - ex->minExpire);
    if (diff < HASH_NEW_EXPIRE_DIFF_THRESHOLD)
        return;
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (subtractExpiredFields && isDictWithMetaHFE(d)) {
            ...
            if (meta->expireMeta.trash == 0)
                expiredItems = ebExpireDryRun(meta->hfe, &hashFieldExpireBucketsType, commandTimeSnapshot());
        }
...
{
    serverAssert(hi->encoding == OBJ_ENCODING_LISTPACK || hi->encoding == OBJ_ENCODING_LISTPACK_EX);
    ...
    if (enc == OBJ_ENCODING_LISTPACK_EX) {
        ...
        while (hashTypeNext(hi, 0) != C_ERR) {
            ...
            if (ret != DICT_OK) {
                ...
                sdsfree(value);              /* Needed for gcc ASAN */
                hashTypeReleaseIterator(hi); /* Needed for gcc ASAN */
                serverLogHexDump(LL_WARNING, "listpack with dup elements dump", lpt->lp, lpBytes(lpt->lp));
                serverPanic("Listpack corruption detected");
            }
...
void hashTypeRandomElement(robj *hashobj, unsigned long hashsize, CommonEntry *key, CommonEntry *val)
{
    if (hashobj->encoding == OBJ_ENCODING_HT) {
        ...
        lpRandomPair(hashTypeListpackGetLp(hashobj), hashsize, (listpackEntry *)key, (listpackEntry *)val, 3);
    } else {
        serverPanic("Unknown hash encoding");
    }
}
...
static uint64_t hashTypeExpire(kvobj *o, ExpireCtx *expireCtx, int updateGlobalHFE)
{
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK_EX) {
        info = (ExpireInfo){
            .maxToExpire = expireCtx->fieldsToExpireQuota, .now = commandTimeSnapshot(), .itemsExpired = 0};
        ...
        info = (ExpireInfo){.maxToExpire = expireCtx->fieldsToExpireQuota,
...
static int hashTypeExpireIfNeeded(redisDb *db, kvobj *o)
{
    ...
    if ((server.loading) || (server.allow_access_expired) ||
...
uint64_t hashTypeDbActiveExpire(redisDb *db, uint32_t maxFieldsToExpire)
{
    ...
    ExpireInfo info = {.maxToExpire = UINT64_MAX, /* Only maxFieldsToExpire play a role */
...
void hsetCommand(client *c)
{
    ...
    if ((kv = hashTypeLookupWriteOrCreate(c, c->argv[1])) == NULL)
        return;
...
static int parseExpireTime(client *c, robj *o, int unit, long long basetime, long long *expire)
{
    long long val;
...
*/
static int hsetexParseArgs(client *c, int *flags, long long *expire_time, int *expire_time_pos, int *first_field_pos,
                           int *field_count)
{
    *flags = 0;
    *first_field_pos = -1;
    ...
    *expire_time_pos = -1;
    ...
    for (int i = 2; i < c->argc; i++) {
        if (!strcasecmp(c->argv[i]->ptr, "fields")) {
            ...
            if (getRangeLongFromObjectOrReply(c, c->argv[i + 1], 1, INT_MAX, &val, "invalid number of fields") != C_OK)
                return C_ERR;
            ...
            *flags |= HFE_EX;
            ...
            if (parseExpireTime(c, c->argv[i], UNIT_SECONDS, commandTimeSnapshot(), expire_time) != C_OK)
                return C_ERR;
            ...
            *flags |= HFE_PX;
            i++;
            if (parseExpireTime(c, c->argv[i], UNIT_MILLISECONDS, commandTimeSnapshot(), expire_time) != C_OK)
                return C_ERR;
...
void hsetexCommand(client *c)
{
    ...
    if (hsetexParseArgs(c, &flags, &expire_time, &expire_time_pos, &first_field_pos, &field_count) != C_OK)
        return;
    ...
    if (flags & (HFE_FXX | HFE_FNX)) {
        ...
        for (int i = 0; i < field_count; i++) {
            ...
            long long vll = LLONG_MAX;
            const int opt =
                HFE_LAZY_NO_NOTIFICATION | HFE_LAZY_NO_SIGNAL | HFE_LAZY_AVOID_HASH_DEL | HFE_LAZY_NO_UPDATE_KEYSIZES;
            ...
            if (((flags & HFE_FXX) && !exists) || ((flags & HFE_FNX) && exists))
                break;
        }
        ...
        if (((flags & HFE_FNX) && !non_exists) || ((flags & HFE_FXX) && !all_exists)) {
            addReplyLongLong(c, 0);
            goto out;
        }
    }
...
void hincrbyCommand(client *c)
{
    ...
    GetFieldRes res = hashTypeGetValue(c->db, o, c->argv[2]->ptr, &vstr, &vlen, &value, HFE_LAZY_EXPIRE, NULL);
    if (res == GETF_OK) {
        if (vstr) {
...
void hincrbyfloatCommand(client *c)
{
    ...
    if ((o = hashTypeLookupWriteOrCreate(c, c->argv[1])) == NULL)
        return;
    GetFieldRes res = hashTypeGetValue(c->db, o, c->argv[2]->ptr, &vstr, &vlen, &ll, HFE_LAZY_EXPIRE, NULL);
    if (res == GETF_OK) {
        if (vstr) {
    ...
    rewriteClientCommandVector(c, 7, shared.hsetex, c->argv[1], shared.keepttl, shared.fields, shared.integers[1],
                               c->argv[2], newobj);
    decrRefCount(newobj);
}
...
void hstrlenCommand(client *c)
{
    ...
    GetFieldRes res = hashTypeGetValue(c->db, o, c->argv[2]->ptr, &vstr, &vlen, &vll, HFE_LAZY_EXPIRE, NULL);
...
void hexistsCommand(client *c)
{
    ...
    addReply(c, hashTypeExists(c->db, o, c->argv[2]->ptr, HFE_LAZY_EXPIRE, NULL) ? shared.cone : shared.czero);
}
...
void hrandfieldWithCountCommand(client *c, long l, int withvalues)
{
    ...
    if (!uniq || count == 1) {
        ...
        } else if (hash->encoding == OBJ_ENCODING_LISTPACK || hash->encoding == OBJ_ENCODING_LISTPACK_EX) {
    ...
    * elements inside the hash: simply return the whole hash. */
    ...
    if (hash->encoding == OBJ_ENCODING_LISTPACK || hash->encoding == OBJ_ENCODING_LISTPACK_EX) {
        unsigned char *lp = hashTypeListpackGetLp(hash);
        int tuple_len = hash->encoding == OBJ_ENCODING_LISTPACK ? 2 : 3;
        listpackEntry *keys, *vals = NULL;
 ...
 *----------------------------------------------------------------------------*/
static hfield _hfieldNew(const void *field, size_t fieldlen, int withExpireMeta, int trymalloc)
{
    if (!withExpireMeta)
        return mstrNew(field, fieldlen, trymalloc);

    hfield hf = mstrNewWithMeta(&mstrFieldKind, field, fieldlen, (mstrFlags)1 << HFIELD_META_EXPIRE, trymalloc);
...
static void propagateHashFieldDeletion(redisDb *db, sds key, char *field, size_t fieldLen)
{
    robj *argv[] = {shared.hdel, createStringObject((char *)key, sdslen(key)), createStringObject(field, fieldLen)};
...
static void httlGenericCommand(client *c, const char *cmd, long long basetime, int unit)
{
    ...
    /* Read number of fields */
    if (getRangeLongFromObjectOrReply(c, c->argv[numFieldsAt], 1, LONG_MAX, &numFields,
                                      "Number of fields must be a positive integer") != C_OK)
        return;
...
static void hexpireGenericCommand(client *c, long long basetime, int unit)
{
    ...
    /* Read number of fields */
    if (getRangeLongFromObjectOrReply(c, c->argv[numFieldsAt], 1, LONG_MAX, &numFields,
                                      "Parameter `numFields` should be greater than 0") != C_OK)
        return;
    ...
    if (deleted + updated > 0) {
        ...
        notifyKeyspaceEvent(NOTIFY_HASH, deleted ? "hdel" : "hexpire", keyArg, c->db->id);
    }
