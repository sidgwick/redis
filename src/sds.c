...
int sdsTest(int argc, char **argv, int flags)
{
...

    {
        ...
        { /* Test adjustTypeIfNeeded() */
            ...
            test_cond("adjustTypeIfNeeded() doesn't change header length when not needed",
                      hdrlen == sdsHdrSize(SDS_TYPE_8));
            ...
            test_cond("adjustTypeIfNeeded() doesn't change header length for SDS_TYPE_5",
                      hdrlen == sdsHdrSize(SDS_TYPE_5));
        }
    }
    return 0;
}
