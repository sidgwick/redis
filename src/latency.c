...
sds createLatencyReport(void)
{
    ...
    if (dictSize(server.latency_events) == 0 && server.latency_monitor_threshold == 0) {
        report = sdscat(report, "I'm sorry, Dave, I can't do that. Latency monitoring is disabled in "
                                "this Redis instance. You may use \"CONFIG SET "
                                "latency-monitor-threshold <milliseconds>.\" in order to enable it. If "
                                "we weren't in a deep space mission I'd suggest to take a look at "
                                "https://redis.io/docs/latest/operate/oss_and_stack/management/"
                                "optimization/latency-monitor.\n");
        return report;
    }
    ...
    while ((de = dictNext(&di)) != NULL) {
        ...
        eventnum++;
        if (eventnum == 1) {
            report = sdscat(report, "Dave, I have observed latency spikes in this Redis instance. "
        ...
        report = sdscatprintf(report,
                              ...
                              "%.2f sec). Worst all time event %lums.",
    ...
    if (eventnum == 0 && advices == 0) {
        report = sdscat(report, "Dave, no latency spike was observed during the lifetime of this Redis "
                                "instance, not in the slightest bit. I honestly think you ought to sit "
                                "down calmly, take a stress pill, and think things over.\n");
    } else if (eventnum > 0 && advices == 0) {
        report = sdscat(report, "\nWhile there are latency events logged, I'm not able to suggest "
                                "any easy fix. Please use the Redis community to get some help, "
                                "providing this report in your help request.\n");
    } else {
        ...
        if (advise_slowlog_enabled) {
            report = sdscatprintf(report,
                                  "- There are latency issues with potentially slow commands you are "
                                  "using. Try to enable the Slow Log Redis feature using the command "
                                  "'CONFIG SET slowlog-log-slower-than %llu'. If the Slow log is "
                                  "disabled Redis is not able to log slow commands execution for you.\n",
        ...
        if (advise_slowlog_tuning) {
            report = sdscatprintf(report,
                                  ...
                                  "use 'CONFIG SET slowlog-log-slower-than %llu'.\n",
                                  (unsigned long long)server.latency_monitor_threshold * 1000);
        }

        if (advise_slowlog_inspect) {
            report = sdscat(report, "- Check your Slow Log to understand what are the commands you "
        ...
        /* Intrinsic latency. */
        if (advise_scheduler) {
            report = sdscat(report, "- The system is slow to execute Redis code paths not containing "
                                    "system calls. This usually means the system does not provide Redis "
                                    "CPU time to run for long periods. You should try to:\n"
                                    "  1) Lower the system load.\n"
                                    "  2) Use a computer / VM just for Redis if you are running other "
                                    "software in the same system.\n"
                                    "  3) Check if you have a \"noisy neighbour\" problem.\n"
                                    "  4) Check with 'redis-cli --intrinsic-latency 100' what is the "
                                    "intrinsic latency in your system.\n"
                                    "  5) Check if the problem is allocator-related by recompiling "
                                    "Redis with MALLOC=libc, if you are using Jemalloc. However this "
                                    "may create fragmentation problems.\n");
        }

        /* AOF / Disk latency. */
        if (advise_local_disk) {
            report = sdscat(report, "- It is strongly advised to use local disks for persistence, "
                                    "especially if you are using AOF. Remote disks provided by "
                                    "platform-as-a-service providers are known to be slow.\n");
        }

        if (advise_ssd) {
            report = sdscat(report, "- SSD disks are able to reduce fsync latency, and total time "
                                    "needed for snapshotting and AOF log rewriting (resulting in "
                                    "smaller memory usage). With extremely high write load SSD disks "
                                    "can be a good option. However Redis should perform reasonably with "
                                    "high load using normal disks. Use this advice as a last resort.\n");
        }

        if (advise_data_writeback) {
            report = sdscat(report, "- Mounting ext3/4 filesystems with data=writeback can provide a "
                                    "performance boost compared to data=ordered, however this mode of "
                                    "operation provides less guarantees, and sometimes it can happen that "
                                    "after a hard crash the AOF file will have a half-written command at "
                                    "the end and will require to be repaired before Redis restarts.\n");
        }
        ...
        if (advise_write_load_info) {
            report = sdscat(report, "- Latency during the AOF atomic rename operation or when the final difference is "
                                    "flushed to the AOF file at the end of the rewrite, sometimes is caused by very "
                                    "high write load, causing the AOF buffer to get very large. If possible try to "
                                    "send less commands to accomplish the same work, or use Lua scripts to group "
        ...
        if (advise_large_objects) {
            report = sdscat(report, "- Deleting, expiring or evicting (because of maxmemory policy) "
...
void latencyCommand(client *c)
{
    ...
    if (!strcasecmp(c->argv[1]->ptr, "history") && c->argc == 3) {
        ...
        const char *help[] = {"DOCTOR",
                              ...
                              "a histogram for the specified command names.",
