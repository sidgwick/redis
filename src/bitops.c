...
{
    char *p = o->ptr;
    char *err = "Invalid bitfield type. Use something like i16 u8. Note that u64 is not supported "
                "but i64 is.";
    long long llbits;
 ...
 */
static kvobj *lookupStringForBitCommand(client *c, uint64_t maxbit, size_t *strOldSize, size_t *strGrowSize)
{
    dictEntryLink link;
...
void setbitCommand(client *c)
{
    ...
    if (strGrowSize || (!!bitval != on)) {
        ...
        if ((strOldSize > 0) && (strGrowSize != 0))
            updateKeysizesHist(c->db, getKeySlot(c->argv[1]->ptr), OBJ_STRING, strOldSize, strOldSize + strGrowSize);
    }
...
unsigned long bitopCommandAVX(unsigned char **keys, unsigned char *res, unsigned long op, unsigned long numkeys,
...
void bitfieldGeneric(client *c, int flags)
{
    ...
    if (readonly) {
         ...
         * this operation. */
        if ((o = lookupStringForBitCommand(c, highest_write_offset, &strOldSize, &strGrowSize)) == NULL) {
            zfree(ops);
            return;
        }
    }
