...
{
    if (nextCommandCategory >= ACL_MAX_CATEGORIES)
        return 0;
...
{
    if (raxFind(Users, (unsigned char *)name, namelen, NULL))
        return NULL;
...
sds ACLLoadFromFile(const char *filename)
{
    ...
    if (sdslen(errors) == 0) {
        ...
        Users = old_users;
        errors = sdscat(errors, "WARNING: ACL errors detected, no change to the previously active "
                                "ACL rules was performed");
        return errors;
    }
}
...
void aclCommand(client *c)
{
    ...
    if (!strcasecmp(sub, "setuser") && c->argc >= 3) {
        ...
        addReplyError(c, "This Redis instance is not configured to use an ACL file. You may want to specify "
                         "users via the ACL SETUSER command and then issue a CONFIG REWRITE (assuming you have "
                         "a Redis configuration file set) in order to store users in the Redis configuration.");
        return;
...
#define GENPASS_MAX_BITS 4096
        ...
        const char *help[] = {"CAT [<category>]",
                              "    List all commands that belong to <category>, or all command categories",
                              "    when no category is specified.",
                              "DELUSER <username> [<username> ...]",
                              "    Delete a list of users.",
                              "DRYRUN <username> <command> [<arg> ...]",
                              "    Returns whether the user can execute the given command without executing the "
                              "command.",
                              "GETUSER <username>",
