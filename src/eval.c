...
void scriptCommand(client *c)
{
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
        const char *help[] = {"DEBUG (YES|SYNC|NO)",
...
int ldbStartSession(client *c)
{
    ...
    if (ldb.forked) {
        ...
        serverLog(LL_NOTICE, "Redis synchronous debugging eval session started");
    }
...
void ldbEndSession(client *c)
{
    ...
    if (ldb.forked) {
        ...
        serverLog(LL_NOTICE, "Redis synchronous debugging eval session ended");
    }
