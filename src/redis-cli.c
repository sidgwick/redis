...
static void cliLegacyIntegrateHelp(void)
{
    ...
    for (size_t j = 0; j < reply->elements; j++) {
        ...
        while (args-- > 0)
            new->docs.params = sdscat(new->docs.params, "arg ");
...
static helpEntry *cliLegacyInitCommandHelpEntry(char *cmdname, char *subcommandname, helpEntry *next,
                                                struct commandDocs *command, dict *groups, sds version)
{
    helpEntry *help = next++;
    cliFillInCommandHelpEntry(help, cmdname, subcommandname);
...
static sds addHintForArguments(sds hint, cliCommandArg *args, int numargs, char *separator)
{
    ...
    for (i = 0; i < numargs; i++) {
        ...
        for (j = i, incomplete = -1; j < numargs; j++) {
            if (!(args[j].flags & CMD_ARG_OPTIONAL))
                break;
    ...
    for (nextword = 0, nextarg = 0; nextword != numwords && nextarg != numargs; ++nextarg) {
        ...
        if (args[nextarg].flags & CMD_ARG_OPTIONAL) {
            ...
            for (lastoptional = nextarg; lastoptional < numargs; lastoptional++) {
                if (!(args[lastoptional].flags & CMD_ARG_OPTIONAL))
                    break;
            }
            matchedWords =
...
static helpEntry *findHelpEntry(int argc, char **argv)
{
    ...
    for (i = 0; i < helpEntriesLen; i++) {
        if (!(helpEntries[i].type & CLI_HELP_COMMAND))
            continue;
...
sds sdsCatColorizedLdbReply(sds o, char *s, size_t len)
{
    ...
    if (strstr(s, "<hint>"))
        color = "bold";
    if (strstr(s, "<value>") || strstr(s, "<retval>"))
        color = "magenta";
...
static int cliReadReply(int output_raw_strings)
{
    ...
    if (redisGetReply(context, &_reply) != REDIS_OK) {
        ...
        if (config.interactive) {
            ...
            if (context->err == REDIS_ERR_IO && (errno == ECONNRESET || errno == EPIPE))
                return REDIS_ERR;
...
static void cliWaitForMessagesOrStdin(void)
{
    ...
    while (config.pubsub_mode) {
        ...
        if (show_info) {
            if (use_color)
                printf("\033[1;90m"); /* Bold, bright color. */
...
static void usage(int err)
{
    ...
    fprintf(target,
            ...
            "                     and in --bigkeys, --memkeys, --keystats, and --hotkeys per 100 "
            "cycles.\n"
            ...
            "  --json             Output in JSON format (default RESP3, use -2 if you want to use with "
            "RESP2).\n"
    ...
    fprintf(target, "  --bigkeys          Sample Redis keys looking for keys with many elements (complexity).\n"
                    "  --memkeys          Sample Redis keys looking for keys consuming a lot of memory.\n"
                    "  --memkeys-samples <n> Sample Redis keys looking for keys consuming a lot of memory.\n"
                    "                     And define number of key elements to sample\n"
                    "  --keystats         Sample Redis keys looking for keys memory size and length (combine "
                    "bigkeys and memkeys).\n"
                    "  --keystats-samples <n> Sample Redis keys looking for keys memory size and length.\n"
                    "                     And define number of key elements to sample (only for memory "
                    "usage).\n"
    ...
    fprintf(target, "Cluster Manager Commands:\n"
                    "  Use --cluster help to list all available cluster manager commands.\n"
                    "\n"
                    "Examples:\n"
                    "  redis-cli -u redis://default:PASSWORD@localhost:6379/0\n"
                    "  cat /etc/passwd | redis-cli -x set mypasswd\n"
                    "  redis-cli -D \"\" --raw dump key > key.dump && redis-cli -X dump_tag "
                    "restore key2 0 dump_tag replace < key.dump\n"
...
static void repl(void)
{
    ...
    while (1) {
        ...
        if (line == NULL) {
            ...
            if (history)
                linenoiseHistoryAdd(line, is_sensitive);
            if (!is_sensitive && historyfile)
                linenoiseHistorySave(historyfile);
...
static int noninteractive(int argc, char **argv)
{
    ...
    if (config.stdin_lastarg) {
        ...
        for (; i < argc; i++) {
            if (strcmp(config.stdin_tag_name, sds_args[i]) != 0)
                continue;
...
static int clusterManagerGetAntiAffinityScore(clusterManagerNodeArray *ipnodes, int ip_count,
                                              clusterManagerNode ***offending, int *offending_len);
...
static int createClusterManagerCommand(char *cmdname, int argc, char **argv)
{
    ...
    if (isColorTerm())
        cmd->flags |= CLUSTER_MANAGER_CMD_FLAG_COLOR;
    ...
    if (config.stdin_lastarg) {
        ...
        for (; i < argc; i++) {
            if (strcmp(argv[i], config.stdin_tag_name) != 0)
                continue;
...
static clusterManagerCommandProc *validateClusterManagerCommand(void)
{
    ...
    if (!proc)
        fprintf(stderr, "Unknown --cluster subcommand\n");
    return proc;
}
...
static sds clusterManagerGetNodeRDBFilename(clusterManagerNode *node)
{
    ...
    filename = sdscatprintf(filename, "redis-node-%s-%d-%s.rdb", node->ip, node->port, node->name);
    return filename;
}
...
static void clusterManagerOptimizeAntiAffinity(clusterManagerNodeArray *ipnodes, int ip_count)
{
    ...
    while (maxiter > 0) {
        ...
        if (score == 0 || offending_len == 0)
            break; // Optimal anti affinity reached
...
static sds clusterManagerNodeInfo(clusterManagerNode *node, int indent)
{
    ...
    int i;
    for (i = 0; i < indent; i++)
        spaces = sdscat(spaces, " ");
    if (indent)
        info = sdscat(info, spaces);
...
static int clusterManagerSetSlot(clusterManagerNode *node1, clusterManagerNode *node2, int slot, const char *status,
                                 char **err)
{
...
static signed int clusterManagerCountKeysInSlot(clusterManagerNode *node, int slot)
{
    ...
    int count = -1;
    int success = clusterManagerCheckRedisReply(node, reply, NULL);
    if (success && reply->type == REDIS_REPLY_INTEGER)
        count = reply->integer;
    if (reply)
        freeReplyObject(reply);
    return count;
 ...
 * error. */
static int clusterManagerCompareKeysValues(clusterManagerNode *n1, clusterManagerNode *n2, redisReply *keys_reply,
                                           list *diffs)
{
...
static redisReply *clusterManagerMigrateKeysInReply(clusterManagerNode *source, clusterManagerNode *target,
                                                    redisReply *reply, int replace, int timeout, char *dots)
{
...
static int clusterManagerMigrateKeysInSlot(clusterManagerNode *source, clusterManagerNode *target, int slot,
                                           int timeout, int pipeline, int verbose, char **err)
{
    int success = 1;
    ...
    while (1) {
        ...
        if (verbose)
            dots = zmalloc((count + 1) * sizeof(char));
...
*/
static int clusterManagerMoveSlot(clusterManagerNode *source, clusterManagerNode *target, int slot, int opts,
                                  char **err)
{
    ...
    if (!option_cold) {
        ...
        while ((ln = listNext(&li)) != NULL) {
            ...
            if (n->flags & CLUSTER_MANAGER_FLAG_SLAVE)
                continue;
    ...
    *err = NULL;
    ...
    while ((p = strstr(lines, "\n")) != NULL) {
        *p = '\0';
        ...
        if (ping_sent != NULL)
            currentNode->ping_sent = atoll(ping_sent);
        if (ping_recv != NULL)
            currentNode->ping_recv = atoll(ping_recv);
...
static list *clusterManagerGetDisconnectedLinks(clusterManagerNode *node)
{
    list *links = NULL;
    redisReply *reply = CLUSTER_MANAGER_COMMAND(node, "CLUSTER NODES");
    if (!clusterManagerCheckRedisReply(node, reply, NULL))
        goto cleanup;
    links = listCreate();
    char *lines = reply->str, *p, *line;
    while ((p = strstr(lines, "\n")) != NULL) {
        ...
        *p = '\0';
        ...
        if (i == 7)
            link_status = line;
        if (nodename == NULL || addr == NULL || flags == NULL || link_status == NULL)
            continue;
        if (strstr(flags, "myself") != NULL)
            continue;
...
static int clusterManagerFixSlotsCoverage(char *all_slots)
{
    ...
    if (cluster_manager.unreachable_masters > 0 && !force_fix) {
        clusterManagerLogWarn("*** Fixing slots coverage with %d unreachable masters is dangerous: redis-cli will "
                              "assume that slots about masters that are not reachable are not covered, and will try "
                              "to reassign them to the reachable nodes. This can cause data loss and is rarely what "
                              "you want to do. If you really want to proceed use the "
...
static int clusterManagerFixOpenSlot(int slot)
{
    ...
    if (cluster_manager.unreachable_masters > 0 && !force_fix) {
        clusterManagerLogWarn("*** Fixing open slots with %d unreachable masters is dangerous: redis-cli will assume "
                              "that slots about masters that are not reachable are not covered, and will try to "
                              "reassign them to the reachable nodes. This can cause data loss and is rarely what you "
                              "want to do. If you really want to proceed use the "
                              "--cluster-fix-with-unreachable-masters option.\n",
                              cluster_manager.unreachable_masters);
        exit(1);
    }
     ...
     * and the importing nodes. */
        ...
        if (try_to_close_slot) {
            ...
            if (!owner || owner != n) {
                ...
                if (r) {
                    if (success)
                        try_to_close_slot = (r->elements == 0);
                    freeReplyObject(r);
                }
                if (!success)
                    goto cleanup;
...
static list *clusterManagerComputeReshardTable(list *sources, int numslots)
{
    ...
    for (i = 0; i < src_count; i++) {
        ...
        for (j = 0; j < CLUSTER_MANAGER_SLOTS; j++) {
            ...
            if (!slot)
                continue;
            if (count >= max || (int)listLength(moved) >= numslots)
                break;
        ...
        while ((ln = listNext(&li)) != NULL) {
            ...
            if (first == NULL) {
                ...
                if (config.prefer_ipv4)
                    anet_flags |= ANET_PREFER_IPV4;
                if (config.prefer_ipv6)
                    anet_flags |= ANET_PREFER_IPV6;
    ...
    if (!master_node) {
        ...
        if (!clusterManagerCheckRedisReply(refnode, reply, &err)) {
            ...
            if (function_list_reply->elements > 0) {
                clusterManagerLogErr(">>> New node already contains functions and can not be added "
                                     "to the cluster. Use FUNCTION FLUSH and try again.\r\n");
                success = 0;
                goto cleanup;
            }
...
cleanup:
    ...
    if (reply)
        freeReplyObject(reply);
    if (function_restore_reply)
        freeReplyObject(function_restore_reply);
    if (function_list_reply)
        freeReplyObject(function_list_reply);
    return success;
invalid_args:
...
static int clusterManagerCommandImport(int argc, char **argv)
{
    ...
    if (!src_reply || src_reply->type == REDIS_REPLY_ERROR) {
        if (src_reply && src_reply->str)
            reply_err = src_reply->str;
        success = 0;
        goto cleanup;
    }
...
static int clusterManagerCommandCall(int argc, char **argv)
{
    ...
    while ((ln = listNext(&li)) != NULL) {
        ...
        if (!n->context && !clusterManagerNodeConnect(n))
            continue;
        redisReply *reply = NULL;
...
unsigned long long sendSync(redisContext *c, int send_sync, char *out_eof, int *out_full_mode)
{
    ...
    while (1) {
        ...
        if (p >= buf + sizeof(buf) - 1)
            break; /* Go back one more char for null-term. */
    }
    *p = '\0';
    if (buf[0] == '-') {
    ...
    while (!done) {
        ...
        if (mask & AE_WRITABLE) {
            ...
            while (1) {
                ...
                if ((obuf_len == 0 && eof) || loop_nwritten > PIPEMODE_WRITE_LOOP_MAX_BYTES)
                    break;
            }
        }
...
static void getKeySizes(redisReply *keys, typeinfo **types, unsigned long long *sizes, int memkeys,
                        long long memkeys_samples)
{
    redisReply *reply;
    unsigned int i;
...
static int displayKeyStatsProgressbar(unsigned long long sampled, unsigned long long total_keys);
            ...
            if (mstime() > refresh_time + REFRESH_INTERVAL && IS_TTY_OR_FAKETTY()) {
                ...
                while ((de = dictNext(&di))) {
                    ...
                    if (current_type->biggest > 0) {
                        line_count += cleanPrintfln("Biggest %-9s found so far %s with %llu %s", current_type->name,
                                                    current_type->biggest_key, current_type->biggest,
...
static void findHotKeys(void)
{
    ...
    do {
        ...
        for (i = 0; i < keys->elements; i++) {
            ...
            k = 0;
            while (k < HOTKEYS_SAMPLE && freqs[i] > counters[k])
                k++;
            if (k == 0)
                continue;
            k--;
        ...
        if (mstime() > refresh_time + REFRESH_INTERVAL && IS_TTY_OR_FAKETTY()) {
            ...
            for (k = HOTKEYS_SAMPLE - 1; k >= 0; k--) {
                if (counters[k] > 0) {
                    line_count +=
                        cleanPrintfln("hot key found with counter: %llu\tkeyname: %s", counters[k], hotkeys[k]);
                }
            }
...
static int displayKeyStatsLengthDist(size_dist *dist)
{
    ...
    for (int i = 0; dist->size_dist[i].size; i++) {
        if (dist->size_dist[i].count) {
            ...
            line_count += cleanPrintfln("%15s %9.4f%% %11llu", bytesToHuman(buf[1], sizeof(buf[1]), size),
                                        (double)100 * total_keys / dist->total_count, total_keys);
        }
    }
    ...
    line_count += cleanPrintfln(
        "Total key length is %s (%s avg)", bytesToHuman(buf[0], sizeof(buf[0]), dist->total_size),
...
static int displayKeyStatsProgressbar(unsigned long long sampled, unsigned long long total_keys)
{
    ...
    if (IS_TTY_OR_FAKETTY()) {
        ...
        snprintf(progressbar, sizeof(progressbar), "%s%s%s%s%s", green, buf[0], red, buf[1], default_color);
    } else {
...
static int displayKeyStatsSizeType(dict *memkeys_types_dict)
{
    ...
    while ((de = dictNext(&di))) {
        typeinfo *type = dictGetVal(de);
        if (type->biggest_key) {
            line_count += cleanPrintfln("%-10s %s is %s", type->name, type->biggest_key,
...
static int displayKeyStatsSizeDist(struct hdr_histogram *keysize_histogram)
{
    ...
    while (hdr_iter_next(&iter)) {
        ...
        if (iter.cumulative_count != last_displayed_cumulative_count) {
            ...
            line_count +=
                cleanPrintfln("%8s %9.4f%% %11lld", bytesToHuman(size, sizeof(size), iter.highest_equivalent_value),
                              percentile, iter.cumulative_count);
...
static int displayKeyStatsType(unsigned long long sampled, dict *memkeys_types_dict, dict *bigkeys_types_dict)
{
    dictIterator di;
    dictEntry *de;
    int line_count = 0;
            ...
            if (bk_de) { /* If we have it in memkeys it should be in bigkeys */
                ...
                if (bigkey_type->sizecmd && bigkey_type->count) {
                    ...
                    if (!strcmp(bigkey_type->sizeunit, "bytes")) {
                        ...
                        bytesToHuman(length_avg, sizeof(length_avg),
                                     (long long)round(avg)); /* better than truncating */
                    } else {
                        snprintf(total_length, sizeof(total_length), "%llu %s", bigkey_type->totalsize,
                                 bigkey_type->sizeunit);
                        snprintf(length_avg, sizeof(length_avg), "%.2f", avg);
                    }
                }
            }
            /* Print the line for the given Redis type */
            line_count += cleanPrintfln("%-10s %11llu %6.2f%% %8s %8s %18s %11s", memkey_type->name, memkey_type->count,
                                        sampled ? 100 * (double)memkey_type->count / sampled : 0, total_size, size_avg,
                                        total_length, length_avg);
        }
    }
    dictResetIterator(&di);
...
static int updateTopSizes(char *key_name, size_t key_name_len, unsigned long long key_size, char *type_name,
                          list *topkeys, unsigned long top_sizes_limit)
{
    listNode *node;
    listIter iter;
    key_info *new_node;
    ...
    if (top_sizes_limit != 0 && topkeys->len == top_sizes_limit &&
...
static void displayKeyStats(unsigned long long sampled, unsigned long long total_keys, unsigned long long total_size,
                            dict *memkeys_types_dict, dict *bigkeys_types_dict, list *top_key_sizes,
...
static void keyStats(long long memkeys_samples, unsigned long long cursor, unsigned long top_sizes_limit)
{
    ...
    /* Status message */
    printf("\n# Scanning the entire keyspace to find the biggest keys and distribution "
           "information.\n");
    ...
    do {
        ...
        for (i = 0; i < keys->elements; i++) {
            ...
            updateTopSizes(keys->element[i]->str, keys->element[i]->len, memkeys_sizes[i], memkeys_types[i]->name,
                           top_sizes, top_sizes_limit);
    ...
    displayKeyStats(sampled, total_keys, total_size, memkeys_types_dict, bigkeys_types_dict, top_sizes, top_sizes_limit,
                    0);
