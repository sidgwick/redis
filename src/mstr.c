...
mstr mstrNewCopy(struct mstrKind *kind, mstr src, mstrFlags newFlags)
{
    ...
    if (newFlags == 0)
        return mstrNew(src, mstrlen(src), 0);
    ...
    if (mstrIsMetaAttached(src)) {
        mstrFlags *pFlags1 = mstrFlagsRef(src), *pFlags2 = mstrFlagsRef(dst);

        mstrFlags flags1Shift = *pFlags1, flags2Shift = *pFlags2;

        unsigned char *at1 = ((unsigned char *)pFlags1), *at2 = ((unsigned char *)pFlags2);
...
int mstrTest(int argc, char **argv, int flags)
{
    ...
    TEST_CONTEXT("Create simple 40 bytes mstr")
    {
        ...
        test_cond("Verify str length and alloc length",
                  mstrAllocLen(s, NULL) == (3 + 40 + 1) && /* mstrhdr8 + str + null */
                      mstrlen(s) == 40 && memcmp(s, str, 40) == 0);
        mstrFree(&kind_mymstr, s);
    }
    ...
    TEST_CONTEXT("Create mstr with random characters")
    {
        ...
        for (i = 0; i < sizeof(len) / sizeof(len[0]); ++i) {
            ...
            test_cond(title, mstrAllocLen(s, NULL) == (mstrhdrSize + len[i] + 1) && /* mstrhdrX + str + null */
                                 mstrlen(s) == len[i] && memcmp(s, str, len[i]) == 0);
            mstrFree(&kind_mymstr, s);
        }
    }
    ...
    TEST_CONTEXT("Create short mstr with TTL4")
    {
        ...
        *ttl = 0x12345678;
        ...
        test_cond("Verify string and TTL4 payload",
                  memcmp(mstrMetaRef(s, &kind_mymstr, 0), expMem, sizeof(expMem)) == 0);
    ...
    TEST_CONTEXT("Create short mstr with TTL4 and value ptr ")
    {
        ...
        *((uint32_t *)(mstrMetaRef(s, &kind_mymstr, META_IDX_MYMSTR_TTL4))) = 0x12345678;

        test_cond("Verify length and alloc length",
                  mstrAllocLen(s, &kind_mymstr) ==
                          (1 + 3 + 1 + 2 + 4 + 8) && /* mstrhdr5 + str + null + mstrFlags + TLL + PTR */
                      mstrlen(s) == 3);
        mstrFree(&kind_mymstr, s);
    }
    ...
    TEST_CONTEXT("Copy mstr and add it TTL4")
    {
        ...
        *((uint32_t *)(mstrMetaRef(s2, &kind_mymstr, META_IDX_MYMSTR_TTL4))) = 0x12345678;
        ...
        test_cond("Verify new mstr includes TTL4",
                  mstrAllocLen(s2, &kind_mymstr) ==
                          (1 + 3 + 1 + 2 + 4) && /* mstrhdr5 + str + null + mstrFlags + TTL4 */
        ...
        test_cond("Verify string and TTL4 payload", memcmp(ppp, expMem, sizeof(expMem)) == 0);
