...
void test_recall(HNSW *index, int ef)
{
    ...
    for (int t = 0; t < num_test_vectors; t++) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 300; j++) {
                test_vector[j] += weights[i] * source_vectors[src_indices[i]][j];
            }
        }
        ...
        int linear_found = hnsw_ground_truth_with_filter(index, test_vector, ef, linear_results, linear_distances, slot,
                                                         0, NULL, NULL);
    ...
    int max_count = 0;
    for (int i = 0; i < 50; i++) {
        if (recall_bins[i] > max_count)
            max_count = recall_bins[i];
    }
...
int main(int argc, char **argv)
{
    ...
    for (int j = 1; j < argc; j++) {
        ...
        if (!strcasecmp(argv[j], "--quant")) {
            ...
            printf("%s [--quant] [--bin] [--thread <count>] [--numele <count>] [--m <count>] "
                   "[--mass-del] [--self-recall] [--recall <ef>]\n",
                   argv[0]);
            exit(0);
        } else {
