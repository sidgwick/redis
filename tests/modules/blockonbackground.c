...
void blockClientMeasureTimeEnd(RedisModuleBlockedClient *bc, BlockPrivdata *block_privdata, int completed)
{
    ...
    if (!block_privdata->measuretime_completed) {
        ...
        if (completed)
            block_privdata->measuretime_completed = 1;
    }
    pthread_mutex_unlock(&block_privdata->measuretime_mutex);
}
...
int HelloDoubleBlock_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    pthread_t tid;
    RedisModuleBlockedClient *bc =
        RedisModule_BlockClient(ctx, HelloBlock_Reply, HelloBlock_Timeout, HelloBlock_FreeData, 0);
    blockClientPrivdataInit(bc);
...
int Block_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
     ...
     */
    blocked_client = RedisModule_BlockClient(ctx, Block_RedisCommand, timeout > 0 ? Block_RedisCommand : NULL,
                                             HelloBlock_FreeStringData, timeout);
    return REDISMODULE_OK;
}
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_CreateCommand(ctx, "block.double_debug", HelloDoubleBlock_RedisCommand, "", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;

    if (RedisModule_CreateCommand(ctx, "block.debug_no_track", HelloBlockNoTracking_RedisCommand, "", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
