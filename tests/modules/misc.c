...
static int KeySpace_NotificationModuleKeyMissExpired(RedisModuleCtx *ctx, int type, const char *event,
...
int test_ld_conv(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_StringCompare(s1, s2) != 0) {
        char err[4096];
        snprintf(err, 4096, "Failed to convert long double to string ('%s' != '%s')",
    ...
    if (ld2 != ld) {
        char err[4096];
        snprintf(err, 4096, "Failed to convert string to long double (%.40Lf != %.40Lf)", ld2, ld);
        RedisModule_ReplyWithError(ctx, err);
        goto final;
    }
...
int test_getclientcert(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    RedisModuleString *cert = RedisModule_GetClientCertificate(ctx, RedisModule_GetClientId(ctx));
    if (!cert) {
...
int test_clientinfo(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    char flags[512];
    snprintf(flags, sizeof(flags) - 1, "%s:%s:%s:%s:%s:%s", ci.flags & REDISMODULE_CLIENTINFO_FLAG_SSL ? "ssl" : "",
...
int test_ull_conv(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    RedisModuleString *s2 = RedisModule_CreateString(ctx, ullstr, strlen(ullstr));
    if (RedisModule_StringCompare(s1, s2) != 0) {
        char err[4096];
        snprintf(err, 4096, "Failed to convert unsigned long long to string ('%s' != '%s')",
                 RedisModule_StringPtrLen(s1, NULL), RedisModule_StringPtrLen(s2, NULL));
        RedisModule_ReplyWithError(ctx, err);
        goto final;
    }
    ...
    if (RedisModule_StringToULongLong(s2, &ull2) == REDISMODULE_ERR) {
        RedisModule_ReplyWithError(ctx, "Failed to convert string to unsigned long long");
        goto final;
    }
    if (ull2 != ull) {
        char err[4096];
        snprintf(err, 4096, "Failed to convert string to unsigned long long (%llu != %llu)", ull2, ull);
        RedisModule_ReplyWithError(ctx, err);
        goto final;
    }
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_SubscribeToKeyspaceEvents(ctx, REDISMODULE_NOTIFY_KEY_MISS | REDISMODULE_NOTIFY_EXPIRED,
    ...
    if (RedisModule_CreateCommand(ctx, "test.rm_call", test_rm_call, "allow-stale", 0, 0, 0) == REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "test.rm_call_flags", test_rm_call_flags, "allow-stale", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "test.rm_call_replicate", test_rm_call_replicate, "allow-stale", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "test.silent_open_key", test_open_key_no_effects, "", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
