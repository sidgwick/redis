...
int fsl_bpop(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (!fsl) {
        RedisModule_BlockClientOnKeys(ctx, bpop_reply_callback, to_cb ? bpop_timeout_callback : NULL, NULL, timeout,
                                      &argv[1], 1, NULL);
    } else {
        RedisModule_Assert(fsl->length);
        RedisModule_ReplyWithLongLong(ctx, fsl->list[--fsl->length]);
...
int fsl_bpopgt(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (fsl->list[fsl->length - 1] <= gt) {
        ...
        *pgt = gt;
        RedisModule_BlockClientOnKeysWithFlags(ctx, bpopgt_reply_callback, bpopgt_timeout_callback,
...
int fsl_bpoppush(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (!src) {
        ...
        RedisModule_BlockClientOnKeys(ctx, bpoppush_reply_callback, bpoppush_timeout_callback, bpoppush_free_privdata,
                                      timeout, &argv[1], 1, argv[2]);
    } else {
        fsl_t *dst;
...
int blockonkeys_popall(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_KeyType(key) == REDISMODULE_KEYTYPE_EMPTY) {
        RedisModule_BlockClientOnKeys(ctx, blockonkeys_popall_reply_callback, blockonkeys_popall_timeout_callback, NULL,
                                      3000, &argv[1], 1, NULL);
    } else {
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_CreateCommand(ctx, "blockonkeys.lpush_unblock", blockonkeys_lpush, "write", 1, 1, 1) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    ...
    if (RedisModule_CreateCommand(ctx, "blockonkeys.blpopn_or_unblock", blockonkeys_blpopn, "write", 1, 1, 1) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    return REDISMODULE_OK;
}
