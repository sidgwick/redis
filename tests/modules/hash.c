...
int hash_set(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (argc == 5) {
        ...
        result =
            RedisModule_HashSet(key, flags, argv[3], value_or_delete(argv[4]), argv[5], value_or_delete(argv[6]), NULL);
    } else if (argc == 9) {
        result = RedisModule_HashSet(key, flags, argv[3], value_or_delete(argv[4]), argv[5], value_or_delete(argv[6]),
                                     argv[7], value_or_delete(argv[8]), NULL);
    } else if (argc == 11) {
        result = RedisModule_HashSet(key, flags, argv[3], value_or_delete(argv[4]), argv[5], value_or_delete(argv[6]),
                                     argv[7], value_or_delete(argv[8]), argv[9], value_or_delete(argv[10]), NULL);
    } else {
...
int test_open_key_hget_expire(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    RedisModule_Assert(RedisModule_HashGet(key, REDISMODULE_HASH_EXISTS | REDISMODULE_HASH_EXPIRE_TIME, argv[2],
                                           &expireAt, NULL) == REDISMODULE_ERR);
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_CreateCommand(ctx, "hash.set", hash_set, "write", 1, 1, 1) == REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "hash.hget_expired", test_open_key_subexpired_hget, "", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "hash.hscan_expired", test_open_key_access_expired_hscan, "", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "hash.hget_expire", test_open_key_hget_expire, "", 0, 0, 0) == REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "hash.hget_two_expire", test_open_key_hget_two_expire, "", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    if (RedisModule_CreateCommand(ctx, "hash.hget_min_expire", test_open_key_hget_min_expire, "", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
