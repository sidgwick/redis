...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_CreateCommand(ctx, "cmdintrospection.xadd", cmd_xadd, "write deny-oom random fast", 0, 0, 0) ==
        REDISMODULE_ERR)
        return REDISMODULE_ERR;
    ...
    RedisModuleCommandInfo info = {
        ...
        .history =
             ...
             * XADD). */
                {"6.2.0", "Added the `NOMKSTREAM` option, `MINID` trimming strategy and the "
                          "`LIMIT` option."},
        ...
        .args =
            (RedisModuleCommandArg[]){
                ...
                {.name = "condition",
                 ...
                 .subargs =
                     (RedisModuleCommandArg[]){
                         {.name = "keepref", .type = REDISMODULE_ARG_TYPE_PURE_TOKEN, .token = "KEEPREF"},
                         {.name = "delref", .type = REDISMODULE_ARG_TYPE_PURE_TOKEN, .token = "DELREF"},
                         {.name = "acked", .type = REDISMODULE_ARG_TYPE_PURE_TOKEN, .token = "ACKED"},
                         {0}}},
                {.name = "trim",
                 ...
                 .subargs =
                     (RedisModuleCommandArg[]){
                         ...
                         {.name = "operator",
                          .type = REDISMODULE_ARG_TYPE_ONEOF,
                          .flags = REDISMODULE_CMD_ARG_OPTIONAL,
                          .subargs = (RedisModuleCommandArg[]){{.name = "equal",
                                                                .type = REDISMODULE_ARG_TYPE_PURE_TOKEN,
                                                                .token = "="},
                                                               {.name = "approximately",
                                                                .type = REDISMODULE_ARG_TYPE_PURE_TOKEN,
