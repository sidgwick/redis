...
void loadingProgressCallback(RedisModuleCtx *ctx, RedisModuleEvent e, uint64_t sub, void *data)
{
    ...
    char *keyname =
        (sub == REDISMODULE_SUBEVENT_LOADING_PROGRESS_RDB) ? "loading-progress-rdb" : "loading-progress-aof";
...
#define VerifySubEventSupported(e, s)                                                                                  \
    ...
    if (!RedisModule_IsSubEventSupported(RedisModuleEvent_ReplicationRoleChanged,
